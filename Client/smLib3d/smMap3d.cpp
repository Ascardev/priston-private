...
void SetViewRadian()
{
	...
	for (i = 0; i < 4; i++) 
	{
		ViewRad_X[i] = (CameraAngX + VRad_X[i]) & ANGCLIP;
		ViewRad_Y[i] = (CameraAngY + VRad_Y[i]) & ANGCLIP;
...
int MakeMapTable()
{
	...
	sindY = zDSin((CameraAngY + ANGLE_90) & ANGCLIP);
	cosdY = zDCos((CameraAngY + ANGLE_90) & ANGCLIP);
	for (i = 0; i < 4; i++) 
	{
		if (i == 0 || i == 1) 
		{
