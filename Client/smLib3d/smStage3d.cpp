...
int smGetThroughPlane(POINT3D *p1, POINT3D *p2, POINT3D *p3, POINT3D *sp, POINT3D *ep, POINT3D *cp)
{
	...
	if (denominator != 0)
	{
		t = (((-nx * sp->x + nx * p1->x) + (-ny * sp->y + ny * p1->y) + (-nz * sp->z + nz * p1->z)) << 3) / denominator;
...
int smSTAGE3D::GetPlaneProduct(smSTAGE_FACE *face, POINT3D *p)
{
	...
	result = ((face->VectNormal[0] >> 4) * ((p->x - vp->x) >> 4) + (face->VectNormal[1] >> 4) * ((p->y - vp->y) >> 4) + (face->VectNormal[2] >> 4) * ((p->z - vp->z) >> 4));
...
int smSTAGE3D::GetTriangleImact(smSTAGE_FACE *face, smLINE3D *pLines, int LineCnt)
{
	...
	for (cnt = 0; cnt < LineCnt; cnt++)
	{
		...
		if (((sp->y < p1.y && sp->y < p2.y && sp->y < p3.y) || (sp->y > p1.y && sp->y > p2.y && sp->y > p3.y)) && ((ep->y < p1.y && ep->y < p2.y && ep->y < p3.y) || (ep->y > p1.y && ep->y > p2.y && ep->y > p3.y)))
			flag = FALSE;
...
static int smMakeTLine(POINT3D *Posi, POINT3D *Angle, int dist, int ObjWidth, int ObjHeight, smLINE3D *Lines, POINT3D *ep)
{
	...
	auto sPosition = GetMoveLocation(0, 0, dist, Angle->x, Angle->y, Angle->z);
	ep->x = Posi->x + sPosition.iX;
	ep->y = Posi->y + sPosition.iY;
	ep->z = Posi->z + sPosition.iZ;

	sPosition = GetMoveLocation(0, PosiMinY, 0, Angle->x, Angle->y, Angle->z);
	Lines[0].sp.x = Posi->x + sPosition.iX;
	Lines[0].sp.y = Posi->y + sPosition.iY;
	Lines[0].sp.z = Posi->z + sPosition.iZ;

	sPosition = GetMoveLocation(0, PosiMinY, dist2, Angle->x, Angle->y, Angle->z);
	Lines[0].ep.x = Posi->x + sPosition.iX;
	Lines[0].ep.y = Posi->y + sPosition.iY;
	Lines[0].ep.z = Posi->z + sPosition.iZ;

	sPosition = GetMoveLocation(0, PosiMaxY, 0, Angle->x, Angle->y, Angle->z);
	Lines[1].sp.x = Posi->x + sPosition.iX;
	Lines[1].sp.y = Posi->y + sPosition.iY;
	Lines[1].sp.z = Posi->z + sPosition.iZ;

	sPosition = GetMoveLocation(0, PosiMaxY, dist2, Angle->x, Angle->y, Angle->z);
	Lines[1].ep.x = Posi->x + sPosition.iX;
	Lines[1].ep.y = Posi->y + sPosition.iY;
	Lines[1].ep.z = Posi->z + sPosition.iZ;

	sPosition = GetMoveLocation(-width, PosiMinY, dist2, Angle->x, Angle->y, Angle->z);
	Lines[2].sp.x = Posi->x + sPosition.iX;
	Lines[2].sp.y = Posi->y + sPosition.iY;
	Lines[2].sp.z = Posi->z + sPosition.iZ;

	sPosition = GetMoveLocation(width, PosiMinY, dist2, Angle->x, Angle->y, Angle->z);
	Lines[2].ep.x = Posi->x + sPosition.iX;
	Lines[2].ep.y = Posi->y + sPosition.iY;
	Lines[2].ep.z = Posi->z + sPosition.iZ;

	sPosition = GetMoveLocation(-width, PosiMaxY, dist2, Angle->x, Angle->y, Angle->z);
	Lines[3].sp.x = Posi->x + sPosition.iX;
	Lines[3].sp.y = Posi->y + sPosition.iY;
	Lines[3].sp.z = Posi->z + sPosition.iZ;

	sPosition = GetMoveLocation(width, PosiMaxY, dist2, Angle->x, Angle->y, Angle->z);
	Lines[3].ep.x = Posi->x + sPosition.iX;
	Lines[3].ep.y = Posi->y + sPosition.iY;
	Lines[3].ep.z = Posi->z + sPosition.iZ;
...
static int smMakeXLine(POINT3D *Posi, POINT3D *Angle, int dist, int ObjWidth, int ObjHeight, smLINE3D *Lines, POINT3D *ep)
{
	...
	sPosition = GetMoveLocation(-width, PosiMinY, -width, Angle->x, Angle->y, Angle->z);
	Lines[2].sp.x = ep->x + sPosition.iX;
	Lines[2].sp.y = ep->y + sPosition.iY;
	Lines[2].sp.z = ep->z + sPosition.iZ;

	sPosition = GetMoveLocation(width, PosiMinY, width, Angle->x, Angle->y, Angle->z);
	Lines[2].ep.x = ep->x + sPosition.iX;
	Lines[2].ep.y = ep->y + sPosition.iY;
	Lines[2].ep.z = ep->z + sPosition.iZ;

	sPosition = GetMoveLocation(-width, PosiMaxY, width, Angle->x, Angle->y, Angle->z);
	Lines[3].sp.x = ep->x + sPosition.iX;
	Lines[3].sp.y = ep->y + sPosition.iY;
	Lines[3].sp.z = ep->z + sPosition.iZ;

	sPosition = GetMoveLocation(width, PosiMaxY, -width, Angle->x, Angle->y, Angle->z);
	Lines[3].ep.x = ep->x + sPosition.iX;
	Lines[3].ep.y = ep->y + sPosition.iY;
	Lines[3].ep.z = ep->z + sPosition.iZ;
...
extern Unit *CheckOtherPlayPosi(int x, int y, int z);
...
int smSTAGE3D::CheckVecImpact(POINT3D *Posi, POINT3D *Angle, int dist)
{
	...
	sLine[0].ep.x = Posi->x + sPosition.iX;
	sLine[0].ep.y = Posi->y + sPosition.iY;
	sLine[0].ep.z = Posi->z + sPosition.iZ;
...
int smSTAGE3D::AddVertex(int x, int y, int z)
{
	...
	{
		if (StageMapRect.top > z)
			StageMapRect.top = z;
		if (StageMapRect.bottom < z)
			StageMapRect.bottom = z;
		if (StageMapRect.left > x)
			StageMapRect.left = x;
		if (StageMapRect.right < x)
			StageMapRect.right = x;
	}
...
int smSTAGE3D::AddVertexLightRound(POINT3D *LightPos, int r, int g, int b, int Range)
{
	...
	for (cnt = 0; cnt < nVertex; cnt++)
	{
		...
		if (ddist < dRange)
		{
			...
			if (Vertex[cnt].sDef_Color[SMC_R] > 360)
				Vertex[cnt].sDef_Color[SMC_R] = 360;
			if (Vertex[cnt].sDef_Color[SMC_G] > 360)
				Vertex[cnt].sDef_Color[SMC_G] = 360;
			if (Vertex[cnt].sDef_Color[SMC_B] > 360)
				Vertex[cnt].sDef_Color[SMC_B] = 360;
		}
	}
...
int smSTAGE3D::CheckFaceIceFoot(POINT3D *Pos, POINT3D *Angle, int CheckHeight)
{
	...
	int acnt = MakeAreaFaceList(Pos->x - (fONE * 64 * 1), Pos->z - (fONE * 64 * 1), fONE * 64 * 2, fONE * 64 * 2, Pos->y + num, Pos->y - num);
...
int smSTAGE3D::GetPolyHeight(smSTAGE_FACE *face, int x, int z)
{
	...
	if (lz != 0)
		lx = ((pbot->x - ptop->x) << FLOATNS) / lz;
	else
		lx = 0;
	if (tz != 0)
		tx = ((pmid->x - ptop->x) << FLOATNS) / tz;
	else
		tx = 0;
	if (bz != 0)
		bx = ((pbot->x - pmid->x) << FLOATNS) / bz;
	else
		bx = 0;
	...
	if (lz != 0)
		ly = ((pbot->y - ptop->y) << FLOATNS) / lz;
	else
		ly = 0;
	if (tz != 0)
		ty = ((pmid->y - ptop->y) << FLOATNS) / tz;
	else
		ty = 0;
	if (bz != 0)
		by = ((pbot->y - pmid->y) << FLOATNS) / bz;
	else
		by = 0;
...
// µØÍ¼·¶Î§
#define STAGE_RECT_LIMIT		(30*2*64*fONE)
#define STAGE_RECT_LIMIT2		(240*2*64*fONE)
...
int smSTAGE3D::DrawStage(int x, int y, int z, int angX, int angY, int angZ, smEMATRIX *eRotMatrix)
{
	...
	if (z<(StageMapRect.top - STAGE_RECT_LIMIT) || z>(StageMapRect.bottom + STAGE_RECT_LIMIT) || x<(StageMapRect.left - STAGE_RECT_LIMIT) || x>(StageMapRect.right + STAGE_RECT_LIMIT))
		return FALSE;
	...
	CameraAngX = angX & ANGCLIP;
	CameraAngY = angY & ANGCLIP;
	CameraAngZ = angZ & ANGCLIP;
...
int smSTAGE3D::DrawStage2(int x, int y, int z, int angX, int angY, int angZ, smEMATRIX *eRotMatrix)
{
	...
	if (z<(StageMapRect.top - STAGE_RECT_LIMIT) || z>(StageMapRect.bottom + STAGE_RECT_LIMIT) || x<(StageMapRect.left - STAGE_RECT_LIMIT) || x>(StageMapRect.right + STAGE_RECT_LIMIT) ||
		z<(StageMapRect.bottom - STAGE_RECT_LIMIT2) || z>(StageMapRect.top + STAGE_RECT_LIMIT2) || x<(StageMapRect.right - STAGE_RECT_LIMIT2) || x>(StageMapRect.left + STAGE_RECT_LIMIT2))
		return FALSE;
...
static char *szSMDOldFileHeader = "SMD Stage data Ver 0.71";
...
int	smSTAGE3D::SaveFile(char *szFile)
{
	...
	if (smMaterialGroup)
		smMaterialGroup->SaveFile(hFile);
