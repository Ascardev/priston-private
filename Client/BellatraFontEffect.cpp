...
static smTEXTUREHANDLE		    *s_BL_lpDDSurfaceBuff[DMAX_BELLATRA_DDS_IMAGELIST];
...
static void DrawEventScore(void)
{
	...
	{
		DrawSpecialFontImage(g_EventScore.TeamMarkBuff[i], int(s_EventScoreTextInfo[iTextPos].TextMarkPos.x * g_fWinSizeRatio_X), int((s_EventScoreTextInfo[iTextPos].TextMarkPos.y + yPos) * g_fWinSizeRatio_Y), D3DCOLOR_XRGB(255, 255, 255));

		lpName = g_lpBellatraTeamNameText[g_EventScore.TeamMarkBuff[i]];
		dsTextLineOut(0, int(s_EventScoreTextInfo[iTextPos].TextNamePos.x * g_fWinSizeRatio_X), int((s_EventScoreTextInfo[iTextPos].TextNamePos.y + yPos) * g_fWinSizeRatio_Y), lpName, lstrlen(lpName), D3DCOLOR_XRGB(255, 246, 196));
		...
		dsTextLineOut(0, lpName, lstrlen(lpName), &rcRect, D3DCOLOR_XRGB(255, 246, 196));
	...
	if (g_EventScore.EntryTeamNum > 0)
	{
		...
		if (g_EventScore.EntryTeamNum == 1)
		{
			...
			wsprintf(szBuff, "%s won", lpName_1);
		}
		else
		{
			lpName_1 = g_lpBellatraTeamNameText[g_EventScore.EntryTeamMarkBuff[0]];
			lpName_2 = g_lpBellatraTeamNameText[g_EventScore.EntryTeamMarkBuff[1]];
			wsprintf(szBuff, "%s and %s won", lpName_1, lpName_2);
		}
		dsTextLineOut(0, int(490 * g_fWinSizeRatio_X), int(320 * g_fWinSizeRatio_Y), szBuff, lstrlen(szBuff), D3DCOLOR_XRGB(255, 216, 0));
		dsTextLineOut(0, int(490 * g_fWinSizeRatio_X), int(350 * g_fWinSizeRatio_Y), "Loading", lstrlen("Loading"), D3DCOLOR_XRGB(255, 216, 0));
		dsTextLineOut(0, int(490 * g_fWinSizeRatio_X), int(380 * g_fWinSizeRatio_Y), "Next Stage!", lstrlen("Next Stage!"), D3DCOLOR_XRGB(255, 216, 0));
	}
}
void CreateBellatraFontEffect(void)
{
	...
	for (i = 0; i < DMAX_BELLATRA_TEX_IMAGELIST; i++)
	{
		...
		s_BL_ImageMatGroup.CreateTextureMaterial(szFullName, NULL, 0, 0, SMMAT_BLEND_ALPHA);
	}
		...
		for (int loop = 0; loop < DMAX_BL_MIN_EFFECT; loop++)
		{
			if (lpMinFontEffectPtrBuff[loop].Life)
			{
				...
				if (ImageNum >= 0)
				{
					...
					{
						if (dest_x + lpMinFontEffectPtr->Size.cx <= 0 || dest_y + lpMinFontEffectPtr->Size.cy <= 0 || dest_x >= 800 || dest_y >= 600)
							continue;
						...
						DrawFzmBackToSurface(dest_x, dest_y, s_BL_lpDDSurfaceBuff[ImageNum], x, y, width, height);
					}
					else
					{
