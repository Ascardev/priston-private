...
namespace MESSAGEBOX
{
	...
	void Model::SetEvent(std::function<void(UIEventArgs const &)> const & f)
	{
		m_vMsgBox.clear();
		m_vMsgBox.push_back(f);
	}
	void Model::SetTitle(std::string strText)
	{
	...
	void Model::SetDescription(std::string strText)
	{
		m_pWindow ? m_pWindow->GetElement<UI::Text>(TEXTID_Text)->SetText(strText) : 0;
	}
	void Model::SetAutoClose(int iDuration)
	{
		if (m_pWindow)
		{
	...
	void Model::Show()
	{
		if (m_pWindow && !m_pWindow->IsOpen())
		{
			...
			m_pJoinElement ? m_pJoinElement->OnEnjoin() : 0;
			MOUSEHANDLER->BlockMouse();
		}
	}
	void Model::AddExtraText(std::string strText, Rectangle2D sBox, BOOL bBold, UI::Color cColor)
	{
	...
	void Model::SetBoxType(int iType)
	{
		m_iBoxType = iType;
	...
	BOOL Model::OnKeyPress(INPUTS::LPKEYBOARD pcKeyboard)
	{
		if (m_iBoxType == BOXTYPEID_RollDice)
			return FALSE;
	...
	void Model::OnYesButtonClick(UIEventArgs eArgs)
	{
		MOUSEHANDLER->UnblockMouse();

		m_bType = TRUE;

		m_pJoinElement ? m_pJoinElement->UnEnjoin() : 0;
	...
	void Model::OnNoButtonClick(UIEventArgs eArgs)
	{
		MOUSEHANDLER->UnblockMouse();
	...
	void Model::OnRollButtonClick(UIEventArgs eArgs)
	{
		OnEvent();
	...
	void Model::OnPassButtonClick(UIEventArgs eArgs)
	{
		OnEvent();
	...
	void Model::BuildWindow()
	{
		...
		pButtonYes->SetEvent(UI::Event::Build(std::bind(&Model::OnYesButtonClick, this, std::placeholders::_1)));
		...
		pButtonNo->SetEvent(UI::Event::Build(std::bind(&Model::OnNoButtonClick, this, std::placeholders::_1)));
		...
		//Text
		UI::LPTEXT pTextDescription = std::make_shared<UI::Text>(Rectangle2D(35, 50, 160, 44));
		...
		UI::LPTEXT pTextAutoClose = std::make_shared<UI::Text>(Rectangle2D(10, 95, 210, 60));
	...
	void Model::BuildWindowRollDice()
	{
