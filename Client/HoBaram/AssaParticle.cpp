...
void cASSAPARTSHOT::Draw(POINT3D *pPosi, POINT3D *pAngle)
{
	if (Face.MatNum)
	{
		...
		else if (EffectType == ASSA_SHOT_LASER || EffectType == ASSA_SHOT_LASER2 || EffectType == ASSA_SHOT_LASER3 || EffectType == ASSA_SHOT_LASER4 || EffectType == ASSA_SHOT_LASER5)
		{
...
void cASSAPARTSHOT::Main()
{
	...
	{
		auto sPosition = GetMoveLocation(0, 0, 6 * fONE, ShootingAngle.x, ShootingAngle.y, ShootingAngle.z);
		Posi.x += sPosition.iX;
		Posi.y += sPosition.iY;
		Posi.z += sPosition.iZ;
	...
	else if (EffectType == SKILL_S_FEAR)
	{
		auto sPosition = GetMoveLocation(0, 0, 6 * fONE, ShootingAngle.x, ShootingAngle.y, ShootingAngle.z);
		Posi.x += sPosition.iX;
		Posi.y += sPosition.iY;
		...
		if (length < 15)
		{
			if (PartNum != -1)
			{
				...
				Time = Max_Time;
				sinEffect_SkillHit(SKILL_S_FEAR, &Posi);
			}
		}
...
void cASSAPARTSHOT::Start(POINT3D *pCurPosi, POINT3D *pDestPosi, cASSAEFFECT *pEffect, int type)
{
	...
	else if (EffectType == SKILL_TWIST_JAVELIN)
	{
		...
		auto sPosition = GetRadian3D(Posi.x, Posi.y, Posi.z, DesPosi.x, DesPosi.y, DesPosi.z);
...
void cASSAPARTSHOT::SetChainLighting(Unit **pCharList, int count)
{
...
void cASSAPARTSHOT::Start(Unit *pChar, POINT3D *pDestPosi, int type)
{
	Time = 0;
	Face.width = 4000;
	Face.x = Posi.x;
	...
	if (EffectType == SKILL_FIRE_JAVELIN)
	{
		DesPosi = *pDestPosi;
		DesPosi.y -= 28 * fONE;
		auto sPosition = GetRadian3D(pChar->pX, pChar->pY, pChar->pZ, DesPosi.x, DesPosi.y, DesPosi.z);
		int angX = sPosition.iX;
		int angY = sPosition.iY;
...
void cASSAPARTSHOT::Start(POINT3D *pCurPosi, POINT3D *pDestPosi, int type)
{
	...
	if (EffectType == ASSA_SHOT_LASER || EffectType == ASSA_SHOT_LASER2 || EffectType == ASSA_SHOT_LASER3 || EffectType == ASSA_SHOT_LASER4 || EffectType == ASSA_SHOT_LASER5)
	{
		Face.r = 200;
		Face.g = 200;
		Face.b = 200;
		...
		else if (EffectType == ASSA_SHOT_LASER4)
			matNum = (int *)AssaSearchRes("Ray03.bmp", SMMAT_BLEND_LAMP);
		else if (EffectType == ASSA_SHOT_LASER5)
	...
	else if (EffectType == SKILL_FIRE_JAVELIN)
	{
		...
		DesPosi.y += 4 * fONE;
		auto sPosition = GetRadian3D(pCurPosi->x, pCurPosi->y, pCurPosi->z, DesPosi.x, DesPosi.y, DesPosi.z);
		...
		sPosition = GetMoveLocation(0, 0, 12 * fONE, ShootingAngle.x, ShootingAngle.y, ShootingAngle.z);
		Posi.x += sPosition.iX;
		Posi.y += sPosition.iY;
		Posi.z += sPosition.iZ;
	...
	else if (EffectType == SKILL_S_FEAR)
	{
		...
		PartNum = g_NewParticleMgr.Start("Skill5AtalantaSnippyFearShot", Posi);
	}
}
...
bool HoGetWeaponPos(Unit *character, WEAPON_PART_POS *partPos)
{
	if (character == NULL)
		return false;
	...
	else if (character->HvLeftHand.dwItemCode != 0)
	{
		...
		{
			auto sPosition = GetMoveLocation(pBot[0].x, pBot[0].y, pBot[0].z, 0, ANGLE_90 & ANGCLIP, 0);
			pBot[0].x = sPosition.iX;
			pBot[0].y = sPosition.iY;
			pBot[0].z = sPosition.iZ;
		}
	}
	else
		return false;
	...
	if (character->HvRightHand.dwItemCode != 0)
		...
		AnimObjectTree(character->HvLeftHand.ObjBip, character->iFrame, angle.x, angle.y, angle.z);
	else
		return false;
	...
	for (index = 0; index < partPos->posCount; index++)
	{
		rx = pBot[index].x * mWorld->_11 + pBot[index].y * mWorld->_21 + pBot[index].z * mWorld->_31;
		ry = pBot[index].x * mWorld->_12 + pBot[index].y * mWorld->_22 + pBot[index].z * mWorld->_32;
		rz = pBot[index].x * mWorld->_13 + pBot[index].y * mWorld->_23 + pBot[index].z * mWorld->_33;
...
bool HoGetJavelinWeaponPos(Unit *character, WEAPON_PART_POS *partPos)
{
	if (character == NULL)
		return false;
...
void cASSAChar::Main()
{
	...
	if ((EffectType == ENCHANT_WEAPON_FIRE || EffectType == ENCHANT_WEAPON_ICE || EffectType == ENCHANT_WEAPON_LIGHT || EffectType == IMPULSION_LIGHT) && FadeStopFlag == false)
	{
		...
		if (pChar->HvRightHand.dwItemCode == 0 && pChar->HvLeftHand.dwItemCode == 0 ||
			(pChar->iLoadedMapIndex >= 0 && STAGE_MAP[pChar->iLoadedMapIndex]->iMapType == MAPTYPE_Town) ||
			(!pChar->psModelAnimation || pChar->psModelAnimation->iType == ANIMATIONTYPE_Die))
		{
			for (int index = 0; index < 3; index++)
			{
				if (EnchantPartNum[index] != -1)
		...
		ZeroMemory(&PART_POS, sizeof(PART_POS));
		...
		if ((ItemCode != pChar->HvRightHand.dwItemCode && pChar->HvLeftHand.dwItemCode == 0) || (ItemCode != pChar->HvLeftHand.dwItemCode && pChar->HvRightHand.dwItemCode == 0))
		{
	...
	if (EffectType == HOLY_VALOR_JANG || EffectType == HOLY_VALOR_MEMBER || EffectType == DRASTIC_SPIRIT_JANG || EffectType == DRASTIC_SPIRIT_MEMBER)
	{
		D3DVECTOR partPos;
	...
	if (EffectType == MONSTER_SERQBUS_STATE1 || EffectType == MONSTER_SERQBUS_STATE2 || EffectType == MONSTER_SERQBUS_STATE3 || EffectType == MONSTER_POSION_STATE1 || EffectType == HOLY_MIND_ATTACK)
	{
		if (FlagShow != pChar->FlagShow)
		{
			FlagShow = pChar->FlagShow;
	...
	if (!MyCharacterFlag)
	{
		if (pChar->bActive)
		{
			if (!pChar->psModelAnimation || pChar->psModelAnimation->iType == ANIMATIONTYPE_Die)
				StopEffect();
		}
	}
...
void cASSAChar::Start(Unit *character, int code, int maxTime, float delay)
{
	if (character == NULL)
		return;
	...
	if (pChar->iLoadedMapIndex >= 0 && STAGE_MAP[pChar->iLoadedMapIndex]->iMapType == MAPTYPE_Town)
		return;
	...
	CODE = code;
	if (EffectType == ENCHANT_WEAPON_FIRE || EffectType == ENCHANT_WEAPON_LIGHT || EffectType == ENCHANT_WEAPON_ICE)
	{
		CODE = ENCHANT_WEAPON;
...
void cASSAItem::Start(Unit *character, int code)
{
	if (character == NULL)
		return;
...
bool SearchObjLocalPos(Unit *character, smOBJ3D *obj, POINT3D *outObjPos)
{
	if (character == NULL ||
		obj == NULL)
		return false;
...
bool SearchObjLocalPos(Unit *character, smOBJ3D *obj, POINT3D *outObjPos, int length)
{
...
void cAssaTracePatMeshParticle::Start(Unit *character, smPAT3D *pat, smOBJ3D *searchObj, char *particleName, int liveTime)
{
...
void cAssaTraceCharMeshParticle::Start(Unit *character, char *meshName, char *particleName, bool attachFlag, int liveTime)
{
	if (character == NULL)
		return;
...
{
	if (!MyCharacterFlag && pChar->bActive == FALSE)
	{
		if (PartNum != -1)
		{
			g_NewParticleMgr.SetFastStop(PartNum);
...
void cAssaMotionBlur::Draw(POINT3D *pPosi, POINT3D *pAngle)
{
	...
	{
		pframe = pChar->iFrame - (cnt*mFrames);
		if (pframe < pChar->psModelAnimation->StartFrame)
			pframe = pChar->psModelAnimation->StartFrame;
		vcnt = cnt << 1;
...
void cAssaMotionBlur::Start(Unit *character, char *meshName1, char *meshName2, int liveTime)
{
	if (character == NULL)
		return;
	...
	if (pChar && pChar->sCharacterData.iMonsterEffectID == MONSTEREFFECTID_Draxos)
		matNum = *(int *)AssaSearchRes("m_DoomG-01.bmp", SMMAT_BLEND_LAMP);
	else
		matNum = *(int *)AssaSearchRes("m_DoomG-01.bmp", SMMAT_BLEND_LAMP);
...
void cAssaTwoPointAni::Start(Unit *character, char *meshName1, char *meshName2)
{
	if (character == NULL)
		return;
	...
	lstrcpy(szFileName, "light_G001.bmp");
...
void AssaSkill3Concentration::Start(Unit* character, int liveCount, int light)
{
	pChar = character;
	if (pChar == NULL)
		return;
...
void AssaSkill3Concentration::Main()
{
	...
	if (EndFlag == FALSE)
	{
		if (Time > Max_Time - 140 ||
			(!MyCharacterFlag && pChar->bActive == FALSE) ||
			(!pChar->psModelAnimation || pChar->psModelAnimation->iType == ANIMATIONTYPE_Die) ||
...
void AssaSkill3SwiftAxe::Start(Unit* character, int liveCount, int light)
{
	pChar = character;
	if (pChar == NULL)
		return;
...
void AssaSkill3SwiftAxe::Main()
{
	...
	if (EndFlag == FALSE)
	{
		if (Time > Max_Time - 140 || (!MyCharacterFlag && pChar->bActive == FALSE) ||
			(!pChar->psModelAnimation || pChar->psModelAnimation->iType == ANIMATIONTYPE_Die) ||
...
void AssaSkillWeaponMotionBlur::Draw(POINT3D *pPosi, POINT3D *pAngle)
{
	...
	for (cnt = 0; cnt < mLevel; cnt++)
	{
		...
		rx = weaponTopPos.x * mWorld->_11 + weaponTopPos.y * mWorld->_21 + weaponTopPos.z * mWorld->_31;
		ry = weaponTopPos.x * mWorld->_12 + weaponTopPos.y * mWorld->_22 + weaponTopPos.z * mWorld->_32;
		rz = weaponTopPos.x * mWorld->_13 + weaponTopPos.y * mWorld->_23 + weaponTopPos.z * mWorld->_33;
		...
		rx = weaponBottomPos.x * mWorld->_11 + weaponBottomPos.y * mWorld->_21 + weaponBottomPos.z * mWorld->_31;
		ry = weaponBottomPos.x * mWorld->_12 + weaponBottomPos.y * mWorld->_22 + weaponBottomPos.z * mWorld->_32;
		rz = weaponBottomPos.x * mWorld->_13 + weaponBottomPos.y * mWorld->_23 + weaponBottomPos.z * mWorld->_33;
...
void AssaSkillWeaponMotionBlur::Start(Unit *character, int liveTime, int Flag)
{
	if (character == NULL)
		return;
	...
	{
	case 0:
		matNum = *(int *)AssaSearchRes("ft3_boneCrash.bmp", SMMAT_BLEND_LAMP);
		break;
	case 1:
		matNum = *(int *)AssaSearchRes("ft5_DownHit.bmp", SMMAT_BLEND_LAMP);
		break;
	case 2:
		matNum = *(int *)AssaSearchRes("light3_02.bmp", SMMAT_BLEND_LAMP);
		break;
	case 3:
		matNum = *(int *)AssaSearchRes("Ray03.bmp", SMMAT_BLEND_LAMP);
		break;
...
{
	auto sPosition = GetMoveLocation(0, 0, 7 * fONE, ShootingAngle.x, ShootingAngle.y, ShootingAngle.z);
	Posi.x += sPosition.iX;
	Posi.y += sPosition.iY;
	Posi.z += sPosition.iZ;
...
void AssaSkill3BombShot::Start(POINT3D *pCur, POINT3D *pDest)
{
	...
	auto sPosition = GetRadian3D(Posi.x, Posi.y, Posi.z, DesPosi.x, DesPosi.y, DesPosi.z);
	ShootingAngle.x = sPosition.iX & ANGCLIP;
	ShootingAngle.y = sPosition.iY & ANGCLIP;
	ShootingAngle.z = sPosition.iZ & ANGCLIP;

	sPosition = GetMoveLocation(0, 0, 40 * fONE, ShootingAngle.x, ShootingAngle.y, ShootingAngle.z);
	Posi.x += sPosition.iX;
	Posi.y += sPosition.iY;
	Posi.z += sPosition.iZ;
...
void AssaSkill3Perforation::Main()
{
	...
	auto sPosition = GetMoveLocation(0, 0, 10 * fONE, ShootingAngle.x, ShootingAngle.y, ShootingAngle.z);
	Posi.x += sPosition.iX;
	Posi.y += sPosition.iY;
	Posi.z += sPosition.iZ;
...
void AssaSkill3SparkShield::Start(Unit* character, int liveCount)
{
	pChar = character;
	if (pChar == NULL)
		return;
...
void AssaSkill3MetalArmor::Start(Unit* character, int liveCount)
{
	pChar = character;
	if (pChar == NULL)
		return;
...
void AssaSkill3MetalArmor::Main()
{
	...
	if (EndFlag == FALSE)
	{
		if (Time > Max_Time - 140 || (!MyCharacterFlag && pChar->bActive == FALSE) || (!pChar->psModelAnimation || pChar->psModelAnimation->iType == ANIMATIONTYPE_Die) ||
...
void AssaSkill3Vanish::Start(Unit* character)
{
	pChar = character;
	if (pChar == NULL)
		return;
...
void AssaSkill3EnergyShield::Start(Unit* character, int liveCount)
{
	pChar = character;
	if (pChar == NULL)
		return;
...
void AssaSkill3EnergyShieldDefence::Start(Unit* character)
{
	pChar = character;
	if (pChar == NULL)
		return;
...
void AssaSkill3SpiritElemental::Start(Unit *character, int liveCount)
{
	pChar = character;
	if (pChar == NULL)
		return;
...
void AssaSkill3DancingSword::Start(Unit *character, int type, int liveCount, int level)
{
	pChar = character;
	if (pChar == NULL)
		return;
...
void AssaSkill3DancingSword::Main()
{
	...
	if (State == DANCING_ATTACK_START1)
	{
		...
		if (CurrentFrame == 160 * 39)
		{
			if (AttackMonster)
			{
				...
				auto sPosition = GetRadian3D(Posi.x, Posi.y, Posi.z, AttackDest.x, AttackDest.y, AttackDest.z);
				Angle.y = (-sPosition.iY + ANGLE_180)&ANGCLIP;
			}
		}
	...
	if (State == DANCING_ATTACK_START2)
	{
		...
		auto sPosition = GetRadian3D(attackCurrent.x, attackCurrent.y, attackCurrent.z, AttackDest.x, AttackDest.y, AttackDest.z);
	...
	{
		auto sPosition = GetMoveLocation(0, 0, fONE * 7, AttackAngle.x, AttackAngle.y, 0);
		AttackLocalPosi.x += sPosition.iX;
		AttackLocalPosi.y += sPosition.iY;
		AttackLocalPosi.z += sPosition.iZ;
...
void AssaSkill3DivineInhalation::Start(Unit* character, int liveCount)
{
	pChar = character;
	if (pChar == NULL)
		return;
...
void AssaSkill3HolyIncantationShot::Start(Unit *character)
{
	pChar = character;
	POINT3D charPos;
	charPos.x = pChar->pX;
...
void AssaSkill3HolyIncantationPart::Start(Unit* character, int liveCount)
{
	pChar = character;
	if (pChar == NULL)
		return;
...
void AssaSkill3HolyIncantationPartHand::Start(Unit* character, int liveCount)
{
	pChar = character;
	if (pChar == NULL)
		return;
...
void AssaSkill3FlameBrandish::Start(Unit *character)
{
	Angle.y = (-character->Angle.y)&ANGCLIP;
	auto sPosition = GetMoveLocation(0, 0, 55 * fONE, 0, -Angle.y, 0);


	Posi.x = character->pX + sPosition.iX;
	Posi.y = character->pY + 50;
	Posi.z = character->pZ + sPosition.iZ;
...
void AssaSkill3VigorBall::Start(POINT3D *pCur, Unit *destChar, D3DVECTOR *velocity, int delay)
{
	Pos.x = (float)pCur->x;
	Pos.y = (float)pCur->y;
	Pos.z = (float)pCur->z;
...
void AssaSkill3ResurrectionShot::Start(Unit *character, int delay)
{
...
void AssaSkill3ResurrectionPart::Start(Unit* character, int liveCount)
{
	pChar = character;
	if (pChar == NULL)
		return;
...
void AssaSkill3Extinction::Start(Unit* character)
{
	pChar = character;
	if (pChar == NULL)
		return;
...
void AssaSkillVirtualLifeMember::Start(Unit *character, int liveCount)
{
	pChar = character;
	if (pChar == NULL)
		return;
...
void AssaSkillTriumphOfValhallaPart::Start(Unit *character, int liveCount)
{
	pChar = character;
	if (pChar == NULL)
		return;
...
void AssaSkillTriumphOfValhallaPart::StartHallOfValhalla(Unit *character, int liveCount)
{
	pChar = character;
	if (pChar == NULL)
		return;
...
void AssaSkillTriumphOfValhallaPart::Main()
{
	...
	if (ParticleID != -1)
	{
		if (ObjBip != NULL)
		{
			...
			AnimObjectTree(ObjBip, pChar->iFrame, 0, (-pChar->Angle.y + ANGLE_180)&ANGCLIP, 0);
			mWorld = &ObjBip->mWorld;
	...
	if (ParticleIDExt != -1)
	{
		if (ObjBipExt != NULL)
		{
			...
			AnimObjectTree(ObjBipExt, pChar->iFrame, 0, (-pChar->Angle.y + ANGLE_180)&ANGCLIP, 0);
			mWorld = &ObjBip->mWorld;
...
{
	smMATRIX *mWorld;
	AnimObjectTree(SearchObj, DestChar->iFrame, 0, (-DestChar->Angle.y + ANGLE_180)&ANGCLIP, 0);
	mWorld = &SearchObj->mWorld;
	...
	float length = term.x*term.x + term.y*term.y + term.z*term.z;
	length = (float)sqrt(length);
	if (length < 15 || (!DestChar->psModelAnimation || DestChar->psModelAnimation->iType == ANIMATIONTYPE_Die) || Time > 100)
	{
		if (ParticleID != -1)
		{
			g_NewParticleMgr.SetRendering(ParticleID, FALSE);
...
void AssaSkillTriumphOfValhallaShot::Start(Unit *curChar, Unit *destChar, D3DVECTOR *velocity, int liveCount)
{
	int *matNum = (int *)AssaSearchRes("shield1.bmp", SMMAT_BLEND_LAMP);
	if (matNum == NULL)
		return;
...
void AssaSkill3LightingJavelin::StartShort(POINT3D *pCur, POINT3D *pDest)
{
	...
	DesPosi.y += 4 * fONE;
	auto sPosition = GetRadian3D(pCur->x, pCur->y, pCur->z, DesPosi.x, DesPosi.y, DesPosi.z);
...
void AssaSkill3LightingJavelin::StartRope(POINT3D *pCur, POINT3D *pDest)
{
	Posi = *pCur;
	Posi.y -= 1000;
	DesPosi = *pDest;
...
void AssaSkill3StormJavelin::Start(POINT3D *pCur, POINT3D *pDest)
{
	...
	sPosition = GetMoveLocation(0, 0, 30 * fONE, ShootingAngle.x, ShootingAngle.y, ShootingAngle.z);
	Posi.x += sPosition.iX;
	Posi.y += sPosition.iY;
	Posi.z += sPosition.iZ;
...
void AssaSkill3SplitJavelin::Start(POINT3D *pCur, POINT3D *pDest, int delay)
{
	...
	ScalePosi.x = sPosition.iX;
	ScalePosi.y = sPosition.iY;
	ScalePosi.z = sPosition.iZ;
...
AssaElementWeapon::AssaElementWeapon()
{
	pChar = NULL;
	MyCharacterFlag = FALSE;
...
void AssaElementWeapon::Start(Unit *character, int Kind, int iHand)
{
	pChar = character;
	...
	ZeroMemory(&charPos, sizeof(POINT3D));

	switch (Kind)
	{
	case 1:
		for (int i = 0; i < MAX_WEAPON_POS; i++)
			ParticleIDGroup[i] = g_NewParticleMgr.Start("ElementWeaponFire", charPos);
		break;
	case 2:
		for (int i = 0; i < MAX_WEAPON_POS; i++)
			ParticleIDGroup[i] = g_NewParticleMgr.Start("ElementWeaponIce", charPos);
		break;
	case 3:
		for (int i = 0; i < MAX_WEAPON_POS; i++)
			ParticleIDGroup[i] = g_NewParticleMgr.Start("ElementWeaponLight", charPos);
		break;
	}
...
void AssaElementWeapon::Main()
{
	...
	if (Hand == RIGHTHAND && pChar->HvRightHand.dwItemCode != 0)
	{
		...
		if ((pChar->HvRightHand.dwItemCode & sinITEM_MASK2) == sinWT1)
		{
			...
			WeaponSize = int((float)pChar->HvRightHand.SizeMax*1.2f);
			WeaponPosCount = WeaponSize / 1000;

			if (WeaponPosCount < MAX_WEAPON_POS)
			{
				for (index = 0; index < WeaponPosCount; index++)
		...
		else if ((pChar->HvRightHand.dwItemCode & sinITEM_MASK2) == sinWS1)
		{
			...
			for (index = 0; index < MAX_WEAPON_POS; index++)
			{
				...
				auto sPosition = GetMoveLocation(WeaponPos[index].x, WeaponPos[index].y, WeaponPos[index].z, 0, ANGLE_45&ANGCLIP, 0);

				WeaponPos[index].x += sPosition.iX;
				WeaponPos[index].y += sPosition.iY;
				WeaponPos[index].z += sPosition.iZ;
			}
		}
		...
		{
			StartZPos = 1500;
			WeaponPosCount = (WeaponSize - 2500) / 1000;

			if (WeaponSize > 14000)
				WeaponPosCount = (WeaponSize - 5500) / 1000;
		...
		else if ((pChar->HvRightHand.dwItemCode & sinITEM_MASK2) == sinWD1)
		{
			...
			if (WeaponSize > 10000)
				WeaponPosCount = (WeaponSize - 2000) / 1000;
		...
		else
		{
			...
			if (WeaponPosCount < MAX_WEAPON_POS)
			{
				...
				{
					WeaponPos[index].z = StartZPos + index * 1000;
					if ((pChar->HvRightHand.dwItemCode & sinITEM_MASK3) == sin15)
						WeaponPos[index].y += 750;
				}
			}
		}
	}
	else if (Hand == LEFTHAND && pChar->HvLeftHand.dwItemCode != 0)
	{
		...
		else if ((pChar->HvLeftHand.dwItemCode & sinITEM_MASK2) == sinWD1)
		{
			StartZPos = 0;
			WeaponSize = int((float)pChar->HvLeftHand.SizeMax*1.2f);
			WeaponPosCount = WeaponSize / 1000;
	...
	if (pChar->HvRightHand.dwItemCode != 0)
		AnimObjectTree(pChar->HvRightHand.ObjBip, pChar->iFrame, angle.x, angle.y, angle.z);

	else if (pChar->HvLeftHand.dwItemCode != 0)
		AnimObjectTree(pChar->HvLeftHand.ObjBip, pChar->iFrame, angle.x, angle.y, angle.z);

	int rx, ry, rz;

	D3DVECTOR charPos[MAX_WEAPON_POS];

	if (WeaponPosCount && WeaponPosCount < MAX_WEAPON_POS)
	{
		if (mWorld)
		{
			for (index = 0; index < WeaponPosCount; index++)
			{
				rx = WeaponPos[index].x * mWorld->_11 + WeaponPos[index].y * mWorld->_21 + WeaponPos[index].z * mWorld->_31;
				ry = WeaponPos[index].x * mWorld->_12 + WeaponPos[index].y * mWorld->_22 + WeaponPos[index].z * mWorld->_32;
				rz = WeaponPos[index].x * mWorld->_13 + WeaponPos[index].y * mWorld->_23 + WeaponPos[index].z * mWorld->_33;

				charPos[index].x = (float)(pChar->pX + (rx >> FLOATNS) + mWorld->_41);
				charPos[index].z = (float)(pChar->pZ + (ry >> FLOATNS) + mWorld->_42);
				charPos[index].y = (float)(pChar->pY + (rz >> FLOATNS) + mWorld->_43);

				if (ParticleIDGroup[index] != -1)
					g_NewParticleMgr.SetPos(ParticleIDGroup[index], charPos[index]);
			}
		}
	}
	...
	if (EndFlag == FALSE)
	{
		...
		if (pChar->dwActionItemCode == pChar->HvRightHand.dwItemCode)
			flag = 2;
		if (pChar->HvLeftHand.dwItemCode == pChar->HvRightHand.dwItemCode)
			flag = 3;
		if (pChar->HvLeftHand.ElementEffect == 0)
			flag = 3;

		if ((!MyCharacterFlag && pChar->bActive == FALSE) || ((flag == 1 || flag == 3) && pChar->HvLeftHand.ElementEffect == 0 && Hand == LEFTHAND) || ((flag == 2 || flag == 3) && pChar->HvRightHand.ElementEffect == 0 && (Hand == RIGHTHAND)))
		{
			Time = Max_Time;
			StopParticle();
			StopParticleID();
...
void AssaElementWeapon::StopParticle()
{
	for (int index = 0; index < MAX_WEAPON_POS; index++)
	{
		if (ParticleIDGroup[index] != -1)
...
void AssaElementWeapon::StopParticleID()
{
	for (int index = 0; index < MAX_WEAPON_POS; index++)
	{
		if (ParticleIDGroup[index] != -1)
...
void AssaClassUpWeapon::Start(Unit* character)
{
	pChar = character;
	if (pChar == NULL)
		return;
...
{
	if (ParticleIDGroup[0] == -1 || ParticleIDGroup[1] == -1 || ParticleIDGroup[2] == -1 || ParticleIDGroup[3] == -1)
	{
		POINT3D charPos;
		charPos.x = pChar->pX;
		charPos.y = pChar->pY;
...
void AssaClassUpWeapon::Main()
{
	...
	if (pChar->HvRightHand.dwItemCode != 0)
	{
		...
		else if ((pChar->HvRightHand.dwItemCode & sinITEM_MASK2) == sinWS1)
		{
			...
			if (pChar->HvRightHand.dwItemCode != 0)
			{
				auto sPosition = GetMoveLocation(weaponPos.x, weaponPos.y, weaponPos.z, 0, ANGLE_45&ANGCLIP, 0);
				weaponPos.x = sPosition.iX;
				weaponPos.y = sPosition.iY;
				weaponPos.z = sPosition.iZ;
			}
		}
		else
	...
	else if (pChar->HvLeftHand.dwItemCode != 0)
	{
		if ((pChar->HvLeftHand.dwItemCode & sinITEM_MASK2) == sinWS1)
		{
			...
			{
				auto sPosition = GetMoveLocation(weaponPos.x, weaponPos.y, weaponPos.z, 0, ANGLE_90&ANGCLIP, 0);
				weaponPos.x = sPosition.iX;
				weaponPos.y = sPosition.iY;
				weaponPos.z = sPosition.iZ;
			}
	...
	if (pChar->HvRightHand.dwItemCode != 0)
	{
		if ((pChar->HvRightHand.dwItemCode & sinITEM_MASK2) == sinWC1 || (pChar->HvRightHand.dwItemCode & sinITEM_MASK2) == sinWS1)
			sPosition = GetMoveLocation(0, LocalPos.y, 0, LocalAngle, 0, 0);
		else
			sPosition = GetMoveLocation(0, LocalPos.y, 0, 0, 0, LocalAngle);
	}
	else if (pChar->HvLeftHand.dwItemCode != 0)
		sPosition = GetMoveLocation(0, LocalPos.y, 0, LocalAngle, 0, 0);

	part1Pos.x += sPosition.iX;
	part1Pos.y += sPosition.iY;
	part1Pos.z += sPosition.iZ;
	...
	part2Pos.x += sPosition.iX;
	part2Pos.y += sPosition.iY;
	part2Pos.z += sPosition.iZ;
	...
	int rx, ry, rz;
	rx = part1Pos.x * mWorld->_11 + part1Pos.y * mWorld->_21 + part1Pos.z * mWorld->_31;
	ry = part1Pos.x * mWorld->_12 + part1Pos.y * mWorld->_22 + part1Pos.z * mWorld->_32;
	rz = part1Pos.x * mWorld->_13 + part1Pos.y * mWorld->_23 + part1Pos.z * mWorld->_33;
	...
	rx = part2Pos.x * mWorld->_11 + part2Pos.y * mWorld->_21 + part2Pos.z * mWorld->_31;
	ry = part2Pos.x * mWorld->_12 + part2Pos.y * mWorld->_22 + part2Pos.z * mWorld->_32;
	rz = part2Pos.x * mWorld->_13 + part2Pos.y * mWorld->_23 + part2Pos.z * mWorld->_33;
	...
	if (EndFlag == FALSE)
	{
		...
		if ((!MyCharacterFlag && pChar->bActive == FALSE) || (flag == 1 && pChar->HvLeftHand.ColorEffect == 0) || (flag == 2 && pChar->HvRightHand.ColorEffect == 0))
		{
			Time = Max_Time;
			StopParticle();
			StopParticleID();
...
void AssaNatureSnow::Main()
{
	...
	auto sPosition = GetMoveLocation(0, 0, 40 * fONE, 0, pChar->Angle.y, 0);

	D3DVECTOR charPos;
	charPos.x = (float)pChar->pX + sPosition.iX;
	charPos.y = (float)pChar->pY + 30000;
	charPos.z = (float)pChar->pZ + sPosition.iZ;
	g_NewParticleMgr.SetPos(ParticleID, charPos);
...
{
	auto sPosition = GetMoveLocation(0, 0, 5 * fONE + 100, ShootingAngle.x, ShootingAngle.y, ShootingAngle.z);
	Posi.x += sPosition.iX;
	Posi.y += sPosition.iY;
	Posi.z += sPosition.iZ;
...
void AssaRunicGuardianBossHit::Start(POINT3D *pCur, Unit *destChar, D3DVECTOR *velocity, int delay, Unit **charList, int charCount)
{
	Pos.x = (float)pCur->x;
	Pos.y = (float)pCur->y;
	Pos.z = (float)pCur->z;
...
void AssaRunicGuardianBossSkill::Start(Unit* character, int liveCount)
{
	pChar = character;
	if (pChar == NULL)
		return;
...
void AssaSkyParticle::Main()
{
	if (ParticleID != -1)
	{
		...
		if (mapY1 > (int)Pos.y)
		{
			...
			AssaParticle_TerrainFireHit(&hitPos, CODE);
			D3DVECTOR vector;
...
void AssaSkyParticle::Start(POINT3D *pCur, POINT3D *pDest, int delay, int SkillCode)
{
	Pos.x = (float)pCur->x;
	Pos.y = (float)pCur->y;
	Pos.z = (float)pCur->z;
...
void AssaRunicGuardianLight::Main()
{
	...
	if (pCurChar && pDestChar)
	{
		...
		{
			smMATRIX *mWorld;
			AnimObjectTree(ObjBip1, pCurChar->iFrame, 0, (-pCurChar->Angle.y + ANGLE_180)&ANGCLIP, 0);
			mWorld = &ObjBip1->mWorld;
...
void AssaRunicGuardianLight::Start(Unit *curChar, Unit *desChar, bool ObjectFlag, bool firstPartFlag, bool secondPartFlag)
{
...
bool AssaRunicGuardianLight::AddFace2DLine(POINT3D *curPos, POINT3D *desPos, int width, int matNum)
{
	...
	auto sc = Point3D(curPos->x, curPos->y, curPos->z);
	auto sd = Point3D(desPos->x, desPos->y, desPos->z);
...
void AssaShelltomUpWeapon::Start(Unit *pCharacter, int timeCount)
{
	pChar = pCharacter;
	if (pChar == NULL)
		return;
...
void AssaShelltomUpWeapon::Main()
{
	...
	if (pChar->HvRightHand.dwItemCode != 0)
	{
		...
		else if ((pChar->HvRightHand.dwItemCode & sinITEM_MASK2) == sinWS1)
		{
			...
			{
				{
					auto sPosition = GetMoveLocation(weaponPos1.x, weaponPos1.y, weaponPos1.z, 0, ANGLE_45&ANGCLIP, 0);
					weaponPos1.x = sPosition.iX;
					weaponPos1.y = sPosition.iY;
					weaponPos1.z = sPosition.iZ;
				}
				{
					auto sPosition = GetMoveLocation(weaponPos2.x, weaponPos2.y, weaponPos2.z, 0, ANGLE_45&ANGCLIP, 0);
					weaponPos2.x = sPosition.iX;
					weaponPos2.y = sPosition.iY;
					weaponPos2.z = sPosition.iZ;
				}
			}
		}
		else
	...
	else if (pChar->HvLeftHand.dwItemCode != 0)
	{
		...
		{
			{
				auto sPosition = GetMoveLocation(weaponPos1.x, weaponPos1.y, weaponPos1.z, 0, ANGLE_90&ANGCLIP, 0);
				weaponPos1.x = sPosition.iX;
				weaponPos1.y = sPosition.iY;
				weaponPos1.z = sPosition.iZ;
			}
			{
				auto sPosition = GetMoveLocation(weaponPos2.x, weaponPos2.y, weaponPos2.z, 0, ANGLE_90&ANGCLIP, 0);
				weaponPos2.x = sPosition.iX;
				weaponPos2.y = sPosition.iY;
				weaponPos2.z = sPosition.iZ;
			}
...
BOOL AssaShelltomUpWeapon::AddFace2DLine(POINT3D *curPos, POINT3D *desPos, int width, int matNum)
{
	...
	float dx = float(sd.iX - sc.iX); //대상 까지의 x축의 기울기를 구한다.
	float dy = float(sd.iY - sc.iZ); //대상 까지의 y축의 기울기를 구한다.
	float length = (float)sqrt(dx*dx + dy * dy);        //대상 까지의 거리를 구한다.
...
void AssaShelltomUpWeapon1::Start(Unit *pCharacter, int timeCount)
{
	pChar = pCharacter;
	if (pChar == NULL)
		return;
...
void AssaShelltomUpWeapon2::Start(Unit *pCharacter, int timeCount, int shellTomCode)
{
	pChar = pCharacter;
	if (pChar == NULL)
		return;
...
void AssaShelltomUpWeapon2::Main()
{
	...
	if (mWorld2 != NULL)
	{
		for (int index = 0; index < MAX_WEAPON_POS; index++)
		{
			rx = WeaponPos2[index].x * mWorld2->_11 + WeaponPos2[index].y * mWorld2->_21 + WeaponPos2[index].z * mWorld2->_31;
			ry = WeaponPos2[index].x * mWorld2->_12 + WeaponPos2[index].y * mWorld2->_22 + WeaponPos2[index].z * mWorld2->_32;
			rz = WeaponPos2[index].x * mWorld2->_13 + WeaponPos2[index].y * mWorld2->_23 + WeaponPos2[index].z * mWorld2->_33;

			WeaponPos2[index].x = (pChar->pX + (rx >> FLOATNS) + mWorld2->_41);
			WeaponPos2[index].z = (pChar->pZ + (ry >> FLOATNS) + mWorld2->_42);
			WeaponPos2[index].y = (pChar->pY + (rz >> FLOATNS) + mWorld2->_43);
		}
	}
...
void AssaShelltomUpWeapon2::Draw(POINT3D *pPosi, POINT3D *pAngle)
{
	...
	for (int index = 0; index < WeaponPosCount2; index++)
	{
		smRender.Init();
		smRender.SetMaterialGroup(smMaterialGroup);

		Face.x = WeaponPos2[index].x;
		Face.y = WeaponPos2[index].y;
		Face.z = WeaponPos2[index].z;
		Face.width = Width + ScaleWidth - index * 5;
		Face.height = Width + ScaleWidth - index * 5;
		if (!AssaAddFace2D(&Face))
			continue;
	...
	if (ParticleSpawnFlag == false)
	{
		...
		if (Delay <= 0)
		{
			...
			case 1:
				...
				break;
			case 2:
				ParticleID = g_NewParticleMgr.Start("IceWorm1", partPos);
				break;
			case 3:
				ParticleID = g_NewParticleMgr.Start("IceWorm2", partPos);
				break;
			case 4:
				ParticleID = g_NewParticleMgr.Start("Skill5_Priestess_Meteo2", partPos);
				break;
			}
...
AssaSkillNormal::AssaSkillNormal()
{
	pChar = NULL;
	ParticleID = -1;
	MyCharacterFlag = FALSE;
...
void AssaSkillNormal::Start(char *pFileName, Unit *character, int Time, int SkillCode)
{
	pChar = character;
	if (pChar == NULL)
		return;
	...
	ParticleID = g_NewParticleMgr.Start(pFileName, charPos);

	Max_Time = (Time + 2) * 70;
	CODE = SkillCode;
	EndFlag = FALSE;
}
void AssaSkillNormal::Main()
{
	if (Time != Max_Time)
	{
		if (!g_NewParticleMgr.GetRunning(ParticleID) && EndFlag)
...
void AssaDarkSkill::Start(POINT3D *pCur, Unit *destChar, D3DVECTOR *velocity, int delay, char *pFileName1, char *pFileName2, int SkillCode)
{
	Pos.x = (float)pCur->x;
	Pos.y = (float)pCur->y;
	Pos.z = (float)pCur->z;
	...
	pos.z = (int)Pos.z;
	if (pFileName1)
		ParticleID = g_NewParticleMgr.Start(pFileName1, pos);

	if (pFileName2)
		ParticleIDExt1 = g_NewParticleMgr.Start(pFileName2, pos);

	Max_Time = 2000;
	Time = -delay;
	CODE = SkillCode;
}
void AssaDarkSkill::Main()
{
	if (Time < 0)
		return;
	if (ParticleID == -1)
	...
	if (length < 15 || Time > 100)
	{
		...
		if (CODE == SKILL_DARK_BOLT)
		{
			SkillPlaySound(SKILL_SOUND_SKILL_DARK_BOLT_HIT, DestChar->pX, DestChar->pY, DestChar->pZ);
			AssaParticle_Effect("DarkBoltHit", DestChar, 0);
		}
		else if (CODE == SKILL_DARK_WAVE)
		{
			SkillPlaySound(SKILL_SOUND_SKILL_DARK_WAVE_HIT, DestChar->pX, DestChar->pY, DestChar->pZ);
			AssaParticle_Effect("DarkWaveHit", DestChar, 0);
		}
	}
	D3DVECTOR velocity;
	velocity.x = term.x / length;
...
AssaSkill5Normal::AssaSkill5Normal()
{
	pChar = NULL;
	MyCharacterFlag = FALSE;
	ParticleID = -1;
...
void AssaSkill5Normal::Start(Unit* character, int liveCount, char *ParicleName1, char *ParicleName2, int iCode)
{
	pChar = character;
	if (pChar == NULL)
		return;
	...
	charPos.y = pChar->pY + 10000;
	charPos.z = pChar->pZ;
	ParticleID = g_NewParticleMgr.Start(ParicleName1, charPos);
	ParticleIDEx = g_NewParticleMgr.Start(ParicleName2, charPos);
	Max_Time = liveCount * 70 + 2;
	EndFlag = 0;
	CODE = iCode;
	if (pChar->AnimPattern)
		ObjBip = pChar->AnimPattern->GetObjectFromName("Bip weapon01");
}
void AssaSkill5Normal::Main()
{
	if (ParticleID == -1)
		return;
	if (!g_NewParticleMgr.GetRunning(ParticleID) && EndFlag)
	...
	if (ObjBip)
	{
		smMATRIX *mWorld;
		D3DVECTOR charPos;
		if (ParticleID != -1 && ParticleIDEx != -1)
		{
			AnimObjectTree(ObjBip, pChar->iFrame, 0, (-pChar->Angle.y + ANGLE_180)&ANGCLIP, 0);
			mWorld = &ObjBip->mWorld;
			charPos.x = (float)(pChar->pX + mWorld->_41);
			...
			g_NewParticleMgr.SetPos(ParticleIDEx, charPos);
			g_NewParticleMgr.SetRendering(ParticleID, pChar->FlagShow);
		}
	}
	if (EndFlag == FALSE)
	{
			...
			(pChar->iLoadedMapIndex >= 0 && STAGE_MAP[pChar->iLoadedMapIndex]->iMapType == MAPTYPE_Town))
		{
			...
			if (ParticleIDEx != -1)
			{
				g_NewParticleMgr.SetFastStop(ParticleIDEx);
				g_NewParticleMgr.SetStop(ParticleIDEx);
			}
...
// 우醵댔샌
AssaSkillHyperSonic::AssaSkillHyperSonic()
{
	pChar = NULL;
	MyCharacterFlag = FALSE;
	EndFlag = FALSE;
...
void AssaSkillHyperSonic::Start(Unit *character, int liveCount)
{
	pChar = character;
...
void AssaSkillHyperSonic::Main()
{
	...
	smMATRIX *mWorld;
	POINT3D weaponPos;
	ZeroMemory(&weaponPos, sizeof(weaponPos));
	if (pChar->HvRightHand.dwItemCode != 0)
	{
		mWorld = &pChar->HvRightHand_Matrix;
		if ((pChar->HvRightHand.dwItemCode & sinITEM_MASK2) == sinWT1)
...
void AssaSkillHyperSonic::StopParticle()
{
	for (int index = 0; index < 4; index++)
	{
		if (ParticleIDGroup[index] != -1)
...
void AssaSkillHyperSonic::StopParticleID()
{
	for (int index = 0; index < 4; index++)
	{
		if (ParticleIDGroup[index] != -1)
...
AssaSkillCircleTrap::AssaSkillCircleTrap()
{
	ParticleID = -1;
	ZeroMemory(&ShootingAngle, sizeof(ShootingAngle));
}
AssaSkillCircleTrap::~AssaSkillCircleTrap()
{

}
void AssaSkillCircleTrap::Start(POINT3D *curPos, POINT3D *desPos, int level)
{
	Posi.x = curPos->x;
	Posi.y = curPos->y;
	Posi.z = curPos->z;
...
AssaSkillKrishna::AssaSkillKrishna()
{
	ParticleID = -1;
	MyCharacterFlag = FALSE;
	EndFlag = FALSE;
...
void AssaSkillKrishna::Start(Unit *character, int LiveCount)
{
	pChar = character;
	...
	CODE = SKILL_B_KRISHNA;
}
void AssaSkillKrishna::Main()
{
	if (Time != Max_Time)
	{
		if (!g_NewParticleMgr.GetRunning(ParticleID) && EndFlag)
...
AssaSkillBenedic::AssaSkillBenedic()
{
	ParticleID = -1;
	MyCharacterFlag = FALSE;
}
...
void AssaSkillBenedic::Start(Unit *character)
{
	POINT3D pos;
	...
	ParticleID = g_NewParticleMgr.Start("Skill5_Knight_HolyBenedic2", Posi);

	Max_Time = 5000;
}
void AssaSkillBenedic::Main()
{
	if (ParticleID != -1)
	{
		if (!g_NewParticleMgr.GetRunning(ParticleID))
...
AssaSkillBenedicEffect::AssaSkillBenedicEffect()
{
	ParticleID = -1;
	MyCharacterFlag = FALSE;
	EndFlag = FALSE;
...
void AssaSkillBenedicEffect::Start(Unit *character, int LiveCount)
{
	pChar = character;
...
void AssaSkillBenedicEffect::Main()
{
	if (Time != Max_Time)
	{
		if (!g_NewParticleMgr.GetRunning(ParticleID) && EndFlag)
...
AssaSkillCoupEffect::AssaSkillCoupEffect()
{
	ZeroMemory(&ShootingAngle, sizeof(ShootingAngle));
	PatMesh = NULL;
	Delay = 0;
...
void AssaSkillCoupEffect::Start(POINT3D *pCur, POINT3D *pDest, int delay)
{
	int *matNum = (int *)AssaSearchRes("shield1.bmp", SMMAT_BLEND_LAMP);
	if (matNum == NULL)
		return;
	...
	sPosition = GetMoveLocation(0, 0, fONE, ShootingAngle.x, ShootingAngle.y, ShootingAngle.z);
...
void AssaSkillCoupEffect::Draw(POINT3D* pPosi, POINT3D *pAngle)
{
	if (PatMesh != NULL)
	{
		smRender.Init();
...
AssaSkillTalaria::AssaSkillTalaria()
{
	ParticleID = -1;
	MyCharacterFlag = FALSE;
	EndFlag = FALSE;
...
void AssaSkillTalaria::Start(Unit *character, int LiveCount)
{
	pChar = character;
...
void AssaLevelEffect::Start(Unit *character)
{
	pChar = character;
	...
	ParticleID = g_NewParticleMgr.Start("HighLevelUserMerit_Star", charPos);
...
void AssaLevelEffect::Main()
{
	if (Time != Max_Time)
	{
		...
		if (EndFlag == FALSE)
		{
			if ((!MyCharacterFlag && pChar->bActive == FALSE) || (!pChar->psModelAnimation || pChar->psModelAnimation->iType == ANIMATIONTYPE_Die))
			{
				Time = Max_Time - 140;
