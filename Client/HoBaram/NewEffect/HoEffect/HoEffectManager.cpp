...
void HoEffectType_Attach_Avater::Init(Unit *pChar, HoEffectGroup *effectGroup, float endTime, float height)
{
	m_pAttachChar  = pChar;
	m_pEffectGroup = effectGroup;
...
void HoEffectType_Attach_Avater::Main(float elapsedTime)
{
 	if(m_pAttachChar)
	{
		...
		if (m_pAttachChar->bActive == FALSE ||
			!m_pAttachChar->psModelAnimation ||
			m_fElapsedTime >= m_fEndTime ||
			(m_pEffectGroup && !m_pEffectGroup->IsLive()) ||
			(m_pAttachChar->psModelAnimation->iType == ANIMATIONTYPE_Die) ||
			(m_pAttachChar->iLoadedMapIndex>=0 && STAGE_MAP[ m_pAttachChar->iLoadedMapIndex ]->iMapType == MAPTYPE_Town))
		{
 			m_bLiveFlag = false;
			if(m_iParticleID != -1)
			{
...
void HoEffectType_Avater_Shot_Default::Init(const HoEffectType_Avater_Shot_Info &info)
{
	...
	auto sPosition = GetRadian3D(info.m_CurrentPos.x, info.m_CurrentPos.y, info.m_CurrentPos.z, info.m_AttackPos.x, info.m_AttackPos.y, info.m_AttackPos.z);
	point3 shotAngle;
	shotAngle.x = (float)sPosition.iX;
	shotAngle.y = (float)sPosition.iY; 
	shotAngle.z = (float)sPosition.iZ;
...
void HoEffectType_Attach_AvaterToAvater::Init(Unit *char1, Unit *char2, const std::string &textureName, int textureCount, float aniDelayTime)
{
...
void HoEffectType_Attach_AvaterToAvater::Init(Unit *char1, Unit *char2, int partID1, int partID2)
{
	m_pAttachChar1 = char1;
	m_pAttachChar2 = char2;
...
void HoEffectType_Avater_Blade_Blur::Init(Unit *pChar, const std::string &fileName, float endTime)
{
...
void HoEffectType_Avater_Blade_Blur::UpdateView(int x, int y, int z, int ax, int ay, int az)
{
	...
	{
		pframe = m_pAttachChar->iFrame -(cnt*mFrames);
		if ( pframe<m_pAttachChar->psModelAnimation->StartFrame ) 
			pframe = m_pAttachChar->psModelAnimation->StartFrame;
 		vcnt = cnt<<1;
...
void HoEffectType_Magnetic_Sphere::Init(Unit *pChar, float endTime, float attackTime)
{
...
void HoEffectType_Magnetic_Sphere::Main(float elapsedTime)
{
	...
	for(int index = 0; index < 3; index++)
	{
		...
		if(m_ParticleID[index] != -1)
		{
			...
			if(attackMonster)
			{
				if(attackMonster->bActive && attackMonster->DisplayFlag)
				{
					POINT3D curPos;
					POINT3D desPos;
	...
	if(m_fElapsedTime >= m_fEndTime ||
		m_pAttachChar->bActive == FALSE ||
		(!m_pAttachChar->psModelAnimation || m_pAttachChar->psModelAnimation->iType == ANIMATIONTYPE_Die) ||
		(m_pAttachChar->iLoadedMapIndex>=0 && STAGE_MAP[ m_pAttachChar->iLoadedMapIndex ]->iMapType == MAPTYPE_Town))
	{
		m_bLiveFlag = false;
...
void HoEffectType_MusPel::OnEvent(int curState, int curEvent)
{
  	...
  	if(curState == MUSPEL_IDLE)
	{
		...
		if(m_fAttackTimeCount >= m_fAttackTime)
		{
			...
			if(m_AttackMonster)
			{
				if(m_AttackMonster->bActive && m_AttackMonster->DisplayFlag)
				{
					if (m_CODE != SKILL_ADVENT_MIGAL && m_CODE != SKILL_ADVENT_MIDRANDA)
						SetState(MUSPEL_ATTACK);
				}
			}
		}
		
		if(m_pAttachChar && m_pAttachChar->psModelAnimation)
		{
			if (m_pAttachChar->psModelAnimation->iType == ANIMATIONTYPE_Walking || m_pAttachChar->psModelAnimation->iType == ANIMATIONTYPE_Running)
...
void HoEffectType_MusPel::Init(Unit *pChar, float endTime, float attackTime,int Kind)
{
	m_CODE = Kind;
	switch(m_CODE)
	{
...
void HoEffectType_RegenerationField::Init(Unit *pChar)
{
...
void HoEffectType_RegenerationField::Main(float elapsedTime)
{
	...
	if(m_pAttachChar)
	{
		if(m_pAttachChar->bActive == FALSE || m_pAttachChar->psModelAnimation == NULL)
		{
			m_bLiveFlag = false;
			if(m_iParticleID != -1)
			{
	 	...
	 	int compFrame = m_pAttachChar->iFrame - (m_pAttachChar->psModelAnimation->StartFrame*160);
		if( compFrame < (int)m_pAttachChar->psModelAnimation->EventFrame[0] || compFrame > (int)m_pAttachChar->psModelAnimation->EventFrame[1])
		{
			m_bLiveFlag = false;
			if(m_iParticleID != -1)
			{
...
void HoEffectType_PhoneixShot::Init(point3 &curPos, point3 &desPos)
{
	...
	auto sPosition = GetRadian3D(int(m_CurrentPos.x*256.f), 
	...
	m_ShotAngle.x = (float)(sPosition.iX & ANGCLIP);
	m_ShotAngle.y = (float)(sPosition.iY & ANGCLIP); 
	m_ShotAngle.z = (float)(sPosition.iZ & ANGCLIP);
...
BOOL HoEffectManager::GetSkillCode(Unit *pChar, int code)
{
	std::vector<HoEffectTypeInterface *>::iterator i;
	for (i = m_EffectTypeList.begin();i != m_EffectTypeList.end(); i++)
    {
...
bool HoEffectManager::SetMaintainEffect(int code , Unit *pAttachChar, float time)
{
...
bool HoEffectManager::StopMaintainEffect(int code, Unit *pAttachChar,int iHead)
{
	int maintainEffectCount = m_MaintainEffectTypeList.size();
...
{
	DELET(m_pEffectGroup);
}
void HoEffectWing::Init(Unit *pChar,HoEffectGroup *effectGroup,int iHead)
{
	m_pAttachChar	= pChar;
	m_pEffectGroup	= effectGroup;
	m_bLiveFlag		= true;
...
void HoEffectWing::Main(float elapsedTime)
{
	...
	AnimObjectTree(m_pObj,m_pAttachChar->iFrame,0,(-m_pAttachChar->Angle.y+ANGLE_180)&ANGCLIP,0);
	...
	switch(m_pAttachChar->sCharacterData.iClass)
	{
	case CHARACTERCLASS_Fighter:
	case CHARACTERCLASS_Mechanician:
	case CHARACTERCLASS_Knight:
		x = -3;y = -3;z = 5;
		if (m_pAttachChar->iLoadedMapIndex>=0 && STAGE_MAP[m_pAttachChar->iLoadedMapIndex]->iMapType != MAPTYPE_Town)
		{
			if (m_pAttachChar->psModelAnimation->iType == ANIMATIONTYPE_Idle)
			{
				x = -3;
				y = -1;
				z = 5;
	...
	if (m_pAttachChar->bActive == FALSE || !m_pAttachChar->psModelAnimation || (m_pEffectGroup && !m_pEffectGroup->IsLive()) || m_pAttachChar->dwWingEffectCode == 0)
		m_bLiveFlag = false;
