...
bool HoPCBangPet::Draw(int x, int y, int z, int angleX, int angleY, int angleZ)
{
	if (m_eCurrentFsmState != PCBang_FsmState::None)
	{
		...
		if (m_bPetAttacking)
		{
			auto sPosition = GetRadian3D(x, y, z, m_AttackPos.x, m_AttackPos.y, m_AttackPos.z);
			petAngle.x = sPosition.iX;
			petAngle.y = (ANGLE_180 - sPosition.iY)&ANGCLIP;
			petAngle.z = sPosition.iZ;
		}
		else
...
void HoPCBangPet::OnEvent(PCBang_FsmStateEvent eEvent, PCBang_FsmState eFsmState)
{
	...
	else if (eFsmState == PCBang_FsmState::Idle)
	{
		...
		case PCBang_FsmStateEvent::EVENT_UPDATE:
		{
			...
			if (!m_bPetAttacking)
			{
				if (m_TargetChar->chrAttackTarget &&
					m_TargetChar->chrAttackTarget->sCharacterData.iMonsterType != MONSTERTYPE_Friendly &&
					m_TargetChar->chrAttackTarget->sCharacterData.sHP.sMin > 0 &&
					m_TargetChar->chrAttackTarget->sCharacterData.iType == CHARACTERTYPE_Monster &&
					(m_TargetChar->psModelAnimation->iType == ANIMATIONTYPE_Attack ||
						m_TargetChar->psModelAnimation->iType == ANIMATIONTYPE_Skill ||
						m_TargetChar->ShootingFlag) && m_bAttackTiming == false &&
					m_bMyCharacterFlag)
				{
			...
			if (m_bPetAttacking)
			{
				if (m_AttackCharTarget && m_AttackCharTarget->sCharacterData.sHP.sMin > 0 && m_AttackCharTarget->sCharacterData.iType == CHARACTERTYPE_Monster && PetNearMonster(m_AttackCharTarget))
				{
			...
			if (m_bMyCharacterFlag)
			{
				if (m_TargetChar->psModelAnimation->iType != ANIMATIONTYPE_Attack && m_TargetChar->psModelAnimation->iType != ANIMATIONTYPE_Skill && m_TargetChar->ShootingFlag == FALSE)
					m_bAttackTiming = false;
			}
			break;
		}
			...
			if (m_bSkillFlag == false)
			{
				if (m_Type == Type::BurstP)
				{
					if (m_iCurrentFrame >= (180 * 160))
					{
						...
						if (m_bMyCharacterFlag && m_AttackChar && m_AttackChar->sCharacterData.sHP.sMin > 0 && m_AttackChar->sCharacterData.iType == CHARACTERTYPE_Monster)
						{
							g_HoPCBangPetAttackSerial = m_AttackChar->iID;
				...
				else if (m_Type == Type::BurstD)
				{
					...
					{
						m_bSkillFlag = true;
						sinSetLife(sinGetLife() + 10 + short(m_TargetChar->sCharacterData.iLevel / 2));
						if (m_TargetChar)
							g_HoPCBangPetAttackSerial = m_TargetChar->iID;
					}
				}
			}
			break;
...
BOOL HoPCBangPet::PetNearMonster(Unit *lpNearMonster)
{
	int dist, x, y, z;
	int	NearDist;
	...
	if (lpCurPlayer->psModelAnimation->iType != ANIMATIONTYPE_Attack && lpCurPlayer->psModelAnimation->iType != ANIMATIONTYPE_Skill && !lpCurPlayer->ShootingFlag)
		return FALSE;
...
void HoPCBangPet::SetFsmAttackState(Unit *pAttackChar)
{
	SetFsmState(HoPCBangPet::PCBang_FsmState::SkillPre);
...
void HoPCBangPet::SetFsmHealState(Unit *pHealChar)
{
	SetFsmState(HoPCBangPet::PCBang_FsmState::SkillPre);
...
void HoPCBangPet::SetSpawn(Unit *pAttachChar, HoPCBangPet::Type type, smPAT3D *meshRes, smPAT3D *boneRes)
{
...
void HoPCBangPet::SetIdle(Unit *pAttachChar, HoPCBangPet::Type type, smPAT3D *meshRes, smPAT3D *boneRes, Unit *pAttackChar)
{
	m_TargetChar = pAttachChar;
	m_Type = type;
	m_PetMeshRes = meshRes;
...
Unit *HoPCBangPet::GetAttachChar()
{
	return m_TargetChar;
}
...
bool HoPCBangPetManager::Create()
{
	...
	m_BoneBurstS = smASE_ReadBone("Effect\\PcBangPet\\Pet_Phoenimin\\Phoenix1.ASE");
	smASE_SetPhysique(m_BoneBurstS);
	m_BoneBurstL = smASE_ReadBone("Effect\\PcBangPet\\Pet_Phoenimin\\Phoenix2.ASE");
	smASE_SetPhysique(m_BoneBurstL);
	m_BoneBurstD = smASE_ReadBone("Effect\\PcBangPet\\Pet_Phoenimin\\Phoenix3.ASE");
	smASE_SetPhysique(m_BoneBurstD);


	m_MeshBurstP = smASE_Read("Effect\\PcBangPet\\Pet_Phoenimin\\Phoenix.ASE", 0);
	m_MeshBurstS = smASE_Read("Effect\\PcBangPet\\Pet_Phoenimin\\Phoenix1.ASE", 0);
	m_MeshBurstL = smASE_Read("Effect\\PcBangPet\\Pet_Phoenimin\\Phoenix2.ASE", 0);
	m_MeshBurstD = smASE_Read("Effect\\PcBangPet\\Pet_Phoenimin\\Phoenix3.ASE", 0);
...
bool HoPCBangPetManager::AddPet(Unit *pAttachChar, HoPCBangPet::Type petType)
{
...
bool HoPCBangPetManager::AddIdlePet(Unit *pAttachChar, int petType, Unit *pAttackChar)
{
...
void HoPCBangPetManager::DeletePet(Unit *pAttachChar)
{
...
HoPCBangPet *HoPCBangPetManager::GetEmptyPet(Unit *pAttachChar)
{
	for (int i = 0; i < MAX_LOWLEVEL_PET; i++)
	{
		if (PcBangPetList[i].GetFsmState() != HoPCBangPet::PCBang_FsmState::None)
...
HoPCBangPet *HoPCBangPetManager::GetEmptyIdlePet(Unit *pAttachChar, int petType, Unit *pAttackChar)
{
	for (int i = 0; i < MAX_LOWLEVEL_PET; i++)
	{
		if (PcBangPetList[i].GetFsmState() != HoPCBangPet::PCBang_FsmState::None)
...
bool g_HoPCBangAddPet(Unit *pAttachChar, HoPCBangPet::Type petType)
{
	s_PcBangPetManager.AddPet(pAttachChar, petType);
	return true;
}
bool g_HoPCBangAddIdlePet(Unit *pAttachChar, int petType, Unit *pAttackChar)
{
	s_PcBangPetManager.AddIdlePet(pAttachChar, petType, pAttackChar);

	return true;
}
void g_HoPCBangDeletePet(Unit *pAttachChar)
{
