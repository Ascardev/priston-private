...
int HoEffectPatDest::Main()
{
	...
	if( CurrentFrame < (AnimationEnd*160-1)-5*FrameStep)
	{
		auto sPosition = GetMoveLocation(0, 0, Step, DestAngleX, DestAngleY, 0);

		if(SkillType == SKILL_PERFECT_AIM || SkillType == SKILL_WIND_ARROW || SkillType == MONSTER_WEB_SHOT1 || SkillType == SKILL_ARROW_OF_RAGE || SkillType == SKILL_AVALANCHE || SkillType == SKILL_GROUND_PIKE || SkillType == SKILL_C_TRAP)
		{
			LocalX += sPosition.iX;
			LocalY += sPosition.iY;
			LocalZ += sPosition.iZ;
			...
			if(SkillType == SKILL_WIND_ARROW)
			{
				...
				StartEffect(int(WorldX + LocalX + sPosition.iX), int(WorldY + LocalY + sPosition.iY), int(WorldZ + LocalZ + sPosition.iZ), 100, 100, 100, 150, EFFECT_PARTICLE_BOW2);
			}
			...
			else if(SkillType == SKILL_ARROW_OF_RAGE)
			{
				...
				{
					sPosition = GetRadian3D(int(WorldX + LocalX), int(WorldY + LocalY), int(WorldZ + LocalZ), DestX, DestY - 40000, DestZ);
				...
				if(length < 30000 && WorkState == PAT_PROCESS)
				{
					...
					sPosition = GetMoveLocation(0, 0, z*fONE, 0, y, 0);

					x = int(WorldX + LocalX) + sPosition.iX;
					z = int(WorldZ + LocalZ) + sPosition.iZ;
			...
			else if(SkillType == SKILL_C_TRAP)
			{
				...
				if(length < 30000 && WorkState == PAT_PROCESS)
				{
					...
					x = int(WorldX + LocalX) + sPosition.iX;
					z = int(WorldZ + LocalZ) + sPosition.iZ;
					StartSkillDest(int(WorldX + LocalX), int(WorldY + LocalY), int(WorldZ + LocalZ), x, DestY - 40000, z, SKILL_ARROW_OF_TRAP_END, Level);
					return TRUE;
				}
			}
		}
		else if(SkillType == SKILL_MECHANIC_BOMB)
		{
			...
			StartEffect(int(WorldX + LocalX) + rand() % 2000 - 1000 + sPosition.iX, int(WorldY + LocalY) + rand() % 1000 - 1000, int(WorldZ + LocalZ) + rand() % 2000 - 1000 + sPosition.iZ, EFFECT_MECHANICBOMB_DUST1);
		...
		LocalZ += sPosition.iZ;
			...
			if(WorkState == PAT_PROCESS && mapY1 > (WorldY+LocalY-4000))
			{
				...
				else if (SkillType == SKILL_ARROW_OF_TRAP_END)
				{
					BlurBlendValue = 0;
					StartEffect(int(WorldX+LocalX), int(WorldY+LocalY), int(WorldZ+LocalZ),EFFECT_ARROW_OF_TRAP_HIT);
				}
			}
		}
		
		POINT3D velocity;
		velocity.x = sPosition.iX;
		velocity.y = sPosition.iY;
		velocity.z = sPosition.iZ;
...
int HoEffectPatDest::StartDest(POINT3D currentPos, POINT3D destPos, smPAT3D *pat, int skillType, int level)
{
	...
	if(SkillType == SKILL_ARROW_OF_RAGE || SkillType == SKILL_C_TRAP)
	{
		Step	= fONE * 8;
		DestY	+= 60000;
	}
	else if(SkillType == SKILL_ARROW_OF_RAGE_END || SkillType == SKILL_ARROW_OF_TRAP_END)
		Step = fONE * 5;
	
	auto sPosition = GetRadian3D(int(WorldX + LocalX), int(WorldY + LocalY), int(WorldZ + LocalZ), DestX, DestY, DestZ);
	
	DestAngleX = sPosition.iX;
	DestAngleY = sPosition.iY;
	DestAngleZ = sPosition.iZ;
	...
	else if (SkillType == SKILL_PERFECT_AIM)
	{
		...
		auto sPosition = GetMoveLocation(0, 0, Step, DestAngleX, DestAngleY, 0);
		PrimitiveBillBoard.StartBillRect(int(WorldX + LocalX) + sPosition.iX,
			int(WorldY + LocalY) + sPosition.iY + 200, int(WorldZ + LocalZ) + sPosition.iZ, "SkillPerfectAim1.ini", ANI_LOOP, PRIMITIVE_PATH_RECT_LINE);
	...
	{
		Step = 4*fONE;
		auto sPosition = GetMoveLocation(0, 0, Step * 7, DestAngleX, DestAngleY, 0);
		DestX += sPosition.iX;
		DestZ += sPosition.iZ;
		FrameStep = 20;
	}
...
int HoEffectPatDest::MainBlur()
{
	...
	if(PosListCount < MAX_BLUR_LIST)
	{
		...
		PosList[0].x = int(WorldX + LocalX + sPosition.iX);
		PosList[0].y = int(WorldY + LocalY + sPosition.iY);
		PosList[0].z = int(WorldZ + LocalZ + sPosition.iZ);
...
int HoEffectShield::Main()
{
	...
	if (ShieldType == SKILL_SHIELD1)
	{
		...
		auto sPosition = GetMoveLocation((int)LocalX, (int)(LocalY), (int)LocalZ, 0, DirectionAngle.y, 0);
		POINT3D tempGeoResult;
		tempGeoResult.x = sPosition.iX;
		tempGeoResult.y = sPosition.iY;
		tempGeoResult.z = sPosition.iZ;

		sPosition = GetMoveLocation(tempGeoResult.x, tempGeoResult.y, tempGeoResult.z, DirectionAngle.x, 0, 0);
	
		sPosition.iY += 4500;
		BackBillBoard.TranslateWorld(int(sPosition.iX + WorldX), int(sPosition.iY + WorldY), int(sPosition.iZ + WorldZ));
		BackBillBoard.Main();
		...
		if (PosListCount < POS_LIST_MAX)
		{
			...
			memcpy(tempPosList, &PosList[0], sizeof(tempPosList));
			memcpy(&PosList[1], tempPosList, sizeof(tempPosList));
			PosList[0].x = sPosition.iX;
			PosList[0].y = sPosition.iY;
			PosList[0].z = sPosition.iZ;
			PosListCount++;
		}
		else
		{
		...
		if (particleCount >= 25)
		{
			StartEffect(int(sPosition.iX + WorldX), int(sPosition.iY + WorldY), int(sPosition.iZ + WorldZ), EFFECT_SHIELD1_PARTICLE);
			particleCount = 0;
		}
		particleCount++;
		...
		if (EndCount >= 300 * 70 || ((Character != NULL) && !(Character->dwDispAppSkill & SKILL_APPMASK_PHYSICAL_ABSORB)))
		{
			if (Character != NULL)
	...
	if (ShieldType == SKILL_SHIELD2)
	{
		...
		EndCount++;
		if(EndCount >= 300 * 70 || ((Character != NULL) && !(Character->dwDispAppSkill & SKILL_APPMASK_AUTOMATION)))
		{
			if(Character != NULL)
				Character->HoSkillCode = 0;
			Init();
...
int HoEffectShield::Start(int x, int y, int z, int shieldType, int angle, int circleRadius, Unit *character)
{
	...
	ZeroMemory(PosList,sizeof(PosList));
	PosListCount		= 0;
...
int HoEffectPower::Start(int x, int y, int z,  int sw,  int aniType, Unit *character)
{
	char iniName[32] = {0,};
	if(sw==1)
		lstrcpy(iniName,"Power1.ini");
	else 
		lstrcpy(iniName,"Power2.ini");
	Skill_n = sw;
...
int HoEffectHitLine::CreatePathFace()
{
	...
	ZeroMemory(TexLink,sizeof(TexLink));
	VertexCount = 0;
	FaceCount = 0;
	TexLinkCount = 0;
...
int HoEffectHitLine::Start(int x, int y, int z, int destX, int destY, int destZ, int sw)
{
	...
	char iniName[32]={0,};
	if(sw==0)
		lstrcpy(iniName,"SkillTornado.ini");
	if(sw==1) 
		lstrcpy(iniName,"SkillTornado2.ini");
	if(sw==2) 
		lstrcpy(iniName,"SkillTornado3.ini");
...
int HoEffectStun::Start(int worldX, int worldY, int worldZ, Unit *character)
{
