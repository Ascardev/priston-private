...
int HoEtc2dPrimitive::Draw()
{
	if (StartFlag)
	{
		...
		if (FAILED(GRAPHICENGINE->GetDevice()->SetTexture(0, smMaterial[mat].smAnimTexture[AnimationData.SequenceDataBuffer[AniDataIndex].AnimFrame[CurrentFrame].ImageNum]->m_pTexture)))
		{
		...
		memcpy(tlVertex, TLVertex, sizeof(D3DTLVERTEX9)*VertexCount);
		...
		GRAPHICENGINE->GetDevice()->DrawPrimitiveUP(D3DPT_TRIANGLESTRIP, VertexCount - 2, tlVertex, sizeof(D3DTLVERTEX9));
...
int HoPrimitiveBillboard::Main()
{
	...
	if (CurrentFrame < AnimDataMgr.SequenceDataBuffer[AniDataIndex].AnimFrameCount)
	{
		if (TimeCount >= AnimDataMgr.SequenceDataBuffer[AniDataIndex].AnimFrame[CurrentFrame].Delay)
		{
			...
			BlendStep = (AnimDataMgr.SequenceDataBuffer[AniDataIndex].AnimFrame[CurrentFrame].AlphaValue - CurrentBlendValue) / AnimDataMgr.SequenceDataBuffer[AniDataIndex].AnimFrame[CurrentFrame].Delay;
...
BOOL HoPrimitiveBillboard::AddFace2D(smFACE2D *face)
{
	...
	{
		auto sd = Point3D(int(WorldX + LocalX), int(WorldY + LocalY), int(WorldZ + LocalZ));

		auto sc = Point3D();

		if (WorkState & PRIMITIVE_PATH_RECT)
		{
			sc = Point3D(int(sd.iX - DirectionVelocity.x*SizeWidth), int(sd.iY - DirectionVelocity.y*SizeWidth), int(sd.iZ - DirectionVelocity.z*SizeWidth));
		}

		if (WorkState & PRIMITIVE_PATH_RECT_STRETCH)
		{
			sc = Point3D(int(WorldX + LocalStartPos.x), int(WorldY + LocalStartPos.y), int(WorldZ + LocalStartPos.z));
		}
	...
	if (WorkState & PRIMITIVE_DEFAULT_RECT)
	{
		...
		Point3D sCamera = (WorkState & MOVE_ANGLE) ?
			Point3D(int(WorldX + localAngleX), int(WorldY + LocalY), int(WorldZ + localAngleZ)) :
			Point3D(face->x, face->y, face->z);

		if (smRender.GetCameraCoord(sCamera) == FALSE)
			return FALSE;
		...
		for (cnt = 0; cnt < 4; cnt++) {
			...
			fx[cnt] += (float)sCamera.iX;
			fy[cnt] += (float)sCamera.iY;
		}

		v[0] = smRender.AddRendVertex((int)fx[0], (int)fy[0], sCamera.iZ, sColor);
		v[1] = smRender.AddRendVertex((int)fx[1], (int)fy[1], sCamera.iZ, sColor);
		v[2] = smRender.AddRendVertex((int)fx[2], (int)fy[2], sCamera.iZ, sColor);
		v[3] = smRender.AddRendVertex((int)fx[3], (int)fy[3], sCamera.iZ, sColor);
	}
...
int HoPrimitiveBillboard::Draw(int x, int y, int z, int ax, int ay, int az)
{
	...
	smMaterialGroup->SetMaterialAnimFrame(&smMaterial[AnimDataMgr.ImageDataBuffer[AnimDataMgr.SequenceDataBuffer[AniDataIndex].DataFileIndex].GetMaterialNum()], AnimDataMgr.SequenceDataBuffer[AniDataIndex].AnimFrame[CurrentFrame].ImageNum);
...
int HoEffectPatHawk::Main()
{
	...
	if (Character)
	{
		if (Character->HoSkillCode != SkillCode ||
			(Character->psModelAnimation->iType == ANIMATIONTYPE_Die) ||
			(Character->iLoadedMapIndex >= 0 && STAGE_MAP[Character->iLoadedMapIndex]->iMapType == MAPTYPE_Town))
			Loop = 1;
	}
...
int HoEffectPatHawk::StartAni(int x, int y, int z, int angleX, int angleY, int angleZ, smPAT3D *patBone, smPAT3D *patAni, Unit *character, int loop)
{
	if (character != NULL)
		Character = character;
	else
...
int HoEffectPatFalcon::StartGoldenFalconAni(int x, int y, int z, int angleX, int angleY, int angleZ, smPAT3D *patBone, smPAT3D *patAni, Unit *character, int liveCount)
{
	if (character != NULL)
		Character = character;
	else
...
int HoEffectPatFalcon::StartAni(int x, int y, int z, int angleX, int angleY, int angleZ, smPAT3D *patBone, smPAT3D *patAni, Unit *character, int liveCount)
{
	if (character != NULL)
		Character = character;
	else
...
int HoEffectPatFalcon::Main()
{
	if (StartFlag)
	{
		if (Character != NULL)
		{
			...
			if (WorkState == FALCON_ATTACK)
			{
				...
				{
					auto sPosition = GetRadian3D(int(WorldX + LocalX), int(WorldY + LocalY), int(WorldZ + LocalZ), AttackDest.x, AttackDest.y, AttackDest.z);
					AttackAngle.x = sPosition.iX;
					AttackAngle.y = sPosition.iY;
					AttackAngle.z = sPosition.iZ;

					Angle.x = sPosition.iX;
					Angle.y = (ANGLE_180 - sPosition.iY)&ANGCLIP;
					Angle.z = sPosition.iZ;
				}

				if (CurrentFrame >= 160 * 130 - 1)
				{
					auto sPosition = GetMoveLocation(0, 0, fONE * 8, AttackAngle.x, AttackAngle.y, 0);
			...
			if (WorkState == FALCON_TURN_START)
			{
				if (CurrentFrame >= 160 * 150 - 1)
				{
					...
					auto sPosition = GetRadian3D(int(WorldX + LocalX), int(WorldY + LocalY), int(WorldZ + LocalZ), Character->pX + mWorld->_41, Character->pY + 20000 + mWorld->_43, Character->pZ + mWorld->_42);
					AttackAngle.x = sPosition.iX;
					AttackAngle.y = sPosition.iY;
					AttackAngle.z = sPosition.iZ;
			...
			if (WorkState == FALCON_TURN_PROCESS)
			{
				...
				auto sPosition = GetMoveLocation(0, 0, fONE * 6, AttackAngle.x, AttackAngle.y, 0);
			...
			if (WorkState == FALCON_KEEP_ON)
			{
				if (Character->chrAttackTarget && Character->chrAttackTarget->bActive &&
					Character->chrAttackTarget->sCharacterData.sHP.sMin > 0 &&
					(Character->psModelAnimation->iType == ANIMATIONTYPE_Attack || Character->psModelAnimation->iType == ANIMATIONTYPE_Skill || Character->ShootingFlag))
				{
	...
	int EndCount = pEndCounter + 70 * 10;

	if (!Character || pCounter > EndCount)
		StartFlag = 0;
	...
	if (!StartFlag ||
		...
		Character->HoSkillCode != SkillCode ||
		(Character->iLoadedMapIndex >= 0 && STAGE_MAP[Character->iLoadedMapIndex]->iMapType == MAPTYPE_Town) ||
		(!Character->psModelAnimation || Character->psModelAnimation->iType == ANIMATIONTYPE_Die))
	{
		if (Character)
			Character->HoSkillCode = 0;
...
HoEffectMgr::~HoEffectMgr()
{
	...
	for (index = 0; index < MAX_EFFECTOBJECT_BUFFER; index++)
		DELET(EffectZSortBuffer[index]);

	for (index = 0; index < MAX_EFFECTOBJECT_BUFFER; index++)
		DELET(EffectObjectBuffer[index]);
...
void HoEffectMgr::Init()
{
	...
	ZeroMemory(ChrInEffectObject, sizeof(ChrInEffectObject));
	...
	index = 0;
	MaterialNum[index] = CreateTextureMaterial("Effect\\ImageData\\Particle\\dust.tga", 0, 0, 0, SMMAT_BLEND_LAMP);
	index++;

	MaterialNum[index] = CreateTextureMaterial("Effect\\ImageData\\Particle\\bubble.tga", 0, 0, 0, SMMAT_BLEND_LAMP);
	index++;

	MaterialNum[index] = CreateTextureMaterial("Effect\\ImageData\\Particle\\Red.tga", 0, 0, 0, SMMAT_BLEND_LAMP);
	index++;

	MaterialNum[index] = CreateTextureMaterial("Effect\\ImageData\\Particle\\Blue.tga", 0, 0, 0, SMMAT_BLEND_LAMP);
	index++;

	MaterialNum[index] = CreateTextureMaterial("Effect\\ImageData\\Particle\\cc.tga", 0, 0, 0, SMMAT_BLEND_LAMP);
	index++;

	MaterialNum[index] = CreateTextureMaterial("Effect\\ImageData\\Particle\\Fire.tga", 0, 0, 0, SMMAT_BLEND_LAMP);
	index++;

	MaterialNum[index] = CreateTextureMaterial("Effect\\ImageData\\Particle\\flare.tga", 0, 0, 0, SMMAT_BLEND_LAMP);
	index++;

	MaterialNum[index] = CreateTextureMaterial("Effect\\ImageData\\Particle\\bkh0.tga", 0, 0, 0, SMMAT_BLEND_SHADOW);
	index++;

	MaterialNum[index] = CreateTextureMaterial("Effect\\ImageData\\Particle\\Red_KTJimsi.tga", 0, 0, 0, SMMAT_BLEND_SHADOW);
	index++;
...
int HoEffectMgr::Draw(int x, int y, int z, int ax, int ay, int az)
{
	...
	GRAPHICENGINE->GetDevice()->SetRenderState(D3DRS_ZWRITEENABLE, TRUE);
	GRAPHICENGINE->GetDevice()->SetRenderState(D3DRS_LASTPIXEL, FALSE);
	return TRUE;
}
	...
	case SKILL_ARROW_OF_TRAP_END:
	{
		...
		for (int index = 0; index < 10 + level / 2; index++)
		{
			...
			if (patDest != NULL)
			{
				...
				destPos.x = int(destX + (GetCos[ang] / 65536.f)*(4000.f + rand() % (level * 2000)));
				destPos.y = int(destY);
				destPos.z = int(destZ + (GetSin[ang] / 65536.f)*(4000.f + rand() % (level * 2000)));

				patDest->StartDest(currentPos, destPos, PatObj[6], SKILL_ARROW_OF_TRAP_END);
				patDest->AnimationEnd = PatAnimationEnd[6];
				AssaParticle_CircleTrap(&currentPos, &destPos, rand() % 3);
...
int HoEffectMgr::StartSkill(int x, int y, int z, int angleX, int angleY, int angleZ, int skillType, int level)
{
	...
	case SKILL_M_BLOW:
	{
		...
		extPrimitive->StartBillRect(x, y, z, 600, 600, "SkillTripleImpactHit.ini", ANI_ONE);
		...
		for (index = 0; index < 20; index++)
		{
			...
			if (billBoard != NULL)
			{
				...
				destPos.y = int(cosf(RandomPitch)*((20.f)*(20.f)) + y);
				destPos.x = int(sinf(RandomPitch)*(20.f) * cosf(RandomYaw)*(20.f) + x);
				destPos.z = int(sinf(RandomPitch)*(20.f) * sinf(RandomYaw)*(20.f) + z);
				...
				billBoard->StartDestPath(worldPos, localPos, destPos, size, "SkillTripleImpactParticle.ini", 1600);
		...
		for (index = 0; index < 15; index++)
		{
			...
			primitiveBillBoard->SetPhysics(physicsParticle, PRIMITIVE_DEFAULT_RECT);
	...
	case SKILL_PIKE_WIND:
	{
		...
		for (index = 0; index < 15 + level; index++)
		{
			...
			if (pat != NULL)
			{
				pat->StartAni(x + int((GetCos[ang] / 65536.f)*(level * 650)), y, int(z + (GetSin[ang] / 65536.f)*(level * 650)), 0, ANGLE_180 - ang, 0, PatObj[3]);
	...
	case SKILL_S_ROPE:
	{
		...
		for (int index = 0; index < 6; index++)
		{
			...
			extPrimitive->StartBillRect(x, y + 8000, z, "ThunderRoop2.ini", ANI_ONE);
			if (!EffectTimeObjectBuffer[timeObjectIndex].AddObject(extPrimitive, 45 + index * 2))
			{
				delete extPrimitive;
				extPrimitive = NULL;
	...
	case EFFECT_BLOODKNIGHT_SKILL:
	{
		...
		for (index = 0; index < 4 + level / 5; index++)
		{
			HoEffectPat	*pat = new HoEffectPat();
			if (pat != NULL)
			{
...
int HoEffectMgr::StartSkill(int x, int y, int z, int angleX, int angleY, int angleZ, Unit *character, int skillType, int level)
{
	switch (skillType)
	{
	case SKILL_SHIELD1:
	{
		...
		for (index = 0; index < CHR_IN_EFFECT_OBJECT_MAX; index++)
		{
			...
			{
				if (ChrInEffectObject[index].object->GetSkillCode() == SKILL_SHIELD1 && ((HoEffectShield *)ChrInEffectObject[index].object)->Character == character)
					return FALSE;
			}
		}
	...
	case SKILL_SHIELD2:
	{
		...
		for (index = 0; index < CHR_IN_EFFECT_OBJECT_MAX; index++)
		{
			if (ChrInEffectObject[index].object != NULL && character != NULL)
			{
				if (ChrInEffectObject[index].object->GetSkillCode() == SKILL_SHIELD2 && ((HoEffectShield *)ChrInEffectObject[index].object)->Character == character)
					return FALSE;
			}
		}
	...
	case SKILL_GOLDEN_FALCON:
	{
		...
		for (index = 0; index < CHR_IN_EFFECT_OBJECT_MAX; index++)
		{
			if (ChrInEffectObject[index].object != NULL && character != NULL)
			{
				if (ChrInEffectObject[index].object->GetSkillCode() == SKILL_GOLDEN_FALCON && ((HoEffectPatFalcon *)ChrInEffectObject[index].object)->Character == character)
				{
					((HoEffectPatFalcon *)ChrInEffectObject[index].object)->pCounter = 0;
					((HoEffectPatFalcon *)ChrInEffectObject[index].object)->pEndCounter = level * 70;
					return FALSE;
				}
			}
		}
		...
		if (patFalcon != NULL)
		{
			...
			for (index = 0; index < CHR_IN_EFFECT_OBJECT_MAX; index++)
			{
				if (ChrInEffectObject[index].object == NULL)
				{
					if (patFalcon != NULL && character != NULL)
					{
						ChrInEffectObject[index].object = patFalcon;
						break;
					}
				}
			}
	...
	case SKILL_FALCON:
	{
		...
		for (index = 0; index < CHR_IN_EFFECT_OBJECT_MAX; index++)
		{
			if (ChrInEffectObject[index].object != NULL && character != NULL)
			{
				if (ChrInEffectObject[index].object->GetSkillCode() == SKILL_FALCON && ((HoEffectPatFalcon *)ChrInEffectObject[index].object)->Character == character)
				{
					((HoEffectPatFalcon *)ChrInEffectObject[index].object)->pCounter = 0;
					((HoEffectPatFalcon *)ChrInEffectObject[index].object)->pEndCounter = level * 70;
					return FALSE;
	...
	case SKILL_SCOUT_HAWK:
	{
		...
		for (index = 0; index < CHR_IN_EFFECT_OBJECT_MAX; index++)
		{
			if (ChrInEffectObject[index].object != NULL && character != NULL)
			{
				if (ChrInEffectObject[index].object->GetSkillCode() == SKILL_SCOUT_HAWK && ((HoEffectPatHawk *)ChrInEffectObject[index].object)->Character == character)
					return FALSE;
			}
		}
...
int HoEffectMgr::Start(int x, int y, int z, int effectType, int level)
{
	...
	{
		SetDynLight(x, y, z, 255, 50, 50, 180, 250, 1);
	...
	case EFFECT_INFERNO_HIT:
	{
		...
		for (index = 0; index < 20; index++)
		{
			...
			if (primitiveBillBoard != NULL)
			{
				...
				currentPos.y = y;
				currentPos.z = z;
				primitiveBillBoard->StartPath(currentPos, "ParticleMeteo4.ini", ANI_LOOP);
		...
		for (index = 0; index < 20; index++)
		{
			...
			if (billBoard != NULL)
			{
				...
				billBoard->StartDestPath(worldPos, localPos, destPos, size, "ParticleMeteo5.ini", 700);
				if (!EffectTimeObjectBuffer[timeObjectIndex].AddObject(billBoard, index*rand() % 20))
				{
					delete billBoard;
					billBoard = NULL;
		...
		{
			HoPrimitiveBillboard *extPrimitive = new HoPrimitiveBillboard();
			extPrimitive->StartBillRect(x + rand() % 2800 * index, y, z + rand() % 2800 * index, "Meteo2.ini", ANI_ONE);
	...
	case EFFECT_WLORD_HIT:
	{
		int index;

		SetDynLight(x, y, z, 50, 50, 255, 255, 200, 1);
		...
		for (index = 0; index < 20; index++)
		{
			...
			if (primitiveBillBoard != NULL)
			{
				...
				currentPos.y = y;
				currentPos.z = z;
				primitiveBillBoard->StartPath(currentPos, "ParticleWitchLord1.ini", ANI_LOOP);
		...
		for (index = 0; index < 20; index++)
		{
			...
			if (billBoard != NULL)
			{
				...
				billBoard->StartDestPath(worldPos, localPos, destPos, size, "ParticleWitchLord2.ini", 700);
				if (!EffectTimeObjectBuffer[timeObjectIndex].AddObject(billBoard, index*rand() % 20))
				{
					delete billBoard;
					billBoard = NULL;
		...
		{
			HoPrimitiveBillboard *extPrimitive = new HoPrimitiveBillboard();
			extPrimitive->StartBillRect(x + rand() % 2800 * index, y, z + rand() % 2800 * index, "Meteo4.ini", ANI_ONE);
...
int	HoEffectMgr::Start(int x, int y, int z, int effectType, Unit *character)
{
	switch (effectType)
	{
	case EFFECT_STUN1:
...
int HoEffectMgr::StartMonsterDest(POINT3D current, POINT3D dest, POINT3D angle, int effectType)
{
	...
	case MONSTER_SHADOW_SHOT2:
	{
		...
		{
			dest.y -= 5000;
			effectMove->Start(current, dest, PatObj[24], MONSTER_SHADOW_SHOT2);
			if (!EffectTimeObjectBuffer[timeObjectIndex].AddObject(effectMove, 0))
			{
				delete effectMove;
				effectMove = NULL;
...
int HoEffectMgr::StartMonsterDest(POINT3D current, POINT3D dest, int effectType)
{
	...
	case MONSTER_TURTLEDRAGON_SHOT1:
	{
		int timeObjectIndex = EmptyTimeObjectBufferIndex();
		if (timeObjectIndex == -1)
			return FALSE;
		...
		if (effectMove != NULL)
		{
			current.y += 5000;
			dest.y -= 15 * fONE;
			effectMove->Start(current, dest, PatMissile, MONSTER_TURTLEDRAGON_SHOT1);
			if (!EffectTimeObjectBuffer[timeObjectIndex].AddObject(effectMove, 0))
			{
				delete effectMove;
				effectMove = NULL;
	...
	case MONSTER_TURTLEDRAGON_SHOT2:
	{
		int timeObjectIndex = EmptyTimeObjectBufferIndex();
		if (timeObjectIndex == -1)
			return FALSE;
		...
		if (effectMove != NULL)
		{
			current.y += 5000;
			dest.y -= 15 * fONE;
			effectMove->Start(current, dest, PatMissile, MONSTER_TURTLEDRAGON_SHOT2);
			if (!EffectTimeObjectBuffer[timeObjectIndex].AddObject(effectMove, 0))
			{
				delete effectMove;
				effectMove = NULL;
	...
	case MONSTER_IMP_SHOT4:
	{
		...
		if (tracker != NULL)
		{
			...
			if (particleSystem != NULL)
			{
				...
				particleSystem->Start(currentPos.x, currentPos.y, currentPos.z, MaterialNum[2], MONSTER_IMP_SHOT4);

				tracker->Start(currentPos, destPos, MONSTER_IMP_SHOT4, particleSystem);
	...
	case MONSTER_IMP_SHOT5:
	{
		...
		if (tracker != NULL)
		{
			...
			if (particleSystem != NULL)
			{
				...
				particleSystem->Start(currentPos.x, currentPos.y, currentPos.z, MaterialNum[2], MONSTER_IMP_SHOT5);

				tracker->Start(currentPos, destPos, MONSTER_IMP_SHOT5, particleSystem);
	...
	case MONSTER_PIGON_INFERNO:
	{
		...
		if (tracker != NULL)
		{
			...
			if (particleSystem != NULL)
			{
				...
				particleSystem->Start(currentPos.x, currentPos.y, currentPos.z, MaterialNum[1], MONSTER_PIGON_INFERNO);
				tracker->Start(currentPos, destPos, effectType, particleSystem);
				if (!EffectTimeObjectBuffer[timeObjectIndex].AddObject(tracker, 0))
				{
					delete tracker;
	...
	case MONSTER_PIGON_WLORD:
	{
		...
		if (tracker != NULL)
		{
			...
			if (particleSystem != NULL)
			{
				...
				particleSystem->Start(currentPos.x, currentPos.y, currentPos.z, MaterialNum[1], MONSTER_PIGON_WLORD);
				tracker->Start(currentPos, destPos, effectType, particleSystem);
				if (!EffectTimeObjectBuffer[timeObjectIndex].AddObject(tracker, 0))
				{
					delete tracker;
	...
	case MONSTER_PIGON_DMYSTERY:
	{
		...
		if (tracker != NULL)
		{
			...
			if (particleSystem != NULL)
			{
				...
				particleSystem->Start(currentPos.x, currentPos.y, currentPos.z, MaterialNum[1], MONSTER_PIGON_DMYSTERY);
				tracker->Start(currentPos, destPos, effectType, particleSystem);
				if (!EffectTimeObjectBuffer[timeObjectIndex].AddObject(tracker, 0))
				{
					delete tracker;
...
int HoEffectMgr::StartMonster(POINT3D current, int effectType)
{
	...
	case MONSTER_PIGON_PARTICLE1_BLG:
	{
		int objIndex = -1;
		int timeObjectIndex = EmptyTimeObjectBufferIndex();
		if (timeObjectIndex == -1)
	...
	case MONSTER_PIGON_PARTICLE1_BLM:
	{
		int objIndex = -1;
		int timeObjectIndex = EmptyTimeObjectBufferIndex();
		if (timeObjectIndex == -1)
	...
	case MONSTER_PIGON_POWER4:
	{
		...
		HoEffectPower *effectPower = new HoEffectPower();
		effectPower->Start(current.x, current.y, current.z, 3);
	...
	case EFFECT_DMYSTERY_HIT:
	{
		...
		if (effectType == EFFECT_DMYSTERY_HIT)
			primitiveBillBoard->StartBillRect(current.x, current.y, current.z, 60, 60, "RBlackHole.ini", ANI_ONE);
		...
		for (index = 0; index < 10; index++)
		{
			...
			if (primitiveBillBoard != NULL)
			{
				...
				if (effectType == EFFECT_DMYSTERY_HIT)
					primitiveBillBoard->StartPath(currentPos, "ParticleMeteo1_red.ini", ANI_LOOP);
				else
					primitiveBillBoard->StartPath(currentPos, "ParticleMeteo1_blue.ini", ANI_LOOP);
	...
	case MONSTER_REDEYE_SKILL:
	{
		...
		for (int index = 0; index < 20; index++)
		{
			...
			if (effectMove != NULL)
			{
				...
				effectMove->Start(cur, dest, MONSTER_REDEYE_SKILL);
				if (!EffectTimeObjectBuffer[timeObjectIndex].AddObject(effectMove, index * 7))
				{
					delete effectMove;
					effectMove = NULL;
	...
	case EFFECT_LANDMINNING_SKILL:
	{
		...
		for (int index = 0; index < 20; index++)
		{
			...
			if (effectMove != NULL)
			{
				...
				cur.x = current.x + int((GetSin[ang] / 65536.f)*(float(5000) + rand() % 1500));
				cur.y = current.y + 8000;
				cur.z = current.z + int((GetCos[ang] / 65536.f)*(float(5000) + rand() % 1500));

				dest.x = cur.x + int((GetSin[ang] / 65536.f)*(float(22000) + rand() % 6000));
				dest.y = cur.y;
				dest.z = cur.z + int((GetCos[ang] / 65536.f)*(float(22000) + rand() % 6000));

				effectMove->Start(cur, dest, EFFECT_LANDMINNING_SKILL);

				if (!EffectTimeObjectBuffer[timeObjectIndex].AddObject(effectMove, index * 3))
				{
					delete effectMove;
					effectMove = NULL;
					return FALSE;
...
int HoEffectMgr::StartChrState(Unit *chr, int effectType, int liveCount)
{
	switch (effectType)
	{
...
int HoEffectTracker::Main()
{
	...
	if ((SkillCode == MONSTER_PIGON_SHOT1) ||
		...
		(SkillCode == MONSTER_IMP_SHOT4) ||
		(SkillCode == MONSTER_IMP_SHOT5) ||
		(SkillCode == MONSTER_PIGON_INFERNO) ||
		(SkillCode == MONSTER_PIGON_DMYSTERY) ||
		(SkillCode == MONSTER_PIGON_WLORD))
	{
		if (EffectObj)
		{
			if (liveFlag)
			...
			if (length < 800 * 800 || liveCount > 3)
			{
				...
				case MONSTER_PIGON_INFERNO:
					EffectObj->SetState(FALSE);
					StartFlag = FALSE;
					StartEffect(currentPos.x, currentPos.y, currentPos.z, EFFECT_INFERNO_HIT);
					esPlaySound(esSOUND_FIGON_IMPACT, GetDistVolume(currentPos.x, currentPos.y, currentPos.z));
					break;
				case MONSTER_PIGON_WLORD:
					EffectObj->SetState(FALSE);
					StartFlag = FALSE;
					StartEffect(currentPos.x, currentPos.y, currentPos.z, EFFECT_WLORD_HIT);
					esPlaySound(esSOUND_FIGON_IMPACT, GetDistVolume(currentPos.x, currentPos.y, currentPos.z));
					break;
				}
				liveCount = 0;
				...
				if (SkillCode == MONSTER_IMP_SHOT4)
					StartEffectMonster(currentPos.x, currentPos.y, currentPos.z, SKILL_SILRAPHIM_HIT);
				if (SkillCode == MONSTER_IMP_SHOT5)
					StartEffectMonster(currentPos.x, currentPos.y, currentPos.z, MONSTER_WEB_HIT1);
				...
				if (SkillCode == MONSTER_PIGON_DMYSTERY)
					StartEffectMonster(currentPos.x, currentPos.y, currentPos.z, EFFECT_DMYSTERY_HIT);
	...
	if ((SkillCode == SKILL_S_IMPACT))
	{
		...
		{
			StartFlag = FALSE;
			StartEffect(currentPos.x, currentPos.y, currentPos.z, EFFECT_IMPACT2, Level);
		...
		{
			sparkCount = 0;
			StartChildEffect(currentPos.x, currentPos.y, currentPos.z, EFFECT_IMPACT1, this);
		}

		sparkCount++;
	}

	if (SkillCode == MONSTER_IMP_SHOT2 || SkillCode == EFFECT_FIRE_CRACKER || SkillCode == MONSTER_IMP_SHOT4 || SkillCode == MONSTER_IMP_SHOT5)
	{
		IMP_SHOT2_liveCount++;
		...
		if (IMP_SHOT2_liveCount == 0)
		{
			...
			else if (SkillCode == MONSTER_IMP_SHOT4)
				StartEffectMonster(currentPos.x, currentPos.y, currentPos.z, MONSTER_PIGON_PARTICLE1_BLG);
			else if (SkillCode == MONSTER_IMP_SHOT5)
				StartEffectMonster(currentPos.x, currentPos.y, currentPos.z, MONSTER_PIGON_PARTICLE1_BLM);
		}
	}
	if (SkillCode == MONSTER_IMP_SHOT3)
	{
...
int HoEffectTracker::Start(POINT3D currentPos, POINT3D destPos, int skillCode, HoEffectObject *effectObj, int level)
{
		...
		(SkillCode == MONSTER_PIGON_WLORD))
	{
		...
		if (SkillCode == MONSTER_PIGON_SHOT1 || SkillCode == MONSTER_FORZENMIST_SHOT1 || SkillCode == MONSTER_FORZENMIST_SHOT2 || SkillCode == MONSTER_IMP_SHOT4 || SkillCode == MONSTER_PIGON_INFERNO || SkillCode == MONSTER_PIGON_WLORD)
		{
			Vx += vx / length * 1500;
			Vy += vy / length * 1500;
			Vz += vz / length * 1500;
		}
		else if (SkillCode == MONSTER_IMP_SHOT1 || SkillCode == MONSTER_MEPHIT_SHOT1 || SkillCode == MONSTER_MEPHIT_SHOT2 || SkillCode == MONSTER_IMP_SHOT2 || SkillCode == MONSTER_IMP_SHOT5)
		{
			Vx += vx / length * 1300;
			Vy += vy / length * 1300;
			Vz += vz / length * 1300;
		}
		else if (SkillCode == MONSTER_IMP_SHOT3 || SkillCode == MONSTER_PIGON_DMYSTERY)
		{
	...
	else if (SkillCode == SKILL_S_IMPACT)
	{
		Vx = 0.f;
		Vy = 0.f;
		Vz = 0.f;
...
int HoNewEffectChrMove::Start(Unit *chr, int skillCode, int liveCount)
{
...
int HoNewEffectChrMove::Main()
{
	...
	if (Character->psModelAnimation)
	{
		if (Character->psModelAnimation->iType == ANIMATIONTYPE_Die)
			StopEffect();
	}
...
int HoNewEffectMove::Start(POINT3D curPos, POINT3D destPos, POINT3D angle, smPAT3D *pat, int skillCode)
{
	...
	auto sPosition = GetRadian3D(int(WorldX + LocalX), int(WorldY + LocalY), int(WorldZ + LocalZ), DestPos.x, DestPos.y, DestPos.z);
	Angle.x = 0;
	Angle.y = (ANGLE_180 - sPosition.iY)&ANGCLIP;
	Angle.z = 0;
...
int HoNewEffectMove::Start(POINT3D curPos, POINT3D destPos, int skillCode)
{
	...
	if ((SkillCode == MONSTER_STYGIANLORD_PARTICLE1) || (SkillCode == MONSTER_COLDEYE_SKILL) || (SkillCode == MONSTER_REDEYE_SKILL) || (SkillCode == EFFECT_LANDMINNING_SKILL))
	{
		Velocity.x = 0.0f;
		Velocity.y = 0.0f;
		Velocity.z = 0.0f;
...
int HoNewEffectMove::Start(POINT3D curPos, POINT3D destPos, smPAT3D *pat, int skillCode)
{
	...
	else if (SkillCode == MONSTER_SHADOW_SHOT2)
	{
		auto sPosition = GetRadian3D(int(WorldX + LocalX), int(WorldY + LocalY), int(WorldZ + LocalZ), DestPos.x, DestPos.y, DestPos.z);
		Angle.x = sPosition.iX;
		Angle.y = sPosition.iY;
		...
		data.y = int(WorldY + LocalY);
		data.z = int(WorldZ + LocalZ);
		PartEmitterID = g_NewParticleMgr.Start("DMysteryParticle1", data);
		PartEmitterIDExt = g_NewParticleMgr.Start("DMysteryParticle2", data);
	}
	...
	else if (SkillCode == MONSTER_DMACHINE_MISSILE2)
	{
		...
		auto sPosition = GetRadian3D(int(WorldX + LocalX), int(WorldY + LocalY), int(WorldZ + LocalZ), int(WorldX + LocalX), int(WorldY + LocalY) + int(Velocity.y), int(WorldZ + LocalZ));
		Angle.x = sPosition.iX;
		Angle.y = sPosition.iY;
		Angle.z = sPosition.iZ;
		Angle.y = (-Angle.y + ANGLE_180)&ANGCLIP;
	...
	if (SkillCode == MONSTER_TURTLEDRAGON_SHOT1)
	{
		auto sPosition = GetRadian3D(int(WorldX + LocalX), int(WorldY + LocalY), int(WorldZ + LocalZ), DestPos.x, DestPos.y, DestPos.z);
		Angle.x = sPosition.iX;
		Angle.y = sPosition.iY;
		...
		data.y = int(WorldY + LocalY);
		data.z = int(WorldZ + LocalZ);
		PartEmitterID = g_NewParticleMgr.Start("TurtleDragonHit1", data);
	...
	else if (SkillCode == MONSTER_TURTLEDRAGON_SHOT2)
	{
		POINT3D data;
		data.x = int(WorldX + LocalX);
		data.y = int(WorldY + LocalY);
...
int HoNewEffectMove::Main()
{
	...
	if (SkillCode == MONSTER_SHADOW_SHOT1)
	{
		if (CurrentFrame >= 160 * 10)
		{
			...
			auto sPosition = GetRadian3D(int(WorldX + LocalX - Velocity.x*fONE), int(WorldY + LocalY - Velocity.y*fONE), int(WorldZ + LocalZ - Velocity.z*fONE), int(WorldX + LocalX), int(WorldY + LocalY), int(WorldZ + LocalZ));
			Angle.x = sPosition.iX&ANGCLIP;
			Angle.y = sPosition.iY&ANGCLIP;
			Angle.z = sPosition.iZ&ANGCLIP;
			Angle.y = (-Angle.y + ANGLE_180)&ANGCLIP;
	...
	if (SkillCode == MONSTER_REDEYE_SKILL)
	{
		if (ParticleStartFlag == FALSE)
		{
			POINT3D data;
			...
			data.y = int(WorldY + LocalY);
			data.z = int(WorldZ + LocalZ);
			PartEmitterID = g_NewParticleMgr.Start("RedEyeHit2", data);
			ParticleStartFlag = TRUE;
			g_NewParticleMgr.Start("RedEyeHit4", data);
		}
	...
	if (SkillCode == EFFECT_LANDMINNING_SKILL)
	{
		if (ParticleStartFlag == FALSE)
		{
			POINT3D data;
	...
	if (SkillCode == MONSTER_TURTLEDRAGON_SHOT1)
	{
		...
		if (length < 10)
		{
			AssaParticle_TurtleDragonHit(&DestPos);
			if (PartEmitterID != -1)
				g_NewParticleMgr.SetStop(PartEmitterID);
	...
	if (SkillCode == MONSTER_TURTLEDRAGON_SHOT2)
	{
		POINT3D pos;
		pos.x = int(WorldX + LocalX);
		pos.y = int(WorldY + LocalY);
		...
		if (length < 10)
		{
			AssaParticle_TurtleDragonSkill(&DestPos);
			if (PartEmitterID != -1)
				g_NewParticleMgr.SetStop(PartEmitterID);
...
int StartEffect(int x, int y, int z, int effectType, Unit *character)
{
	//EnterCriticalSection(&cLoadSection);
...
int StartSkill(int x, int y, int z, int angleX, int angleY, int angleZ, Unit *character, int skillType, int level)
{
	if (character == NULL)
		return FALSE;
...
int StartEffectMonster(int x, int y, int z, int effectType)
{
	...
	case MONSTER_BONEHOUND_HIT:
	{
		...
		{
		case 0:
			g_NewParticleMgr.Start("BonehoundHit1", pos);
			break;
		case 1:
			g_NewParticleMgr.Start("BonehoundHit2", pos);
			break;
		case 2:
			g_NewParticleMgr.Start("BonehoundHit3", pos);
			break;
		}
		break;
	}
	...
	case MONSTER_MINEGOLEM_HIT1:
		// �阮천틜�
		SetDynLight(pos.x, pos.y, pos.z, 255, 100, 100, 255, 200, 1);
		g_NewParticleMgr.Start("MineGolemHit", pos);
		break;
		// 직槿삽黛
	case SKILL_SILRAPHIM_HIT:
...
int StartEffectMonster(int x, int y, int z, int angleX, int angleY, int angleZ, int effectType)
{
	...
	case MONSTER_OMICRON_HIT1:
		g_NewParticleMgr.Start("OmiCronHit1", pos);
		break;
	case MONSTER_MINEGOLEM_HIT2:
		g_NewParticleMgr.Start("MineGolemSkill", pos);
		break;
	}
...
int StartEffectChrState(Unit *chr, int effectType, int liveCount)
{
...
int DrawMenuFlame()
{
	if (SETTINGHANDLE->Get().bHightTextureQuality == TRUE)
		MenuFlame.Draw();
...
int InitEffect()
{
	...
	AnimDataMgr.ImageLoad("Effect\\ImageData\\ParticleMeteo4.ini");
	AnimDataMgr.ImageLoad("Effect\\ImageData\\ParticleMeteo5.ini");
	AnimDataMgr.ImageLoad("Effect\\ImageData\\Meteo2.ini");
	AnimDataMgr.ImageLoad("Effect\\ImageData\\RBlackHole.ini");
	AnimDataMgr.ImageLoad("Effect\\ImageData\\ParticleMeteo1_red.ini");
	AnimDataMgr.ImageLoad("Effect\\ImageData\\ParticleWitchLord1.ini");
	AnimDataMgr.ImageLoad("Effect\\ImageData\\ParticleWitchLord2.ini");
	AnimDataMgr.ImageLoad("Effect\\ImageData\\Meteo4.ini");

#ifdef Making_SKILL_TORNADO3
	AnimDataMgr.ImageLoad("Effect\\ImageData\\SkillTornado3.ini");
#endif

	AnimDataMgr.SequenceLoad("Effect\\AnimationData\\NormalHit1.ini"); //1
	...
	AnimDataMgr.SequenceLoad("Effect\\AnimationData\\ParticleWitchLord1.ini");
	AnimDataMgr.SequenceLoad("Effect\\AnimationData\\ParticleWitchLord2.ini");
	...
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\UndeadMinerHit.part");
	...
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\ElementWeaponFire.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\ElementWeaponIce.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\ElementWeaponLight.part");
	...
	//벽깥疳
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\IceWorm1.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\IceWorm2.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\IceWormHIt1.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\IceWormHIt2.part");
	//웁�슥驗�
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\MineGolemHit.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\MineGolemSkill.part");
	// 웁�슭㎁�
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\SealCrasherlSkill1.part");
	// 깥솖뗀籠
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\TarantulikaHit.part");

	// 桔씩杰세콘可
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\ItechnicianSkill.part");
	// 桔씩杰세減可
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\LtechnicianSkill.part");
	// 桔씩杰묏넋可
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\LengineerSkill.part");
	// 桔씩杰�閉먼�
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\IsoldierSkill.part");
	// 桔씩杰쒸括깡
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\LguardianSkill.part");
	// 멕섬핫쫠諒
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\IeliteSkill.part");
	// 간옹薑
	...
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\LizardBigmamaSkill.part");
	//鱇즈바
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\LizardCommanderHit.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\LizardCommanderSkill.part");

	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\LizardDoctorSkill.part");
	//怒댐
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\LizardElderHit.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\LizardElderSkill.part");

	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\LizardPapaSkill.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\LizardSoldierSkill.part");

	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\BlueMountainHit1.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\MorgoniaSkill1.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\MorgoniaHit1.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\RedEyeHit1.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\RedEyeHit2.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\RedEyeHit3.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\RedEyeHit4.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\TurtleDragonHit1.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\TurtleDragonHit2.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\TurtleDragonHit3.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\TurtleDragonSkill1.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\Bonehoundskill1.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\Bonehoundskill2.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\Bonehoundskill3.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\BonehoundHit1.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\BonehoundHit2.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\BonehoundHit3.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\BonehoundHit4.part");


	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\NihilSkill.part"); //1
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\NihilHit.part"); // 2
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\AceroStart.part"); // 3
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\AceroSkill.part"); // 4
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\AceroStart.part"); // 5
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\ChalybsStart.part"); //6
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\ChalybsSkill.part"); //7
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\ChalybsHit.part"); //8
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\GreedyStart.part"); //9
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\GreedySkill.part"); //10
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\GreedyBody.part"); //11
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\GreedyHit1.part"); //12
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\GreedyHit2.part"); //13
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\RGuardSkill.part"); //14
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\RGuardHit.part"); //15
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\DMysteryParticle1.part"); //16
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\DMysteryParticle2.part"); //17
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\MonsterIgnisSkill.part"); //18
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\MonsterIgnisHit.part"); //19
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\MonsterNajanSkill1.part"); //20
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\MonsterNajanSkill2.part"); //21
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\MonsterNajanHit.part"); //22
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\CriosSkill.part"); //23
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\CriosHit.part"); //24
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\WLordSkill1.part"); //25
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\WLordSkill2.part"); //26
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\MonsterMidrandaUpperArm.part"); //27
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\MonsterMidrandaPelvis.part"); //28
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\MonsterMidrandaHand.part"); //29
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\MonsterMidrandaSkill.part"); //30
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\MonsterMidrandaHit1.part"); //31
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\MonsterMidrandaHit2.part"); //32
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\DeyStart.part"); //33
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\DeySkill.part"); //34
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\DeyHit.part"); //35
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\ArhdyraSkill.part"); //36
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\ArhdyraHit.part"); //37
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\GurkobStart.part"); //38
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\GurkobHit.part"); //39
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\ShathlaStart.part"); //40
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\ShathlaSkill.part"); //41
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\ShathlaHit.part"); //42
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\FaugnBody.part"); //43
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\FaugnSkill.part"); //44
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\FaugnHit.part"); //45
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\KoonSkill.part"); //46
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\KoonHit.part"); //47
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\YagdithaStart.part"); //48
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\YagdithaSkill.part"); //49
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\YagdithaHit1.part"); //50
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\YagdithaHit2.part"); //51
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\script\\YagdithaBody.part"); //52
	...
	//댕괬루汽
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\Skill5_Priestess_Meteo1.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\Skill5_Priestess_Meteo2.part");

	//�誅Ο느�
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\Skill5MagicianPrimaIgnisShot.part");

	//늬와세콘
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\Stingger.part");//2
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\RunningHit.part");//3
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\Wisp1.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\Wisp2.part");//1
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\VenomThrone.part");//4
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\Alas.part");//4
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\SoulShock.part");//5
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\SoreSword.part");//6
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\BeatUp.part");//7
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\Inpes.part");//8
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\FrostWindAttack.part");//9
	...
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\PollutedAttack.part");//10
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\PastingShadowBegin.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\PastingShadowAttack.part");//11
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\ViolenceStab1.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\ShadowBomb.part");//12
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\RisingSlash1.part");//12
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\RisingSlash2.part");//13
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\ViolenceStab2.part");//14
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\ViolenceStab3.part");//14
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\Storm.part");//15

	//힉찮
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\DarkBolt.part");//16
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\DarkBoltHit.part");//19
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\DarkWave1.part");//17
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\DarkWave2.part");//18
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\DarkWaveHit.part");//20
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\CurseLazy1.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\SpiritualFlare.part");//21
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\SoulManacle.part");//22
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\RainMaker1.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\RainMaker2.part");//24
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\LandOfGhost1.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\LandOfGhost2.part");//25
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\ScratchAura.part");
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\Haunt1.part");//26
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\Haunt2.part");//27
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\ChasingHunt.part");//23
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\Scratch1.part");//28
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\Scratch2.part");//28
	...
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\MourningOfPreyHit.part");//29
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\PressOfDeity1.part");//30
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\PressOfDeity2.part");//30
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\JudgementStart.part");//30
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\JudgementHit.part");//30
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\CreedKeep.part");//30
	...
	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\GhostyNail2.part"); //31


	g_NewParticleResMgr.LoadScript("Effect\\Particle\\Script\\HighLevelUserMerit_Star.part"); //31
...
int DrawEffect(int x, int y, int z, int ax, int ay, int az)
{
	if (SETTINGHANDLE->Get().bDebugMode)
	{
		...
		if (VRKeyBuff[VK_NUMPAD2])
		{
			...
			if (count == 0)
			{
				Unit *MonsterCharacter[6];
				int count = 0;
				int NearDist = 300 * 300;


				for (int cnt = 0; cnt < MAX_UNITS; cnt++)
				{
					if (chrOtherPlayer[cnt].bActive && chrOtherPlayer[cnt].sCharacterData.iType == CHARACTERTYPE_Monster)
					{
						if (chrOtherPlayer[cnt].sCharacterData.iMonsterType != MONSTERTYPE_Friendly && chrOtherPlayer[cnt].sCharacterData.sHP.sMin > 0)
						{
				...
				SkillMetalionSphereStart(lpCurPlayer);
			else if (count == 3)
			...
			else if (count == 5)
			{
				...
				POINT3D pos;
				pos.x = lpCurPlayer->pX + sPosition.iX;
				pos.y = lpCurPlayer->pY + 5000;
				pos.z = lpCurPlayer->pZ + sPosition.iZ;
			...
			else if (count == 12)
			{
				...
				SkillSaintGodlyShieldStart(lpCurPlayer);
			else if (count == 14)
			...
			{
				auto sPosition = GetMoveLocation(0, 0, 150 * fONE, 0, lpCurPlayer->Angle.y, 0);
				POINT3D curPos;
				POINT3D desPos;
				curPos.x = lpCurPlayer->pX;
				...
				desPos.x = lpCurPlayer->pX + sPosition.iX;
				desPos.y = lpCurPlayer->pY;
				desPos.z = lpCurPlayer->pZ + sPosition.iZ;

				SkillValhallaXRage(&curPos, &desPos);
			}
			else if (count == 20)
			{
				auto sPosition = GetMoveLocation(0, 0, 200 * fONE, 0, lpCurPlayer->Angle.y, 0);
				POINT3D curPos;
				POINT3D desPos;
				curPos.x = lpCurPlayer->pX;
				curPos.y = lpCurPlayer->pY + 10000;
