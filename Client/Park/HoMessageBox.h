...
public:
	...
	void DrawTwoLineMessage(int x, int y, char *message1, char *message2, DWORD color1 = D3DCOLOR_XRGB(0,0,0), DWORD color2 = D3DCOLOR_XRGB(0,0,0), smTEXTUREHANDLE *clanMark = NULL, BOOL selected = FALSE, int bellaMarkIndex = -1,int vipIndex = -1);
	
	void DrawCharacterMessage(int x, int y, char *message, int lineSize = 30, smTEXTUREHANDLE *clanMark = NULL, char *clanName = NULL, DWORD color1 = D3DCOLOR_XRGB(0,0,0), BOOL selected = FALSE, int bellaMarkIndex = -1,int vipIndex = -1);
	
	void DrawCharacterMessageFrame(int x, int y, int messageWidth, int messageHeight, bool tailFlag = false, BOOL selected = FALSE); 
	void DrawCharacterMessageFrame1(int x, int y, int messageWidth, int messageHeight,int linecount, bool tailFlag, BOOL selected = FALSE);
	void DrawClanMessage(int x, int y, char *message, int lineSize, DWORD color = D3DCOLOR_XRGB(255,255,255));
	void DrawClanMessageFrame(int x, int y, int messageWidth, int messageHeight);
	void DrawClanMark(smTEXTUREHANDLE *clanMark, char *clanName, int x, int y, BOOL frameFlag = FALSE);
	void DrawClanName(int x, int y, char *clanName);
	void DrawBellaIcon(int x, int y, int bellaIndex = 1);
	// 斡횡VIP暠깃
	void DrawVipIcon(int x,int y,int vipIndex);
	void DrawDayNightMessage(int x, int y, char *message1, char *message2);
	void DrawShopSellMessage(int x, int y, char *message, int lineSize = 30, smTEXTUREHANDLE *clanMark = NULL, DWORD color1 = D3DCOLOR_XRGB(0, 0, 0), BOOL selected = FALSE);
	void DrawShopSellMessageFrame(int x, int y, int messageWidth, int messageHeight, bool tailFlag = false, BOOL selected = FALSE);
	
	void DrawShopByeMessage(int x, int y, char *message, int lineSize = 30, smTEXTUREHANDLE *clanMark = NULL,DWORD color1 = D3DCOLOR_XRGB(0,0,0), BOOL selected = FALSE);
	...
	void DrawMessageBox(int x, int y, char *message, int boxAttrib = BOX_ONE, int Color = D3DCOLOR_XRGB(255, 255, 255));
...
public:
	...
	BOOL	Main(int mouseX, int mouseY, int downFlag = 0);
	BOOL	Draw(int x, int y, char *message, int Color = D3DCOLOR_XRGB(255, 255, 255));
	void	Init();
