...
static void filterChar(const char *src, char *dst, const char *chars)
{
	...
	while (*srcp) 
	{
		...
		else 
		{
			...
			if (srcp > src && ((msb == 0x82 && lsb > 0x9e && lsb < 0xf2) || 
				(msb == 0x83 && lsb > 0x3f && lsb < 0x97)|| 
				(msb >= 0x88 && msb <= 0x9f && msb != 0x7f && lsb > 0x3f && lsb < 0xfd) || 
				(msb >= 0xe0 && msb <= 0xfc && msb != 0x7f && lsb > 0x3f && lsb < 0xfd))) 
			{
				*dstp = *srcp;   dstp += 1;
			}
		}
...
char *ConvertString(char * str, int max_len)
{
	...
	while (ptr = getField(ptr, '\n', line), line[0])
	{
		...
		while (cptr = strstr(string, curse), cptr)
		{
			...
			if ((int)(max_len) < (int)((cptr - string) + lstrlen(replace) + lstrlen(cptr + lstrlen(curse)))) 
			{
				strncpy(result,string, cptr - string);
				lstrcpy(result + (cptr - string), cptr + lstrlen(curse));
			}
			else 
			{               	    
				strncpy(result,string, cptr - string);
				lstrcpy(result + (cptr - string), replace);
				lstrcpy(result + (cptr - string + lstrlen(replace)), cptr + lstrlen(curse));
			}

			lstrcpy(string, result);
		}
	}
...
char *ConvertStringTHAI(char * str, int max_len)
{
	...
	while (ptr = getField(ptr, '\n', line), line[0]) 
	{
		...
		while (cptr = strstr(string, curse), cptr) 
		{
			...
			for(int i=0; i<(int)lstrlen(string); i++)
			{
				lstrcat(replace,"*");
			}
			if (!replace[0]) 
