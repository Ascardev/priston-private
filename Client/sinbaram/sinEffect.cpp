	...
	for (i = 0; i < SIN_MAX_EFFECT; i++)
	{
		if (sinEffect[i].State == SIN_EFFECT_DEAD)
		{
			...
			case SIN_PARTICLE_ROTATE_RISE:
				...
				sinEffect[i].sinFace = sinSetFace(cSin3D.MatParticleBlue[0], x + SIN_RAND_1000 - 1000 + sinEffect[i].MoveAngle.x, y + SIN_RAND_1000, z + SIN_RAND_1000 - 1000 + sinEffect[i].MoveAngle.z, ResultSize, ResultSize, 255, 255, 255, 255);
			...
			case SIN_EFFECT_RAGE_OF_SINBARAM:
				if (sinEffect_Process[i].Time == 1)
				{
					for (j = 0; j < 40; j++)
					{
						...
						sinEffect_Process[i].Index[j] = sinSetEffect(SIN_EFFECT_RISE_LINE, sinEffect_Process[i].Posi.x + px, sinEffect_Process[i].Posi.y, sinEffect_Process[i].Posi.z + pz, 2, 0, j * 128 & ANGCLIP);
					}
				}
				...
				if (sinEffect_Process[i].Time >= 300)
					sinEffect_Process[i].Kind = 0;
