...
void sinAssaParticleProcess(cASSAEFFECT *pEffect)
{
	...
	case ASSA_PARTICLE_CHAR_LIGHT:
		...
		{
			if (pEffect->Face.Transparency < 150)
		...
		else
		{
			...
			{
				if (pEffect->Face.Transparency > 0)
					pEffect->Face.Transparency--;
			}
		}
		break;
...
int AssaParticle_Char_Light(Unit *pChar, DWORD ColorCODE, int Time, int MaxAlpha, int  SizeW, int SizeH)
{
	switch (ColorCODE) {
