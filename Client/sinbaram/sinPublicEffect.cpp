...
void sinInitPublicEffect()
{
	...
	MatLine2[0] = CreateTextureMaterial(szBuff, 0, 0, 0, SMMAT_BLEND_LAMP);

	for (i = 0; i < 5; i++) {
		wsprintf(szBuff, "%s\\Light\\Light0%d.tga", Path, i + 1);
		MatLight5[i] = CreateTextureMaterial(szBuff, 0, 0, 0, SMMAT_BLEND_LAMP);
	}
	for (i = 0; i < 3; i++) {
		wsprintf(szBuff, "%s\\Hit\\HitYellow%d.tga", Path, i + 1);
		MatHit2Circle[i] = CreateTextureMaterial(szBuff, 0, 0, 0, SMMAT_BLEND_LAMP);
	}
	for (i = 0; i < 3; i++) {
		wsprintf(szBuff, "%s\\Hit\\HitYellow%d.tga", Path, i + 1);
		MatHit2Circle[i] = CreateTextureMaterial(szBuff, 0, 0, 0, SMMAT_BLEND_LAMP);
	...
	MatMonParticle[0] = CreateTextureMaterial(szBuff, 0, 0, 0, SMMAT_BLEND_SHADOW);

	for (i = 1; i < 10; i++) {
		wsprintf(szBuff, "%s\\Particle\\MonParticle0%d.tga", Path2, i + 1);
		MatMonParticle[i] = CreateTextureMaterial(szBuff, 0, 0, 0, SMMAT_BLEND_LAMP);

	}

	wsprintf(szBuff, "%s\\Ratoo\\glow.tga", Path2);
	MatMonVigorRatoo[0] = CreateTextureMaterial(szBuff, 0, 0, 0, SMMAT_BLEND_LAMP);
	wsprintf(szBuff, "%s\\Ratoo\\glow1.tga", Path2);
	MatMonVigorRatoo[1] = CreateTextureMaterial(szBuff, 0, 0, 0, SMMAT_BLEND_LAMP);
...
void sinPublicEffectMove(cSINEFFECT2 *pEffect)
{
	...
	case SIN_EFFECT_WIDELINE:
	{
		...
		auto sPosition = GetMoveLocation(0, 0, pEffect->MoveSpeed.z, pEffect->Angle.x, pEffect->Angle.y, 0);
		pEffect->Posi.x += sPosition.iX;
		pEffect->Posi.y += sPosition.iY;
		pEffect->Posi.z += sPosition.iZ;
	...
	{
		sinGetMoveLocation2(pEffect);
		auto sPosition = GetMoveLocation(0, 0, 50, pEffect->Angle.x, pEffect->Angle.y, 0);
		pEffect->sinFace.x += sPosition.iX;
		pEffect->sinFace.y += sPosition.iY;
		pEffect->sinFace.z += sPosition.iZ;
		pEffect->sinFace.y += pEffect->MoveSpeed.y + pEffect->Gravity;
	...
	{
		sinGetMoveLocation2(pEffect);
		auto sPosition = GetMoveLocation(0, 0, 128, pEffect->Angle.x, pEffect->Angle.y, 0);
		pEffect->sinFace.x += sPosition.iX;
		pEffect->sinFace.y += sPosition.iY;
		pEffect->sinFace.z += sPosition.iZ;
	}
	...
	case SIN_EFFECT_MUTENTTREE_HIT:
	{
		...
		pEffect->Gravity -= 5;
		pEffect->sinFace.x += sPosition.iX;
		pEffect->sinFace.y += sPosition.iY + pEffect->MoveSpeed.y + pEffect->Gravity;
		pEffect->sinFace.z += sPosition.iZ;
	}
		break;
	case SIN_EFFECT_RATOO:
		...
		{
			sinGetMoveLocation2(pEffect);
			auto sPosition = GetMoveLocation(0, 0, 256 * 32 * 4, pEffect->Angle.x, (-(pEffect->Angle.y&ANGCLIP) + ANGLE_180)&ANGCLIP, 0);
...
int sinEffect_WideContinueLine2(POINT3D *pPosi, int Mat)
{
	...
	auto sPosition = GetMoveLocation(0, 0, distance * 512 * 4, cSinEffect2[Index2].Angle.x, cSinEffect2[Index2].Angle.y, 0);
	cSinEffect2[Index2].Posi.x += sPosition.iX;
	cSinEffect2[Index2].Posi.y += sPosition.iY;
	cSinEffect2[Index2].Posi.z += sPosition.iZ;
...
int sinEffect_WideParticle(POINT3D *pPosi, int Mat, int Size)
{
	...
	cSinEffect2[Index2].MoveSpeed.z = 128;
	sinGetMoveLocation2(&cSinEffect2[Index2]);
	auto sPosition = GetMoveLocation(0, 0, 128, cSinEffect2[Index2].Angle.x, cSinEffect2[Index2].Angle.y, 0);
...
int sinEffect_Hit2(POINT3D *pPosi, int Mat)
{
	...
	for (int i = 0; i < 20; i++) 
	{
		...
		sinGetMoveLocation2(&cSinEffect2[Index2]);
		auto sPosition = GetMoveLocation(0, 0, 1024, cSinEffect2[Index2].Angle.x, cSinEffect2[Index2].Angle.y, 0);
		cSinEffect2[Index2].sinFace.x += sPosition.iX;
		cSinEffect2[Index2].sinFace.y += sPosition.iY;
		cSinEffect2[Index2].sinFace.z += sPosition.iZ;
...
int sinEffect_Sen(POINT3D *pPosi) //센 이펙트 
{
	...
	sinGetMoveLocation2(&cSinEffect2[Index2]);
	auto sPosition = GetMoveLocation(0, 0, 1024 * 2, cSinEffect2[Index2].Angle.x, cSinEffect2[Index2].Angle.y, 0);
	cSinEffect2[Index2].sinFace.x += sPosition.iX;
	cSinEffect2[Index2].sinFace.y += sPosition.iY;
	cSinEffect2[Index2].sinFace.z += sPosition.iZ;
...
int sinEffect_MutentTree_Fire(POINT3D *pPosi) //뮤턴트 트리 
{
	...
	while (Cnt--) {
		...
		cSinEffect2[Index2].MoveSpeed.y = rand() % 25 + 25;
		sinGetMoveLocation2(&cSinEffect2[Index2]);
		auto sPosition = GetMoveLocation(0, 0, 256 * 2, cSinEffect2[Index2].Angle.x, cSinEffect2[Index2].Angle.y, 0);
		cSinEffect2[Index2].sinFace.x += sPosition.iX;
		cSinEffect2[Index2].sinFace.y += sPosition.iY;
		cSinEffect2[Index2].sinFace.z += sPosition.iZ;
...
int sinEffect_MutentTree_Attack_Particle(POINT3D *pPosi)
{
	...
	while (Cnt--) {
		...
		sinGetMoveLocation2(&cSinEffect2[Index2]);
		auto sPosition = GetMoveLocation(0, 0, 64 * 7, cSinEffect2[Index2].Angle.x, cSinEffect2[Index2].Angle.y, 0);
		cSinEffect2[Index2].sinFace.x += sPosition.iX;
		cSinEffect2[Index2].sinFace.y += sPosition.iY;
		cSinEffect2[Index2].sinFace.z += sPosition.iZ;
	}
...
int sinEffect_Ratoo_Vigor(Unit *pChar)
{
	...
	cSinEffect2[Index2].MoveSpeed.y = rand() % 25 + 25;
	sinGetMoveLocation2(&cSinEffect2[Index2]);
	auto sPosition = GetMoveLocation(0, 0, 256 * 64, cSinEffect2[Index2].Angle.x, cSinEffect2[Index2].Angle.y, 0);
	cSinEffect2[Index2].sinFace.x += sPosition.iX;
	cSinEffect2[Index2].sinFace.y += sPosition.iY;
	cSinEffect2[Index2].sinFace.z += sPosition.iZ;
	return TRUE;
...
int sinEffect_RollingSmash(Unit *pChar, int x, int y, int z)
{
	...
	Assa = SetAssaEffect(0, "Tornado.ASE", pChar, &pos);
...
int sinEffect_Splash(Unit *pChar)
{
	...
	Assa = SetAssaEffect(0, "NatureSplash.ASE", pChar, &pos);

	cAssaEffect[Assa]->AniMaxCount = 30;
	cAssaEffect[Assa]->AniDelayTime = 3;
	cAssaEffect[Assa]->Angle.x = ANGLE_90;
...
int sinEffect_Benedic(Unit *pChar, int LiveCount)
{
	...
	if (AddAssaCodeEffectTime(pChar, SKILL_H_BENEDIC, SkillTime) == 0)
	{
		pos.x = pChar->pX;
		pos.y = pChar->pY;
		pos.z = pChar->pZ;
