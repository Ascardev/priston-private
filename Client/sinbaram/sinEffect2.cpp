...
int InitEffect2()
{
	...
	int i=0;
	char szBuff[128];
	Matstun = CreateTextureMaterial( "image\\Sinimage\\Effect\\stun\\Star.tga" , 0, 0, 0, SMMAT_BLEND_LAMP );	

	for(i = 0 ; i < 5 ; i++)
	{
		wsprintf(szBuff,"image\\Sinimage\\Effect\\IceVigo\\0%d.tga",i+1);
		MatIceVigo[i] = CreateTextureMaterial( szBuff , 0, 0, 0, SMMAT_BLEND_LAMP );
	}

	MatBossEffect[0]	= CreateTextureMaterial( "image\\Sinimage\\Effect\\BossMonster\\mamangjin1.tga" , 0	,0	,0	,SMMAT_BLEND_LAMP);
	MatBossEffect[1]	= CreateTextureMaterial( "image\\Sinimage\\Effect\\BossMonster\\mamangjin2.tga" , 0	,0	,0	,SMMAT_BLEND_LAMP);
	MatBossEffect[2]	= CreateTextureMaterial( "image\\Sinimage\\Effect\\BossMonster\\mamangjin3.tga" , 0	,0	,0	,SMMAT_BLEND_LAMP);

	MatParticle2[0]		= CreateTextureMaterial( "image\\Sinimage\\Effect\\Particle\\Par-1.tga"			, 0	,0	,0	,SMMAT_BLEND_LAMP);
	MatParticle2[1]		= CreateTextureMaterial( "image\\Sinimage\\Effect\\Particle\\Par-5.tga"			, 0	,0	,0	,SMMAT_BLEND_LAMP);

	MatTestParticle		= CreateTextureMaterial( "image\\Sinimage\\Effect\\Particle2\\TestParticle.tga" , 0	,0	,0	,SMMAT_BLEND_LAMP);

	MatParticleLine		= CreateTextureMaterial( "image\\Sinimage\\Effect\\Particle2\\raysoflight1.tga" , 0	,0	,0	,SMMAT_BLEND_LAMP);
	MatLightEffect		= CreateTextureMaterial( "image\\Sinimage\\Effect\\Skill\\SwordBlast\\bal.tga"	, 0	,0	,0	,SMMAT_BLEND_LAMP);
	MatSongPyeun		= CreateTextureMaterial( "image\\Sinimage\\Effect\\Particle2\\song.tga"			, 0	,0	,0	,SMMAT_BLEND_LAMP);
	return TRUE;
}
...
int sinSetEffect_IceVigo(Unit *pChar,int Time)
{
	Index = sinSearchEmptyIndex();                            
	cSinEffect2[Index].Flag					= 1; 
...
*								스턴 
*-----------------------------------------------------------------------------*/	
int sinSetEffect_Stun(Unit *pChar ,int Time)
{
...
int setBossMonsterEffect(Unit *pChar,int ProcessCount)
{
	Index = sinSearchEmptyIndex();                            
...
int sinDrawEffect2(int x, int y,int z,int ax, int ay, int az)
{
	...
	{  
		...
		{ 
			...
			{
				if(cSinEffect2[sinEffect_Zdepth2[i].Index-1].Color_A || cSinEffect2[sinEffect_Zdepth2[i].Index-1].Color_R || cSinEffect2[sinEffect_Zdepth2[i].Index-1].Color_G || cSinEffect2[sinEffect_Zdepth2[i].Index-1].Color_B )
				{
					...
					if(cSinEffect2[sinEffect_Zdepth2[i].Index-1].Color_A > 255)
						cSinEffect2[sinEffect_Zdepth2[i].Index-1].Color_A = 255;
	...
	{      
		if(cSinEffect2[i].Flag)
		{
			...
			if((cSinEffect2[i].Max_Time != SIN_EFFECT_NO_TIME && cSinEffect2[i].Time > cSinEffect2[i].Max_Time) ||
				(lpCurPlayer->psModelAnimation && lpCurPlayer->psModelAnimation->iType == ANIMATIONTYPE_Die) ||
				(lpCurPlayer->iLoadedMapIndex>=0 && STAGE_MAP[lpCurPlayer->iLoadedMapIndex]->iMapType == MAPTYPE_Town))
			...
			if(cSinEffect2[i].AniTime)
			{
				if((cSinEffect2[i].Time % cSinEffect2[i].AniTime)==0)
				{
					if(!cSinEffect2[i].AniReverseNum)
					{
						...
						if(cSinEffect2[i].AniCount >=cSinEffect2[i].AniMax)
							cSinEffect2[i].AniCount = 0;
						if(cSinEffect2[i].sinFace.MatNum)
...
int sinDrawTexture2(cSINEFFECT2 *pSinEffect2)
{
	if(pSinEffect2->sinFace.MatNum)
	{
		...
		smRender.SetCameraPosi(sinPosi2.x, sinPosi2.y, sinPosi2.z,sinAngle2.x, sinAngle2.y, sinAngle2.z);
...
int CheeseTestEffectImage()
{
	...
	while( !feof( fp ) )					//  feof: file end까지 읽어라 
	{
		...
		if ( !NextFlag && lstrcmp( sinDecode , "*이미지파일이름" )==0 ) {
			...
			if(FileFormatFlag == 1){
				cSinEffect2[Index].sinFace.MatNum = CreateTextureMaterial( szTempBuff , 0, 0, 0, SMMAT_BLEND_LAMP );
...
*						    기본 셋팅
*-----------------------------------------------------------------------------*/	
int sinEffectDefaultSet(int Index , int Kind , Unit *pChar , POINT3D *pPosi , int Y)
{
	cSinEffect2[Index].Flag = 1; 
...
{
	for(int i = 0; i < SIN_MAX_EFFECT2; i++)
	{
		if (&cSinEffect2[i])
		{
			if (cSinEffect2[i].CODE == Code)
			{
				if (cSinEffect2[i].pChar == pChar)
				{
					cSinEffect2[i].Max_Time += 50;
					return TRUE;
				}
			}
		}
