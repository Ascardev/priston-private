	...
	void Draw(POINT3D *pPosi, POINT3D *pAngle)
	{
		if (Face.MatNum)
		{
			...
			case ASSAFACE_BILLBOARD:
				if (!AssaAddFace2D(&Face, ZposiFlag, FACEY))
					return;
				break;
			case ASSAFACE_WORLD:
				if (!AssaAddFaceWorld(&Face, &Angle))
					return;
				break;
			case ASSAFACE_SCALE:
				if (!AssaAddFace2DLine(&Face, &ScalePosi, CenterFlag))
					return;
				break;
			case ASSAFACE_TRACE:
	...
	void Main()
	{
		...
		if (!NotFadeIn)
		{
			...
			else
			{
				...
				if (AlphaStartTime > MaxAlphaAmount)
					MaxAlphaAmount = 255;
...
*-----------------------------------------------------------------------------*/
//AssaEffect셋팅
int  SetAssaEffect(int MaxTime, char *FileName, Unit *pChar, POINT3D *pPosi, int AddHeight = 0, int BlendType = 0);
