...
void MoveAssaEffect(cASSAEFFECT *pEffect)
{
	...
	if (pEffect->MOVECODE == ASSA_MOVE_RANDOM_ROTATE)
	{
		...
		auto sPosition = GetMoveLocation(pEffect->MoveSpeed.x, pEffect->MoveSpeed.y, pEffect->MoveSpeed.z, 0, pEffect->Angle.y, 0);
		POINT3D tempGeoResult;
		tempGeoResult.x = sPosition.iX;
		tempGeoResult.y = sPosition.iY;
		tempGeoResult.z = sPosition.iZ;
		sPosition = GetMoveLocation(tempGeoResult.x, tempGeoResult.y, tempGeoResult.z, pEffect->Angle.x, 0, 0);
		if (pEffect->MOVECODESUB & ASSA_CHAR_POSI)
		{
			pEffect->Posi.x = pEffect->pChar->pX + sPosition.iX;
			pEffect->Posi.y = pEffect->pChar->pY + sPosition.iY + pEffect->AddHeight;
			pEffect->Posi.z = pEffect->pChar->pZ + sPosition.iZ;
		}
		else
		{
			pEffect->Posi.x = pEffect->CenterPosi.x + sPosition.iX;
			pEffect->Posi.y = pEffect->CenterPosi.y + sPosition.iY;
			pEffect->Posi.z = pEffect->CenterPosi.z + sPosition.iZ;
		}
		return;
	}
	...
	{
		if (!pEffect->ReSize.w)
			pEffect->ReSize.w = pEffect->Size.w / pEffect->Max_Time;
		if (!pEffect->ReSize.h)
			pEffect->ReSize.h = pEffect->Size.h / pEffect->Max_Time;
...
int  SetAssaEffect(int MaxTime, char *FileName, Unit *pChar, POINT3D *pPosi, int AddHeight, int BlendTpye)
{
	...
	for (int i = 0; i < len; i++)
	{
		if (TempFileName[i] == '.')
		{
			lstrcpy(FileFormat, &TempFileName[i + 1]);
			break;
		}
	}
	...
	for (int i = 0; i < cASSAEFFECT_MAX; i++)
	{
		if (!cAssaEffect[i])
		{
			if (lstrcmpi(FileName, "Bone") == 0)
			{
				cASSAFACE *pFace = new cASSAFACE;
				cAssaEffect[i] = pFace;
				cAssaEffect[i]->Bone = 1;
			}
			if (lstrcmpi(FileFormat, "bmp") == 0 || lstrcmpi(FileFormat, "tga") == 0 || lstrcmpi(FileFormat, "jpg") == 0 || lstrcmpi(FileFormat, "png") == 0)
			{
				cASSAFACE *pFace = new cASSAFACE;
				cAssaEffect[i] = pFace;
			...
			if (pChar)
			{
				...
				if (cAssaEffect[i]->pChar == lpCurPlayer)
					cAssaEffect[i]->MyPointerFlag = 1;
			}
			if (pPosi)
			{
...
int AssaSetAngle(cASSAEFFECT *pEffect, int AngleKind)
{
	...
	case ASSA_ANG_DESCHAR:
		...
		{
			auto sPosition = GetRadian3D(pEffect->pChar->pX, pEffect->pChar->pY, pEffect->pChar->pZ, pEffect->desChar->pX, pEffect->desChar->pY, pEffect->desChar->pZ);
			pEffect->Angle.x = sPosition.iX;
			pEffect->Angle.y = (-(sPosition.iY&ANGCLIP) + ANGLE_180)&ANGCLIP;
		}
		break;
	case ASSA_ANG_BONE:
		if (pEffect->pChar && pEffect->desChar)
...
int AssaSetStartPosi(cASSAEFFECT *pEffect)
{
	...
	auto sPosition = GetMoveLocation(pEffect->StartPosi.x, pEffect->StartPosi.y, pEffect->StartPosi.z, pEffect->Angle.x, pEffect->Angle.y, 0);
...
int AssaGetAniFileMat(cASSAEFFECT *pEffect)
{
	...
	for (i = 0; i < lstrlen(TempFileName); i++)
	{
		if (TempFileName[i + 1] == '.')
		{
			...
			lstrcpy(FirstFileName, TempFileName);
			lstrcpy(FileFormat, &TempFileName[i + 2]);
			break;
		}
	}
	for (i = 0; i < 10; i++)
...
int cASSAEFFECT::AssaReSetCharPointer()
{
	if (pChar)
	{
		...
		if (!pChar->bActive || sinGetLife() == 0)
			Time = Max_Time;

		if (CODE != ASSA_PARTICLE_UNIONCORE)
		{
