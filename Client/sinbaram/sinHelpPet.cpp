...
void cHELPPET::Load()
{
	MatPetImage[0] = CreateTextureMaterial("Image\\SinImage\\PetData\\Pet_terry-pose.tga", 0, 0, 0, SMMAT_BLEND_ALPHA);
	MatPetImage[1] = CreateTextureMaterial("Image\\SinImage\\PetData\\Pet_nepsys-pose.tga", 0, 0, 0, SMMAT_BLEND_ALPHA);
	MatPetImage[2] = CreateTextureMaterial("Image\\SinImage\\PetData\\Pet_io-pose.tga", 0, 0, 0, SMMAT_BLEND_ALPHA);
	MatPetImage[3] = CreateTextureMaterial("Image\\SinImage\\PetData\\Pet-mute-pose.tga", 0, 0, 0, SMMAT_BLEND_ALPHA);
}
...
void cHELPPET::Draw()
{
	if (PetKind && PetShow)
	{
		if (HelpMsgFlag)
		{
			...
			DrawSprite(522, 94, cSinSod2.lpbltr_Button_OK_G, 0, 0, 47, 12);

			if (PetMessageOkButtonFlag)
				DrawSprite(522, 94, cSinSod2.lpbltr_Button_OK, 0, 0, 47, 12);
		}
		DrawPetMessage();
	}
}
...
void cHELPPET::DrawText()
{
	if (HelpMsgFlag)
	{
		for (int i = 0; i < (int)PetMsg_Map[HelpMsgId].size(); i++)
			dsTextLineOut(0, 75, 22 + (i * 15), PetMsg_Map[HelpMsgId][i]->c_str(), PetMsg_Map[HelpMsgId][i]->length(), D3DCOLOR_XRGB(255, 255, 255));
	}
}
void cHELPPET::Close()
{
...
void cHELPPET::Main()
{
	...
	{
		HelpMsgTime++;
		if (522 < pCursorPos.x && 522 + 50 > pCursorPos.x && 94 < pCursorPos.y && 94 + 25 > pCursorPos.y)
		{
			if (PetMessageOkButtonFlag != 2)
...
int cHELPPET::PetRandomBattleMessage()
{
	...
	if (lpCurPlayer->psModelAnimation->iType == ANIMATIONTYPE_Attack)
	{
		...
		{
			if (sinChar->iLevel == 8 && cHelpPet.PetExp >= 90 && !ExpMessageFlag)
			{
				ExpMessageFlag = 1;
				...
				case CHARACTERCLASS_Assassin:
					PetMessageHelp("*Special_Message8_Assassin");
					break;
				case CHARACTERCLASS_Shaman:
					PetMessageHelp("*Special_Message8_Shaman");
					break;
				}

			}
			if (sinChar->iLevel == 9 && cHelpPet.PetExp >= 90 && ExpMessageFlag != 2)
			{
				ExpMessageFlag = 2;
...
{
	if (!PetKind || (sinChar->iLevel >= 10 && cHelpPet.PetShow == 0))
		return FALSE;
	switch (PetKind)
	{
	case 1:
