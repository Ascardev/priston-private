...
#define SIN_PARTICLE_TENUS			10019
...
void sinInitSkillEffect()
{
	...
	for (i = 0; i < 9; i++)
	{
		...
		MatFarina[i] = CreateTextureMaterial(szBuff, 0, 0, 0, SMMAT_BLEND_LAMP);
	}
	for (i = 0; i < 9; i++)
	{
		wsprintf(szBuff, "%s\\Silraphim\\Silraphim%d.tga", Path, i);
		MatSilraphim[i] = CreateTextureMaterial(szBuff, 0, 0, 0, SMMAT_BLEND_LAMP);
	}
	for (i = 0; i < 4; i++)
	{
		wsprintf(szBuff, "%s\\HolyBody\\H_BAP0%d.tga", Path, i);
		MatHolyBodyCross[i] = CreateTextureMaterial(szBuff, 0, 0, 0, SMMAT_BLEND_LAMP);
	}
	for (i = 0; i < 9; i++)
	{
		wsprintf(szBuff, "%s\\MultiSpark\\m_spark0%d.tga", Path, i);
		MatMultiSpark[i] = CreateTextureMaterial(szBuff, 0, 0, 0, SMMAT_BLEND_LAMP);
	}
	for (i = 0; i < 11; i++)
	{
		wsprintf(szBuff, "%s\\Agony\\Agony%d.tga", Path, i);
		MatAgony[i] = CreateTextureMaterial(szBuff, 0, 0, 0, SMMAT_BLEND_LAMP);
	}
	for (i = 0; i < 4; i++)
	{
		wsprintf(szBuff, "%s\\HolyMind\\H_MIND0%d.tga", Path, i);
		MatHolyMind[i] = CreateTextureMaterial(szBuff, 0, 0, 0, SMMAT_BLEND_LAMP);
	}
	wsprintf(szBuff, "%s\\HolyMind\\H_Particle.tga", Path, i);
	MatHolyMindParticle = CreateTextureMaterial(szBuff, 0, 0, 0, SMMAT_BLEND_LAMP);
	for (i = 0; i < 7; i++)
	{
		wsprintf(szBuff, "%s\\VigorSpear\\v_spear0%d.tga", Path, i);
		MatVigorSpear[i] = CreateTextureMaterial(szBuff, 0, 0, 0, SMMAT_BLEND_LAMP);
	}
	for (i = 0; i < 5; i++)
	{
		wsprintf(szBuff, "%s\\Zenith\\%dos.tga", Path, i + 1);
		MatZenith[i] = CreateTextureMaterial(szBuff, 0, 0, 0, SMMAT_BLEND_LAMP);
	}
	for (i = 0; i < 5; i++)
	{
		wsprintf(szBuff, "%s\\ThirdAnima\\%dos.tga", Path, i + 1);
		MatThirdAnima[i] = CreateTextureMaterial(szBuff, 0, 0, 0, SMMAT_BLEND_LAMP);
	}
	for (i = 0; i < 3; i++)
	{
		wsprintf(szBuff, "%s\\FireBolt\\FireBolt0%d.tga", Path, i + 1);
		MatFireBolt[i] = CreateTextureMaterial(szBuff, 0, 0, 0, SMMAT_BLEND_LAMP);
	}
	for (i = 0; i < 6; i++)
	{
		wsprintf(szBuff, "%s\\Bomb\\Bomb%d.tga", Path, i + 1);
		MatBomb6[i] = CreateTextureMaterial(szBuff, 0, 0, 0, SMMAT_BLEND_LAMP);
	}

	for (i = 0; i < 2; i++)
	{
		wsprintf(szBuff, "%s\\HolyBody\\HolyBody0%d.tga", Path, i + 1);
		MatHolyBody[i] = CreateTextureMaterial(szBuff, 0, 0, 0, SMMAT_BLEND_LAMP);
	}

	for (i = 0; i < 2; i++)
	{
		wsprintf(szBuff, "%s\\SwordBlast\\SwordBlast0%d.tga", Path, i + 1);
		MatSwordBlast[i] = CreateTextureMaterial(szBuff, 0, 0, 0, SMMAT_BLEND_LAMP);
	}
	for (i = 0; i < 3; i++)
	{
		wsprintf(szBuff, "%s\\ShieldStrike\\sd%d.tga", Path, i + 1);
		MatShieldStrike[i] = CreateTextureMaterial(szBuff, 0, 0, 0, SMMAT_BLEND_LAMP);
	}
...
void sinSkillEffectMove(cSINEFFECT2 *pEffect)
{
	...
	case SIN_PARTICLE_FARINA:
	{
		...
		auto sPosition = GetMoveLocation(0, 0, 32, pEffect->Angle.x, pEffect->Angle.y, 0);
		pEffect->sinFace.x += sPosition.iX;
		pEffect->sinFace.y += sPosition.iY;
		pEffect->sinFace.z += sPosition.iZ;
	}
		break;
	...
	{
		sinGetMoveLocation2(pEffect);
		auto sPosition = GetMoveLocation(0, 0, 256, pEffect->Angle.x, pEffect->Angle.y, 0);
		pEffect->sinFace.x += sPosition.iX;
		pEffect->sinFace.y += sPosition.iY;
		pEffect->sinFace.z += sPosition.iZ;
	}
	...
	case SKILL_SILRAPHIM:
		...
		Posi.y = pEffect->sinFace.y;
		Posi.z = pEffect->sinFace.z;
		sinEffect_SilraphimEffect(&Posi);
		pEffect->sinFace.x = 0;
		pEffect->sinFace.z = 0;
		pEffect->sinFace.y = 0;
		break;
	case SIN_PARTICLE_SILRAPHIM:
		//pEffect->Posi.x = pEffect->sinFace.x + pEffect->MoveSpeed.x;
		//pEffect->Posi.y = pEffect->sinFace.y + pEffect->MoveSpeed.y;
		//pEffect->Posi.z = pEffect->sinFace.z + pEffect->MoveSpeed.z;
	...
	case SKILL_V_TENUS:
		...
		Posi.y = pEffect->sinFace.y;
		Posi.z = pEffect->sinFace.z;
		sinEffect_TenusEffect(&Posi);
		pEffect->sinFace.x = 0;
		pEffect->sinFace.z = 0;
		pEffect->sinFace.y = 0;
		break;
	case SIN_PARTICLE_TENUS:
		pEffect->sinFace.x += pEffect->MoveSpeed.x;
		pEffect->sinFace.z += pEffect->MoveSpeed.z;
		pEffect->sinFace.y += pEffect->MoveSpeed.y;
		if (pEffect->ActionTime[0] <= pEffect->Time)
		{
			pEffect->sinFace.MatNum = MatAgony[9];
			pEffect->sinFace.y += (pEffect->MoveSpeed.y * 3);
		}
		break;
	...
	{
		sinGetMoveLocation2(pEffect);
		auto sPosition = GetMoveLocation(0, 0, 64, pEffect->Angle.x, pEffect->Angle.y, 0);
		pEffect->sinFace.x += sPosition.iX;
		pEffect->sinFace.y += sPosition.iY;
		pEffect->sinFace.z += sPosition.iZ;
	}
	...
	case SIN_PARTICLE_HOLYMIND2:
		...
		if (!pEffect->pChar->psModelAnimation || pEffect->pChar->psModelAnimation->iType == ANIMATIONTYPE_Die)
			memset(pEffect, 0, sizeof(cSINEFFECT2));
		break;
	case SKILL_ZENITH:
	...
	{
		sinGetMoveLocation2(pEffect);
		auto sPosition = GetMoveLocation(0, 0, 512, pEffect->Angle.x, pEffect->Angle.y, 0);
		pEffect->sinFace.x += sPosition.iX;
		pEffect->sinFace.y += sPosition.iY;
		pEffect->sinFace.z += sPosition.iZ;
	}
...
int sinEffect_Sword_Blast(Unit *pChar, Unit *desChar)
{
	...
	{
		//대상의 각을 구해서 적용한다   
		auto sPosition = GetRadian3D(pChar->pX, pChar->pY, pChar->pZ, cSinEffect2[Index2].DesChar->pX, cSinEffect2[Index2].DesChar->pY, cSinEffect2[Index2].DesChar->pZ);
		cSinEffect2[Index2].Angle.x = sPosition.iX;
		cSinEffect2[Index2].Angle.y = (-(sPosition.iY&ANGCLIP) + ANGLE_180)&ANGCLIP;
...
int sinEffect_Sword_Blast_Particle(POINT3D *pPosi)
{
	...
	for (int i = 0; i < 15; i++) {
		...
		for (int j = 0; j < 2; j++) {
			...
			auto sPosition = GetMoveLocation(0, 0, 256, cSinEffect2[Index2].Angle.x, cSinEffect2[Index2].Angle.y, 0);
			cSinEffect2[Index2].sinFace.x += sPosition.iX;
			cSinEffect2[Index2].sinFace.y += sPosition.iY;
			cSinEffect2[Index2].sinFace.z += sPosition.iZ;
		}
	}
	return TRUE;
...
int sinEffect_MultiSpark_Particle2(POINT3D *pPosi)
{
	...
	for (int i = 0; i < 1; i++) {
		...
		cSinEffect2[Index2].MoveSpeed.z = 64;
		sinGetMoveLocation2(&cSinEffect2[Index2]);
		auto sPosition = GetMoveLocation(0, 0, 64, cSinEffect2[Index2].Angle.x, cSinEffect2[Index2].Angle.y, 0);
		cSinEffect2[Index2].sinFace.x += sPosition.iX;
		cSinEffect2[Index2].sinFace.y += sPosition.iY;
		cSinEffect2[Index2].sinFace.z += sPosition.iZ;
...
int sinEffect_MultiSpark(Unit *pChar, Unit *desChar, int Num)
{
	...
	for (int i = 0; i < Num; i++) {
		...
		if (cSinEffect2[Index2].DesChar) 
		{
			auto sPosition = GetRadian3D(pChar->pX, pChar->pY, pChar->pZ, cSinEffect2[Index2].DesChar->pX, cSinEffect2[Index2].DesChar->pY, cSinEffect2[Index2].DesChar->pZ);
			cSinEffect2[Index2].Angle.x = sPosition.iX;
			cSinEffect2[Index2].Angle.y = sPosition.iY;
		}
		...
		else 
		{
			if (((i + 1) % 2) == 0)
				...
				Temp2 = -1.0f;
			auto sPosition = GetMoveLocation((int)((fONE*(10 + 1 + (i * 12)))*Temp2), 0, 0, cSinEffect2[Index2].Angle.x, cSinEffect2[Index2].Angle.y, cSinEffect2[Index2].Angle.z);
			cSinEffect2[Index2].sinFace.x += sPosition.iX;
			cSinEffect2[Index2].sinFace.y += sPosition.iY;
			cSinEffect2[Index2].sinFace.z += sPosition.iZ;
...
int sinEffect_HolyMind(Unit *pChar, int Time)
{
	...
	/////////// 생속성 적용 
	int TempResi = 100 - pChar->sCharacterData.sElementalDef[sITEMINFO_BIONIC];
...
int sinEffect_SilraphimEffect(POINT3D *pPosi)
{
	...
	for (int i = 0; i < 10; i++)
	{
		Index2 = sinSearchEmptyIndex();
		sinEffectDefaultSet(Index2, SIN_EFFECT_FACE, 0, pPosi, 0);
		cSinEffect2[Index2].sinFace.MatNum = MatSilraphim[2];
		Size = rand() % 1000 + 500;
		cSinEffect2[Index2].sinFace.height = Size;
		cSinEffect2[Index2].sinFace.width = Size;
		cSinEffect2[Index2].Max_Time = 20;
		...
		cSinEffect2[Index2].MoveSpeed.y = (cSinEffect2[Index2].sinFace.y - pPosi->y) / 40;
...
int sinEffect_TenusEffect(POINT3D *pPosi)
{
	...
	Index2 = sinSearchEmptyIndex();
	sinEffectDefaultSet(Index2, SIN_EFFECT_FACE, 0, pPosi, 0);
	cSinEffect2[Index2].sinFace.MatNum = MatAgony[9];
	...
	cSinEffect2[Index2].MoveSpeed.x = rand() % 3;
	cSinEffect2[Index2].MoveSpeed.z = rand() % 3;
	cSinEffect2[Index2].MoveSpeed.y = rand() % 3;
...
int sinEffect_Zenith(Unit *pChar, int Time, int Flag)
{
	if (!pChar)
		return FALSE;
	...
	if (AddCodeEffectTime(pChar, SKILL_ZENITH))
		return FALSE;
	else
	{
		for (int i = 0; i < 5; i++)
...
int sinEffect_Shield_Strike(POINT3D *pPosi)
{
	...
	cSinEffect2[Index2].Size.x += TempSize;
	cSinEffect2[Index2].Size.y += TempSize;
	cSinEffect2[Index2].Angle.y = ANGLE_270;//ANGLE_270;
	...
	for (int i = 0; i < 5; i++)
	{
		...
		sinGetMoveLocation2(&cSinEffect2[Index2]);
		auto sPosition = GetMoveLocation(0, 0, 128 * i, cSinEffect2[Index2].Angle.x, cSinEffect2[Index2].Angle.y, 0);
		cSinEffect2[Index2].sinFace.x += sPosition.iX;
		cSinEffect2[Index2].sinFace.y += sPosition.iY;
		cSinEffect2[Index2].sinFace.z += sPosition.iZ;
		cSinEffect2[Index2].SizeDecreTime = 1;
...
int sinEffect_StartMagic2(POINT3D *pPosi, int CharFlag)
{
	...
	case 1:
		...
		cAssaEffect[Assa]->Size.w = 15900;
		Assa = SetAssaEffect(160, "star05C_03.bmp", 0, pPosi, 1500);
		cAssaEffect[Assa]->FACE_TYPE = ASSAFACE_WORLD;
		cAssaEffect[Assa]->Size.w = 18600;
...
int sinEffect_Silraphim(Unit *pChar)
{
	...
	cSinEffect2[Index2].Max_Time = 70;
	cSinEffect2[Index2].sinFace.width = 2000;
	cSinEffect2[Index2].sinFace.height = 2000;
	cSinEffect2[Index2].sinFace.MatNum = MatAgony[0];
	cSinEffect2[Index2].AniMax = 4;
	cSinEffect2[Index2].AniTime = 10;
	cSinEffect2[Index2].AniCount = 1;
	cSinEffect2[Index2].lpMatAni = MatHolyMind;
	cSinEffect2[Index2].RotateDistance.z = 24 * fONE;
...
int sinEffect_Tenus(Unit *pChar)
{
	...
	cSinEffect2[Index2].sinFace.width = Size;
	cSinEffect2[Index2].Max_Time = 70;
	cSinEffect2[Index2].CODE = SKILL_V_TENUS;
	cSinEffect2[Index2].AniCount = 1;
	cSinEffect2[Index2].AniMax = 4;
	cSinEffect2[Index2].AniTime = 10;
	cSinEffect2[Index2].lpMatAni = MatHolyMind;
...
int sinEffect_Anima(Unit *pChar, int Time, int Flag)
{
	...
	if (AddCodeEffectTime(pChar, SKILL_T_ANIMA))
		return FALSE;
	else
	{
		for (int i = 0; i < 5; i++)
		{
			...
			cSinEffect2[Index2].pChar = pChar;
			cSinEffect2[Index2].CODE = SKILL_T_ANIMA;
			cSinEffect2[Index2].Max_Time = Time * 70;
			cSinEffect2[Index2].Index = i + 1;
