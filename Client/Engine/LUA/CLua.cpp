...
CLua::CLua()
{
	...
	m_pLuaState->state_ptr = luaL_newstate();

	lua_checkstack(m_pLuaState->state_ptr, 100);

	luaL_openlibs(m_pLuaState->state_ptr);

	CopyMemory(m_pLuaState->self_ptr, m_pLuaState->state_ptr, sizeof(lua_State));

	m_pLuaState->lua = this;
...
CLua::~CLua()
{
	m_pLuaState->lua = NULL;

	lua_close(m_pLuaState->state_ptr);
...
{
	lua_register(m_pLuaState->get_ptr(), strName.c_str(), (lua_CFunction)pFunction);
...
void CLua::DoFile(string strFilePath, BOOL bIgnoreEncrypt)
{
	if (bIgnoreEncrypt)
	{
		luaL_loadfile(m_pLuaState->get_ptr(), strFilePath.c_str());
		lua_pcall(m_pLuaState->get_ptr(), 0, LUA_MULTRET, 0);
	}
	...
	if (strFilePath[strFilePath.size() - 1] == 'c')
	{
		...
		if (fp)
		{
			...
			luaL_loadbuffer(m_pLuaState->get_ptr(), pBuffer, lSize, "lua");
			lua_pcall(m_pLuaState->get_ptr(), 0, LUA_MULTRET, 0);
	...
	else
	{
		...
		luaL_loadfile(m_pLuaState->get_ptr(), strFilePath.c_str());
		lua_dump(m_pLuaState->get_ptr(), LuaWriter, &sLuaCompiled, 1);
		lua_pop(m_pLuaState->get_ptr(), 1);
