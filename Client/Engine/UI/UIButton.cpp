...
namespace UI
{
	...
	BOOL Button::OnMouseClick(class INPUTS::Mouse *pcMouse,int iX,int iY,int iWidth,int iHeight,int iSourceX,int iSourceY)
	{
		if (!IsOpen() || m_bDisable || IsLinkObject())
			return FALSE;
		...
		if ((psPosition->iX >= iRenderX) && (psPosition->iX <= (iRenderX + iRenderWidth - iSourceX)))
		{
			...
			if ((psPosition->iY >= iRenderY) && (psPosition->iY <= (iRenderY + iRenderHeight - iSourceY)))
			{
				...
				if (m_iEventID && m_eMouseEvent == pcMouse->GetEvent() || m_bHolding)
				{
					if (m_bCanHold)
						m_bHolding = TRUE;
	...
	void Button::OnMouseMove(class INPUTS::Mouse *pcMouse,int iX,int iY,int iWidth,int iHeight,int iSourceX,int iSourceY)
	{
		if (!IsOpen() || m_bDisable || IsLinkObject())
			return;
