...
namespace UI
{
	...
	public:
		...
		//Event Handles
		virtual BOOL OnMouseClick(class INPUTS::Mouse *pcMouse,int iX,int iY,int iWidth,int iHeight,int iSourceX,int iSourceY) {return FALSE;}
		virtual void OnMouseMove(class INPUTS::Mouse *pcMouse,int iX,int iY,int iWidth,int iHeight,int iSourceX,int iSourceY) {}
		virtual BOOL OnMouseScroll(class INPUTS::Mouse *pcMouse,int iX,int iY,int iWidth,int iHeight,int iSourceX,int iSourceY) {return FALSE;}
		...
		BOOL IsLinkObject() { return m_bLinkObject; }

		void OnLinkObject() { m_bLinkObject = TRUE; }
		void UnLinkObject() { m_bLinkObject = FALSE; }

		virtual BOOL IsParentChild() { return TRUE; }
	protected:
		int m_iID = -1;
		BOOL m_bShow = TRUE;
		BOOL m_bLinkObject = FALSE;
		BOOL m_bDebug = FALSE;
		Rectangle2D m_rRect;
