...
namespace UI
{
	...
	BOOL Scroll::OnMouseClick(class INPUTS::Mouse *pcMouse,int iX,int iY,int iWidth,int iHeight,int iSourceX,int iSourceY)
	{
	...
	BOOL Scroll::OnMouseScroll(class INPUTS::Mouse *pcMouse,int iX,int iY,int iWidth,int iHeight,int iSourceX,int iSourceY)
	{
		if (!IsOpen() || IsLinkObject())
			return FALSE;
	...
	void Scroll::OnMouseMove(class INPUTS::Mouse *pcMouse,int iX,int iY,int iWidth,int iHeight,int iSourceX,int iSourceY)
	{
		if (!IsOpen() || !m_bScrollable || IsLinkObject())
			return;
	...
	int Scroll::GetPercent()
	{
		...
		if (m_bVertical)
			...
			fRet = (((float)m_pScroll->iX * fMinMax) / ((float)GetWidth() - (float)(m_iWidthScroll >> 1))) + m_fMinScroll;
