...
snFIND_EFFECT	snFindEffects[snFIND_EFFECT_MAX] =
{
	...
	{ "wav\\Effects\\monster\\VampiricMachine\\",MONSTEREFFECTID_VAMPRICMACHINE},
	...
	{ "wav\\Effects\\monster\\Draxos\\",		MONSTEREFFECTID_Draxos },
	...
	///////////////////////// °ø¼ºÀü //////////////////////////////
	{ "wav\\Effects\\npc\\castledoor\\",	MONSTEREFFECTID_CASTLE_DOOR },
...
{
	{	"attack"		,		ANIMATIONTYPE_Attack	},
	{	"damage"		,		ANIMATIONTYPE_Flinch	},
	{	"dead"			,		ANIMATIONTYPE_Die	},
	{	"neutral"		,		ANIMATIONTYPE_Idle	},
	{	"move"			,		ANIMATIONTYPE_Walking	},
	{	"disappear"		,		ANIMATIONTYPE_Special	},
	{	"change"		,		ANIMATIONTYPE_Special	},
	{	"skill"			,		ANIMATIONTYPE_Skill	},
	{	"hammer"		,		ANIMATIONTYPE_Hammer	}
};
...
int PlayFootStep(Unit *lpChar)
{
	...
	if (lpChar->iLoadedMapIndex >= 0) {
		floor = STAGE_MAP[lpChar->iLoadedMapIndex]->iMapType;
...
int	CharPlaySound(Unit *lpChar)
{
	...
	if (lpChar->sCharacterData.iType == CHARACTERTYPE_Player)
		dwCharSoundCode = lpChar->sCharacterData.iClass;
	else
		dwCharSoundCode = lpChar->sCharacterData.iMonsterEffectID;

	if (!lpChar->psModelAnimation)
		return FALSE;

	dwMotionCode = lpChar->psModelAnimation->iType;

	if (dwMotionCode == ANIMATIONTYPE_Revive)
	{
		esPlaySound(6, vol);
		return TRUE;
	}
	if (dwMotionCode == ANIMATIONTYPE_Walking || dwMotionCode == ANIMATIONTYPE_Running)
	{
		if (vol < 300)
			return TRUE;

		dwMotionCode = ANIMATIONTYPE_Walking;
	}

	if (lpChar->sCharacterData.sGlow == MONSTER_CLASS_HAMMER && dwMotionCode == ANIMATIONTYPE_Attack)
		dwMotionCode = ANIMATIONTYPE_Hammer;
	...
	if (CodeBuffCnt)
	{
		int	Feq;
		Feq = 2205;
		if (lpChar->sCharacterData.sSize && lpChar->sCharacterData.iMonsterType != MONSTERTYPE_Friendly)
		{
			if (lpChar->sCharacterData.sSize < fONE)
				Feq = 3600;
			if (lpChar->sCharacterData.sSize > fONE * 2)
				Feq = 1400;
		}
...
//ÎäÆ÷ÉùÒô
int	WeaponPlaySound(Unit *lpChar)
{
	int cnt;
	...
	ItemData * pItemData = ITEMHANDLE->GetItemDataByID(lpChar->dwActionItemCode);

	if (pItemData)
	{
		HandType = pItemData->iItemSlotFlag;
	}
	else
		return FALSE;
	...
	{
		if (HandType == ITEMSLOTFLAG_RightHand)
			dwCode = 12;
		else
			dwCode = 13;
	}
	else
	{
		...
		case sinWS1:
			...
			break;
		case sinWM1:
			if (lpChar->sCharacterData.iClass == CHARACTERCLASS_Priestess || lpChar->sCharacterData.iClass == CHARACTERCLASS_Magician)
			{
		...
		case sinWD1:
			...
			break;
		case sinWN1:
			if (lpChar->sCharacterData.iClass == CHARACTERCLASS_Shaman)
			{
				if (lpChar->WeaponEffect)
					return TRUE;
				else
...
{
	if (SETTINGHANDLE->Get().bNoMusic == FALSE)
	{
		if (!dwFadeStartTime)
		{
			if (NextMusicNum == MusicNum)
				return FALSE;
			NextMusicNum = MusicNum;
	...
	if (dwFadeStartTime)
	{
		...
		if (vol <= 0)
		{
			if (NextMusicNum >= 0)
			{
				...
				PlayBGM();
				SetVolumeBGM(GetVolumeBGM(SETTINGHANDLE->Get().iMusicVolume));
			}
			else
				StopBGM();
		...
		else
		{
			...
			{
				if (vol > GetVolumeBGM(SETTINGHANDLE->Get().iMusicVolume))
					vol = GetVolumeBGM(SETTINGHANDLE->Get().iMusicVolume);
...
{
	g_TalkSoundSum = GetCodebyName(lpSrcMessage);
