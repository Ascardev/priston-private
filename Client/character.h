...
struct	rsBLESS_CASTLE {
	...
	AttackDamageData	*lpAttackDamageList;
	char	bCrystalSolderCount[4];
	char	bCrystalTowerCount[4];
	Unit	*lpChar_ValhallaTower;
...
public:
	...
	int		uUnitInfoSize;				//Àü¼ÛÇÒ µ¥ÀÌÅ¸ ´ë±â¿ë ¹öÆÛ Å©±â

	//Åë½Å °ü·Ã
	int		ServerCode;				//ÇØ´ç Ä³¸¯ÅÍ ¼­¹ö ÄÚµå

	PlayBuffer	sPlayBuff[PLAYBUFF_SIZE];	//ÇÃ·¹ÀÌ ÁøÇà »óÈ²À» ¹öÆÛ¿¡ ÀúÀåÇÏ¿© Åë½Å½Ã ¹öÆÛ¸µÇÔ
	int			iPlayBuffCount;					//PlayBuff ÀÇ Ä«¿îÅÍ
	...
	UnitInfo	sUnitInfo;		//¸ó½ºÅÍ °ü·Ã ±¸Á¶
	...
	int				iStunTimeLeft;		//Áß¸³ »óÅÂ Ä«¿îÅÍ
	...
	int				AttackSkillRange;		//¸ó½ºÅÍ°¡ ½ºÅ³ ¹üÀ§ °ø°Ý »ç¿ë
	...
	DWORD			dwUpdateCharInfoTime;	//¾÷µ¥ÀÌÆ® ½Ãµµ ½Ã°£
	...
	int CheckShootingTest_LastAttackTime(Unit *lpChar);
	...
	int MakePlayBuffFromRecvData(PlayBuffer *StartBuff , PlayBuffer *EndBuff , int len );
	void HandleUnitDataBuffer(char * pBuffer, int iDelayTime, int iSleep);
	int SetTransEffectItems(PacketBuffData *lpEffectItem);
	// ±ðÈË¿´µÄÐ§¹û
	int SetTransEffectElement(PacketBuffData *lpEffectItem);
	int SetTransEffectWeapon(PacketBuffData *lpEffectItem);
	...
	int BeginSkill( int SkilCode , int Level , Unit *lpTarChar , int x, int y, int z );
	int EventSkill();
...
public:
	...
	int SetPosi( int x, int y, int z );				//À§Ä¡ ¼³Á¤
	int SetTransItem( TRANS_ITEM *lpTransItem );	//¼ö½ÅµÈ ¾ÆÀÌÅÛ ¼³Á¤

	int SetPattern( smPAT3D *pat );					//ÆÐÅÏ¼³Á¤
	int SetPosi( int x, int y, int z , int angX, int angY, int angZ );
	int Draw();					//¾ÆÀÌÅÛ ±×¸®±â
...
extern DWORD BabelPlayField;

extern POINT3D	HardCoreEventOpenPos[7][3];
