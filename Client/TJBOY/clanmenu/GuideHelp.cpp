		...
		if (GuideHelpButton[0])
		{
			...
			case 23:
				if (!bLButtonDown)
				{
					nPos = (i + nScl_Bar - 12);
					if (sinChar->iClass < 5)
						CheckLevel(g_LevelLabel[nLevelHelpCount - nPos - 1], sinChar->iClass);
					else
						CheckLevel(g_MorianLabel[nLevelHelpCount - nPos - 1], sinChar->iClass);
...
{
	nQuetion_Icon = CreateTextureMaterial("image\\Sinimage\\Help\\help_icon01.tga", 0, 0, 64, SMMAT_BLEND_ALPHA);
	nQuetion_Icon1 = CreateTextureMaterial("image\\Sinimage\\Help\\help_icon01_.tga", 0, 0, 64, SMMAT_BLEND_ALPHA);
	nHelp = CreateTextureMaterial("image\\Sinimage\\Help\\help_icon01t.tga", 0, 0, 64, SMMAT_BLEND_ALPHA);
	nHelpSee = CreateTextureMaterial("image\\Sinimage\\Help\\help_icon00.tga", 0, 0, 64, SMMAT_BLEND_ALPHA);
	nQuideHelp = CreateTextureMaterial("image\\Sinimage\\Help\\help_icon02t.tga", 0, 0, 64, SMMAT_BLEND_ALPHA);
	nDuru = CreateTextureMaterial("image\\Sinimage\\Help\\help_icon02.tga", 0, 0, 64, SMMAT_BLEND_ALPHA);
	nDuru1 = CreateTextureMaterial("image\\Sinimage\\Help\\help_icon02_.tga", 0, 0, 64, SMMAT_BLEND_ALPHA);
	nLine = CreateTextureMaterial("image\\Sinimage\\Help\\Box_Line.tga", 0, 0, 0, SMMAT_BLEND_ALPHA);
	...
	nScl_icon_tj = CreateTextureMaterial("image\\Sinimage\\Help\\scl_icon.tga", 0, 0, 0, SMMAT_BLEND_ALPHA);
	nScl_Bar_tj = CreateTextureMaterial("image\\Sinimage\\Help\\scl_bar.tga", 0, 0, 0, SMMAT_BLEND_ALPHA);
	nGuideList = CreateTextureMaterial("image\\Sinimage\\Help\\help_Glist.tga", 0, 0, 64, SMMAT_BLEND_ALPHA);
...
void CGuideHelp::menuInit(int num)
{
	...
	case GUIDEHELP_LIST:
		...
		nScl_Bar = 0;
		tscr_Guide.Init(1);
		tscr_Guide.Init(menuPos[nScrollPos].top, menuPos[nScrollPos].top + 256, nLevelHelpCount, LEVELHELP_COUNT_MAX);
	...
	case GUIDEHELP_CONTENT:
		...
		if (nHeight_Count > GUIDEHLEP_CONTENT_HEIGHT_MAX)
		{
			...
			nScl_Bar = 0;
			tscr_Guide.Init(1);
			tscr_Guide.Init(menuPos[nScrollPos].top, menuPos[nScrollPos].top + 256, nHeight_Count, GUIDEHLEP_CONTENT_HEIGHT_MAX);
