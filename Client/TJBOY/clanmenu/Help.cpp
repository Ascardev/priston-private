...
void CHelp::Load()
{
	nHelp0 = CreateTextureMaterial( "image\\Sinimage\\Help\\help_icon03.tga", 0, 0, 64, SMMAT_BLEND_ALPHA );
	nHelp1 = CreateTextureMaterial( "image\\Sinimage\\Help\\help_icon04.tga", 0, 0, 64, SMMAT_BLEND_ALPHA );

	nBox1  = CreateTextureMaterial( "image\\Sinimage\\Help\\GH-Box01.tga", 0, 0, 32, SMMAT_BLEND_ALPHA );
	nBox2  = CreateTextureMaterial( "image\\Sinimage\\Help\\GH-Box02.tga", 0, 0, 16, SMMAT_BLEND_ALPHA );
	nBox3  = CreateTextureMaterial( "image\\Sinimage\\Help\\GH-Box03.tga", 0, 0, 32, SMMAT_BLEND_ALPHA );
	nBox4  = CreateTextureMaterial( "image\\Sinimage\\Help\\GH-Box04.tga", 0, 0, 32, SMMAT_BLEND_ALPHA );
	nBox5  = CreateTextureMaterial( "image\\Sinimage\\Help\\GH-Box05.tga", 0, 0, 16, SMMAT_BLEND_ALPHA );
	nBox6  = CreateTextureMaterial( "image\\Sinimage\\Help\\GH-Box06.tga", 0, 0, 32, SMMAT_BLEND_ALPHA );
	nBox7  = CreateTextureMaterial( "image\\Sinimage\\Help\\GH-Box07.tga", 0, 0, 32, SMMAT_BLEND_ALPHA );
	nBox8  = CreateTextureMaterial( "image\\Sinimage\\Help\\GH-Box08.tga", 0, 0, 16, SMMAT_BLEND_ALPHA );
	nBox9  = CreateTextureMaterial( "image\\Sinimage\\Help\\GH-Box09.tga", 0, 0, 32, SMMAT_BLEND_ALPHA );
	...
	nGuideHelp = CreateTextureMaterial(  "image\\Sinimage\\Help\\help_icon02t.tga", 0, 0, 64, SMMAT_BLEND_ALPHA );
...
{	
	...
	{		
		...
		case LEVELUP_HELP_CONTENT:
			...
			if((sinChar->iLevel != 9) && (sinChar->iLevel != 39))
			...
			if(menuMouseOver[1]>0) 
				DrawSprite(HelpMenu[1].left,HelpMenu[1].top, hClose[1],  0, 0, 32, 16, 1);
			else	
				DrawSprite(HelpMenu[1].left,HelpMenu[1].top, hClose[0],  0, 0, 32, 16, 1);

			if(m_bIsContentOver)
			{
				if(menuMouseOver[0] > 0)
					Draw_Help_Text2(">>Help...", HelpMenu[0].left, HelpMenu[0].top);
				else
					Draw_Help_Text3(">>Help...", HelpMenu[0].left, HelpMenu[0].top);
			}
			break;
		}
	}
...
void CHelp::LoadGuideImage(char* file)
{
	ClearImage();
	//GuideImage->Init(1);
	nGuideImageNum = CreateTextureMaterial(file,0, 0, 128, SMMAT_BLEND_ALPHA );
	ReadTextures();
}
...
jmp1:
			if(sw==2)
			{
				dsTextLineOut(0, x+1, y+1, strbuf, lstrlen(strbuf),D3DCOLOR_XRGB(50, 50, 50));
				dsTextLineOut(0, x, y, strbuf, lstrlen(strbuf),D3DCOLOR_XRGB(255, 240, 200));
			}
			else
				dsTextLineOut(0, x, y, strbuf, lstrlen(strbuf),D3DCOLOR_XRGB(255, 240, 200));
...
int CHelp::dsDrawTexImageFloat(int Mat , float x, float y, float w, float h , int Transp , int specular)
{
	...
	if (smMaterialGroup->smMaterial[Mat].smTexture[0] != NULL && smMaterialGroup->smMaterial[Mat].smTexture[0]->m_pTexture != NULL)
		psDraw2DImage(smMaterialGroup->smMaterial[Mat].smTexture[0]->m_pTexture, tlVertex);
...
jmp1:
			...
			{
				dsTextLineOut(0, x+1, y+1, strbuf, lstrlen(strbuf),D3DCOLOR_XRGB(100, 100, 100));
				dsTextLineOut(0, x, y, strbuf, lstrlen(strbuf),D3DCOLOR_XRGB(255, 200, 100));
			}
			else
				dsTextLineOut(0, x, y, strbuf, lstrlen(strbuf),D3DCOLOR_XRGB(255, 200, 100));
...
{	
	...
	{
		if(g_LevelLabel[i] == sinChar->iLevel)
		{
			bisEqual = TRUE;
			SaveLevel = i;
			break;
	...
	g_GuideHelp.CheckLevel(g_LevelLabel[SaveLevel],sinChar->iClass);
...
{
	if ((Code&sinITEM_MASK2)==sinWA1 || (Code&sinITEM_MASK2)==sinWC1 || (Code&sinITEM_MASK2)==sinWH1 || (Code&sinITEM_MASK2)==sinWM1 || (Code&sinITEM_MASK2)==sinWP1 || (Code&sinITEM_MASK2)==sinWS1 || (Code&sinITEM_MASK2)==sinWS2 || (Code&sinITEM_MASK2)==sinWT1 || (Code&sinITEM_MASK2)==sinWD1 || (Code&sinITEM_MASK2)==sinWN1)
