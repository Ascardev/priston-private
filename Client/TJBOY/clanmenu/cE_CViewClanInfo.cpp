...
CViewClanInfo::~CViewClanInfo()
{
	...
	{
		DELET(ClanInfo[i].hClanMark);
		DELET(ClanInfo[i].hClanMark32);
	}
}
...
int CViewClanInfo::readFILE_ClanInfo(DWORD clanNum)
{
	...
	if (nClanMarklen == 9)
	{
		...
		lstrcpy(ClanUnique, &szClanNum[3]);
	}
	else
	{
		GServer[0] = szClanNum[0];
		GServer[1] = szClanNum[1];
		lstrcpy(ClanUnique, &szClanNum[4]);
	}
	...
	HANDLE hFile;
	DWORD dwRead;
	hFile = CreateFile(szTemp, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
	...
	DELET(ClanInfo[nCount].hClanMark);
	DELET(ClanInfo[nCount].hClanMark32);
...
int CViewClanInfo::Search_Memory(char *clanNum)
{
	...
	if (l >= 10)
	{
		...
		lstrcpy(uni_cl.cl.cNum, &clanNum[4]);
	}
	else
	{
		uni_cl.cl.gn[0] = clanNum[0];
		...
		lstrcpy(uni_cl.cl.cNum, &clanNum[3]);
	}
	if (l >= 10)
	{
	...
	for (int i = 0; i < CLAN_MAX; i++, pCL++)
	{
		...
		if (l >= 10)
		{
			...
			lstrcpy(uni_cl.cl.cNum, &pCL->ClanInfoHeader.ClanMark_Num[4]);
		}
		else
		{
			uni_cl.cl.gn[0] = pCL->ClanInfoHeader.ClanMark_Num[0];
			...
			lstrcpy(uni_cl.cl.cNum, &pCL->ClanInfoHeader.ClanMark_Num[3]);
		}
		if (l >= 10)
		{
			buf[0] = uni_cl.cl.gn[0];
...
int CViewClanInfo::Search_Files(char *clanNum)
{
	...
	{
		GServer[0] = clanNum[0];
		lstrcpy(szClanNum, &clanNum[3]);
	}
	else
	{
		GServer[0] = clanNum[0];
		GServer[1] = clanNum[1];
		lstrcpy(szClanNum, &clanNum[4]);
	}
	...
	if (hFile != INVALID_HANDLE_VALUE)
	{
		...
		else
		{
			...
			auto ld = LoadDibSurfaceOffscreen(szTemp);
			if (ld != NULL)
			{
				DELET(ld);
...
int CViewClanInfo::SearchClan(DWORD dwClanNum)
{
	...
	wsprintf(ClanMarkNum, "%u", dwClanNum);
	...
	if (nStr >= 10)
	{
		...
		lstrcpy(szNum, &ClanMarkNum[4]);
	}
	else
	{
		GServer[0] = ClanMarkNum[0];
		lstrcpy(szNum, &ClanMarkNum[3]);
	}
...
{
	char szTemp[256];
	wsprintf(szTemp, "%s\\%s\\%s_%s_16.%s", CLANINFO_DIR, szConnServerName, ci->ClanInfoHeader.Gserver, ci->ClanInfoHeader.UniqueClaClanInfo_nNum, SAVE_EXE);
...
int CViewClanInfo::DownLoadClanInfo(int num, DWORD dwClanNum)
{
	...
	int i;
	char MarkNum[64];
	STRINGFORMAT(MarkNum, "%u", dwClanNum);
	...
	if (num < 0)
	{
		for (i = 0; i < CLAN_MAX; i++, pCL++)
		{
			if ((pCL->nStatus == 0) || pCL->nStatus == 3)
			{
				nNum = i;
				pCL->nStatus = 1;
				lstrcpy(pCL->ClanInfoHeader.ClanMark_Num, MarkNum);
				break;
			}
		}
...
char* CViewClanInfo::AddClanInfo(char *marknum, char *szInfo, CLANINFO* ci)
{
	...
	lstrcpy(szClanReport, ci->ClanInfoHeader.ClanReport);
	lstrcpy(ci->ClanInfoHeader.ClanMark_Num, marknum);
	...
	if (nClanMarklen == 9)
	{
		...
		lstrcpy(ci->ClanInfoHeader.UniqueClaClanInfo_nNum, &ci->ClanInfoHeader.ClanMark_Num[3]);
	}
	else
	{
		ci->ClanInfoHeader.Gserver[0] = ci->ClanInfoHeader.ClanMark_Num[0];
		ci->ClanInfoHeader.Gserver[1] = ci->ClanInfoHeader.ClanMark_Num[1];
		lstrcpy(ci->ClanInfoHeader.UniqueClaClanInfo_nNum, &ci->ClanInfoHeader.ClanMark_Num[4]);
	}
...
void CViewClanInfo::SortDelete()
{
	...
	DELET(ClanInfo[nLowTime].hClanMark);
...
BOOL CViewClanInfo::CheckWhere(char *MarkNum)
{
	...
	HANDLE hFile;
	DWORD dwAcess = 0;
	hFile = CreateFile(szTemp, GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
