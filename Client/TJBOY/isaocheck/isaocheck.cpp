...
BOOL GetHttpRequest(LPSTR httpname, LPSTR Request, DWORD MaxBuffer, int *len)
{
	...
	m_Session = InternetOpen("AppName", INTERNET_OPEN_TYPE_PRECONFIG, NULL, NULL, 0);
	HINTERNET hHttpFile = NULL;
	hHttpFile = InternetOpenUrl(m_Session, (const char *)httpname, NULL, 0, 0, 0);
	if (hHttpFile)
	{
...
int ISAOCHECK::connect_WEBdb(char *szWriteBuffer, char *szReadBuffer)
{
	...
	HINTERNET hHttpFile = NULL;
	hHttpFile = InternetOpenUrl(m_Session, (const char *)szWriteBuffer, NULL, 0, 0, 0);
	if (hHttpFile)
	{
		BOOL bQuery = HttpQueryInfo(hHttpFile, HTTP_QUERY_CONTENT_LENGTH, szSizeBuffer, &dwLengthSizeBuffer, NULL);
		if (bQuery == TRUE)
...
int ISAOCHECK::IsaoCheck(char *id, char *pass, char *ipaddr, long StartTime)
{
	...
	wsprintf(szWriteBuffer, "http://%s/login.php?UserID=%s&PassWord=%s", hostName, id, pass);
	...
	{
	case 0:
		wsprintf(szWriteBuffer, "http://%s/LoginCheck.php?UserID=%s&StartTime=%d", hostName, id, StartTime);
		ret_chk = connect_WEBdb(szWriteBuffer, szReadBuffer);
		ret_chk = chk_readbuffer("account=", szReadBuffer, &iRejectCode);
		switch (ret_chk)
		{
	...
	case 1:
		goto IsaoCheck_Free;
	case 2:
		wsprintf(szWriteBuffer, "http://%s/InsertUser.php?UserID=%s&PassWord=%s&UserName=%s", hostName, id, pass, "aaaa");
		ret_chk = connect_WEBdb(szWriteBuffer, szReadBuffer);
		ret_chk = chk_readbuffer("account=", szReadBuffer, &iRejectCode);
...
int ISAOCHECK::IsaoCheck_GameExit(char *id, char *pass, char *ipaddr, long usertime)
{
	...
	wsprintf(szWriteBuffer, "http://%s/Login_End.php?UserID=%s&IP=%s&EndTime=%d&UseTime=%d", hostName, id, ipaddr, iTimeOut, usertime);
	ret_chk = connect_WEBdb(szWriteBuffer, szReadBuffer);
...
int ISAOCHECK::isCheckClanJang(char *ClanZang_id, char* gserver)
{
	...
	wsprintf(szWriteBuffer, "http://%s/ServerMain/isCheckClanJang.asp?userid=%s&gserver=%s&ticket=%d", hostName, ClanZang_id, gserver, nClanTicket);
...
int ISAOCHECK::isCheckClanName(char *ClanName, char *gserver)
{
	...
	wsprintf(szWriteBuffer, "http://%s/ServerMain/isCheckClanName.asp?ClName=%s&gserver=%s&ticket=%d", hostName, ClanName, gserver, nClanTicket);
...
int ISAOCHECK::ReadClanInfo(char *clanNum, char* ret_Buf)
{
	...
	wsprintf(szWriteBuffer, "http://%s/ServerMain/isCheckClanNum.asp?num=%s&ticket=%d", hostName, clanNum, nClanTicket);
...
int ISAOCHECK::isCheckClanMember(char *pszServerName, char *pszUserName)
{
	...
	wsprintf(szWriteBuffer, "http://%s/ServerMain/isCheckClanwon.asp?gserver=%s&clwon=%s", hostName, pszServerName, pszUserName);

	int cnt = 0;
	while (1)
	{
		receiveCode = connect_WEBdb(szWriteBuffer, szReadBuffer);
		if (receiveCode != ISAO_ERR_RECONNECT)
			break;
		if (cnt > 5)
			break;
		cnt++;
...
int ISAOCHECK::isClanMember(char *ClanZang_id, char* gserver, char* ClanZang_ChaName, char* ret_Buf)
{
	...
	wsprintf(szWriteBuffer, "http://%s/ServerMain/isClanMember.asp?userid=%s&gserver=%s&chname=%s&ticket=%d", hostName, ClanZang_id, gserver, ClanZang_ChaName, nClanTicket);
...
int ISAOCHECK::joinClanMember(char *ClanZang_ID, char *gserver, char *ClanZang_ChaName, char* ClanName, char* ClanWon, char* ClanWon_ID, int level, DWORD chtype, int joinlevel, char *chipflag)
{
	...
	wsprintf(szWriteBuffer, "http://%s/ServerMain/clanInsertClanWon.asp?userid=%s&gserver=%s&chname=%s&clName=%s&clwon=%s&clwonUserid=%s&lv=%d&ticket=%d&chtype=%d&chlv=%d&chipflag=%s", hostName, ClanZang_ID, gserver, ClanZang_ChaName, ClanName, ClanWon, ClanWon_ID, level, nClanTicket, chtype, joinlevel, chipflag);
...
int ISAOCHECK::makeClan(char *ClanZang_ID, char* gserver, char* ClanZang_ChaName, char* ClanName, char *explanation, DWORD chtype, int level, char* data)
{
	...
	wsprintf(szWriteBuffer, "http://%s/Clan/clanInsert.aspx?userid=%s&gserver=%s&chname=%s&clName=%s&expl=%s&ticket=%d&chtype=%d&lv=%d", hostName1, ClanZang_ID, gserver, ClanZang_ChaName, ClanName, explanation, nClanTicket, chtype, level);
	...
	//debug_fd2( "makeClan() 보낸것 길이 : %d 내용 : %s ", lstrlen(szWriteBuffer), szWriteBuffer);
	//debug_fd2( "makeClan() 받은것 길이 : %d 내용 : %s ", lstrlen(szReadBuffer), szReadBuffer);
...
int ISAOCHECK::read_ClanName(int readClumns)
{
	...
	wsprintf(szWriteBuffer, "http://%s/tjboyh/clan_readClanName.php?columnName=%s&ticket=%d", hostName, clumns[readClumns], nClanTicket);
	ret_chk = connect_WEBdb(szWriteBuffer, szReadBuffer);
...
int ISAOCHECK::read_ClanData(char *ClanZang_ID, char* gserver, char* ClanZang_ChaName, char* ret_Buf)
{
	...
	wsprintf(szWriteBuffer, "http://%s/ServerMain/GetAllMyClanMember.asp?userid=%s&gserver=%s&chname=%s&ticket=%d", hostName, ClanZang_ID, gserver, ClanZang_ChaName, nClanTicket);
	...
	//debug_fd2( "read_ClanData() 보낸것 길이 : %d 내용 : %s ", lstrlen(szWriteBuffer), szWriteBuffer);
	//debug_fd2( "read_ClanData() 받은것 길이 : %d 내용 : %s ", lstrlen(szReadBuffer), szReadBuffer);
...
int ISAOCHECK::ReadNotice(char *userid, char *chaname, char *gserver, char *buf)
{
	...
	wsprintf(szWriteBuffer, "%s?userid=%s&gserver=%s&chname=%s&ticket=%d\r\n\r\n\r\n", szNoticeUrl, userid, gserver, chaname, nClanTicket);
	...
	//debug_fd2( "ReadNotice() 보낸것 길이 : %d(ret_chk %d) 내용 : %s ", lstrlen(szWriteBuffer), ret_chk, szWriteBuffer);
	//debug_fd2( "ReadNotice() 받은것 길이 : %d 내용 : %s ", lstrlen(szReadBuffer), szReadBuffer);
...
int ISAOCHECK::Banishiment(char *ClanZang_ID, char *gserver, char *ClanZang_ChaName, char *ClanName, char *ClanWon)
{
	...
	wsprintf(szWriteBuffer, "http://%s/ServerMain/clanWonRelease.asp?userid=%s&gserver=%s&chname=%s&clName=%s&%s&ticket=%d", hostName, ClanZang_ID, gserver, ClanZang_ChaName, ClanName, ClanWon, nClanTicket);
	...
	//debug_fd2( "Banishiment() 보낸것 길이 : %d(ret_chk %d) 내용 : %s ", lstrlen(szWriteBuffer), ret_chk, szWriteBuffer);
	//debug_fd2( "Banishiment() 받은것 길이 : %d 내용 : %s ", lstrlen(szReadBuffer), szReadBuffer);
...
int ISAOCHECK::AppointSubChip(char *gserver, char* clanwon)
{
	...
	wsprintf(szWriteBuffer, "http://%s/ServerMain/ClanSubChipUpdate.asp?chname=%s&gserver=%s&ticket=%d", hostName, clanwon, gserver, 9999);//nClanTicket);
	...
	//debug_fd2( "AppointSubChip() 보낸것 길이 : %d 내용 : %s ", lstrlen(szWriteBuffer), szWriteBuffer);
	//debug_fd2( "AppointSubChip() 받은것 길이 : %d 내용 : %s ", lstrlen(szReadBuffer), szReadBuffer);
...
int ISAOCHECK::ReleaseSubChip(char *gserver, char* clanwon)
{
	...
	wsprintf(szWriteBuffer, "http://%s/ServerMain/ClanSubChipRelease.asp?chname=%s&gserver=%s&ticket=%d", hostName, clanwon, gserver, 9999);//nClanTicket);
	...
	//debug_fd2( "ReleaseSubChip() 보낸것 길이 : %d 내용 : %s ", lstrlen(szWriteBuffer), szWriteBuffer);
	//debug_fd2( "ReleaseSubChip() 받은것 길이 : %d 내용 : %s ", lstrlen(szReadBuffer), szReadBuffer);
...
int ISAOCHECK::LeaveClan(char *clanname, char *chaname, char *gserver)
{
	...
	wsprintf(szWriteBuffer, "http://%s/ServerMain/clanChipChange.asp?chname=%s&gserver=%s&clName=%s&ticket=%d", hostName, chaname, gserver, clanname, nClanTicket);
	...
	//debug_fd2( "LeaveClan() 보낸것 길이 : %d 내용 : %s ", lstrlen(szWriteBuffer), szWriteBuffer);
	//debug_fd2( "LeaveClan() 받은것 길이 : %d 내용 : %s ", lstrlen(szReadBuffer), szReadBuffer);
...
int ISAOCHECK::Secession(char* clanzang, char* gserver, char* SecessionID, char* clanname)
{
	...
	wsprintf(szWriteBuffer, "http://%s/ServerMain/clanWonSelfLeave.asp?userid=%s&gserver=%s&chname=%s&clName=%s&ticket=%d", hostName, clanzang, gserver, SecessionID, clanname, nClanTicket);
...
int ISAOCHECK::isKPFlag(char *userid, char *clanname, char *chaname, char *gserver, char *pflag, char *gunbun)
{
	...
	wsprintf(szWriteBuffer, "http://%s/ServerMain/isKflag.asp?userid=%s&clName=%s&chName=%s&gserver=%s&KFlag=%s&Gubun=%s&ticket=%d", hostName, userid, clanname, chaname, gserver, pflag, gunbun, nClanTicket);
	...
	//debug_fd2( "isPFlag() 보낸것 길이 : %d 내용 : %s ", lstrlen(szWriteBuffer), szWriteBuffer);
	//debug_fd2( "isPFlag() 받은것 길이 : %d 내용 : %s ", lstrlen(szReadBuffer), szReadBuffer);
...
int ISAOCHECK::isPFlag(char *userid, char *clanname, char *chaname, char *gserver, char *pflag, char *gunbun)
{
	...
	wsprintf(szWriteBuffer, "http://%s/ServerMain/isPFlag.asp?userid=%s&clName=%s&chName=%s&gserver=%s&PFlag=%s&Gubun=%s&ticket=%d", hostName, userid, clanname, chaname, gserver, pflag, gunbun, nClanTicket);
...
int ISAOCHECK::WarhouseRight(char *clname, char *right_str)
{
	...
	wsprintf(szWriteBuffer, "http://%s/tjboyh/clan_WarehouseRight.php?clName=%s&whRight=%s&ticket=%d", hostName, clname, right_str, nClanTicket);
...
int ISAOCHECK::updateFlag(char *clname)
{
	...
	wsprintf(szWriteBuffer, "http://%s/tjboyh/updateFlag.php?clName=%s&ticket=%d", hostName, clname, nClanTicket);
...
void clanDATA_save(char *fname, char *buff, int dataLen)
{
	HANDLE hFile;
	char allFname[64];
	wsprintf(allFname, "%s\\%s\\%s", SAVE_DIR, szConnServerName, fname);

	DWORD dwAcess = 0;
	hFile = CreateFile(allFname, GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
...
void clanDATA_save2(char *fname, char *buff, int dataLen)
{
	HANDLE hFile;
	char allFname[64];
	wsprintf(allFname, "%s\\%s", CLANINFO_DIR, fname);
...
int	ISAOCHECK::GetWavFile(char* FileName)
{
	...
	lstrcpy(szNoticeUrl, "211.39.144.158/NPCSound");
	wsprintf(szWriteBuffer, "http://%s/%s.yul", szNoticeUrl, FileName);
	...
	//debug_fd2( "GetWavFile() 보낸것 길이 : %d 내용 : %s ", lstrlen(szWriteBuffer), szWriteBuffer);
	//debug_fd2( "GetWavFile() 받은것 길이 : %d 내용 : %s ", lstrlen(szReadBuffer), szReadBuffer);
...
int ISAOCHECK::read_ClanImage(char *clanname, char *filename)
{
	...
	wsprintf(szWriteBuffer, "http://%s/ClanContent/%s.bmp", hostName1, filename);
	...
	//debug_fd2( "read_ClanImage() 보낸것 길이 : %d 내용 : %s ", lstrlen(szWriteBuffer), szWriteBuffer);
	//debug_fd2( "read_ClanImage() 받은것 길이 : %d 내용 : bmp파일앞의2개만찍음 %c%c. ", lstrlen(szReadBuffer), szReadBuffer[0],szReadBuffer[1]);
	...
	if ((szReadBuffer[0] == 'B') && (szReadBuffer[1] == 'M'))
	{
		...
		{
			server[0] = filename[0];
			lstrcpy(uniqueclannum, &filename[3]);
		}
		else
		{
			server[0] = filename[0];
			server[1] = filename[1];
			lstrcpy(uniqueclannum, &filename[4]);
		}
...
int ISAOCHECK::read_ClanImage2(char *clanname, char *filename, CLANINFO *ci)
{
	...
	//debug_fd2( "read_ClanImage2() 보낸것 길이 : %d 내용 : %s ", lstrlen(szWriteBuffer), szWriteBuffer);
	//debug_fd2( "read_ClanImage2() 받은것 길이 : %d 내용 : bmp파일앞의2개만찍음 %c%c. ", lstrlen(szReadBuffer), szReadBuffer[0],szReadBuffer[1]);
...
char *fileREAD(char *fileName, char *buf, int *len)
{
	...
	DWORD dwAcess = 0;
	char *newBuf;
	hFile = CreateFile(fileName, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
...
int ISAOCHECK::write_ClanImageFile(char *fname)
{
	...
	wsprintf(fileName, "%s\\%s\\%s", SAVE_DIR, szConnServerName, fname);
	...
	for (i = 0; i < HSsu; i++)
	{
		wsprintf(szWriteBuffer, "POST http://%s/tjboyh/clan_insertClanImage.php?file_name=%s&file_HSsu=%d&file_cnt=%d&file_DATA=%s\r\n\r\n\r\n", hostName, OrigFName, HSsu, i + 1, fileDATA_HS[i]);
...
int ISAOCHECK::WEBdb_Exam()
{
	...
	wsprintf(szWriteBuffer, "POST http://%s/tjboyh/ktj_login.php\r\n\r\n\r\n", hostName);
	ret_chk = connect_WEBdb(szWriteBuffer, szReadBuffer);
...
int ISAOCHECK::read_sod2(char *usid, char *charname, char *gserver, int index, char *szSod2Buff)
{
	...
	wsprintf(szWriteBuffer, "http://%s/servermain/sod2info.asp?userid=%s&chname=%s&gserver=%s&index=%d", hostName, usid, charname, gserver, index);
