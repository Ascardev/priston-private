...
FILE *ReadASE_MATERIAL(FILE *fp, smMATERIAL_GROUP **MatGroup = 0)
{
	...
	for (cnt = 0; cnt < aseMaterialCnt; cnt++)
	{
		...
		if (nCnt > 0 && nCnt < cnt && lstrcmpi(aseMaterial[cnt].BITMAP[0], aseMaterial[nCnt].BITMAP[0]) == 0)
		{
		...
		for (cnt = 0; cnt < s_LightMapRefMax; cnt++)
		{
			...
			if (SubPoint)
			{
				if (s_LightMapRefBuff[cnt].bLightMap)
				{
					...
					CopyMemory(&aseMaterial[EndSubPoint], &aseMaterial[SubPoint + 2], sizeof(ASE_MATERIAL) * s_LightMapRefBuff[cnt].MaxSubMtl);
					...
					for (int scrNum = 0; scrNum < aseScriptCnt; scrNum++)
					{
						...
						else if (aseScript[scrNum].MatNum >= SubPoint + 2 && aseScript[scrNum].MatNum <= SubPoint + 2 + s_LightMapRefBuff[cnt].MaxSubMtl - 1)
				...
				else
				{
					...
					for (int scrNum = 0; scrNum < aseScriptCnt; scrNum++)
					{
						if (aseScript[scrNum].MatNum >= SubPoint && aseScript[scrNum].MatNum <= SubPoint + s_LightMapRefBuff[cnt].MaxSubMtl - 1)
...
FILE *ReadASE_GEOMOBJECT(FILE *fp, smOBJ3D *obj, char *szNodeName = 0)
{
	...
	while (!feof(fp))
	{
		...
		{
			if (smPhysique)
				obj->Init(nVertex, nFace, 1);
			else
				obj->Init(nVertex, nFace, 0);
		...
		if (lstrcmp(decode, "*CONTROL_ROT_SAMPLE") == 0 || lstrcmp(decode, "*CONTROL_TCB_ROT_KEY") == 0)
		{
			p = GetWord(strBuff, p);
	...
	if (tFace)
		delete tFace;
	if (tLightMapPoint)
		delete tLightMapPoint;
	if (tLightMapFace)
		delete tLightMapFace;
...
smPAT3D *smASE_Read(char *file, char *szModelName)
{
	...
	int iLength = STRLEN(file);

	SetDirectoryFromFile(file);

	if ((file[iLength - 3] == 'A' || file[iLength - 3] == 'a') ||
		(file[iLength - 3] == 'S' || file[iLength - 3] == 's') ||
		(file[iLength - 3] == 'E' || file[iLength - 3] == 'e'))
	{
		szFile = smFindFile(file, szFileModel);
	}
	else
		szFile = smFindFile(file, nullptr);

	if (smMeshReload && szFile)
	{
...
smSTAGE3D *smSTAGE3D_ReadASE(char *file, smSTAGE3D *smStagep)
{
	...
	if (smMeshReload && (szFile = smFindFile(file, nullptr)))
	{
	...
	for (cnt = 0; cnt < AseLightCnt; cnt++)
	{
		if ((smAseLight[cnt].Type&smLIGHT_TYPE_DYNAMIC) == 0)
		{
			...
			smStage->AddVertexLightRound(&plight, smAseLight[cnt].r, smAseLight[cnt].g, smAseLight[cnt].b, smAseLight[cnt].Range);
		}
		else
			dLightCnt++;
	}
	if (dLightCnt > 0)
	{
		...
		for (cnt = 0; cnt < AseLightCnt; cnt++)
		{
			if ((smAseLight[cnt].Type&smLIGHT_TYPE_DYNAMIC) != 0)
			{
				...
				smStage->AddDynLight(smAseLight[cnt].Type, &plight, smAseLight[cnt].r, smAseLight[cnt].g, smAseLight[cnt].b, smAseLight[cnt].Range);
			}
		}
	}
