...
int	bSql_EventLogon(User *lpPlayInfo );	//이벤트 과금
int	bSql_EventLogOut(User *lpPlayInfo );	//이벤트 로그아웃

int	bSql_GetExpGameTime(User *lpPlayInfo , int GamePlayTime );		//ExpGameTime 기록및 보기 (베트남)

int	bSql_GetGPCode(char *GPCode, char *PG);
int	bSql_PCUse(User *lpPlayInfo );
int	bSql_PCSearchReserv(User *lpPlayInfo );		//과금 연장 확인
int	bSql_ChangeUserIdGpCodeGame( int PCRNo );	//과금 변경

int	bSql_PCRNoReservProduct(User *lpPlayInfo );	//PC방 추가 시간 표시
...
int	bSql_StarPoint(User *lpPlayInfo , int StarPoint , int CashFlag );	//Star Point 기록및 보기
int	bSql_GiveMoney(User *lpPlayInfo , int Money );		//GiveMoney 기록
int	bSql_PollRecord(User *lpPlayInfo , int PollKind , int PollCount , BYTE *PollData );//Polling 기록및 보기
int	bSql_ClanMoney(User *lpPlayInfo , int Cash , int CashFlag , int Kind );	//SOD 공성 클랜 돈창고 
int	bClan_GetClanMarkCode(User *lpPlayInfo );								//클랜 마크 코드 구하기

int	bSql_EventJoin(User *lpPlayInfo );	//이벤트 참여
